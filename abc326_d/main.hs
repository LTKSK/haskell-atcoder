...
import Data.Type.Coercion qualified as L
...
data Cell = A | B | C | Null deriving (Eq, Ix, Ord, Bounded)
...
main = do
  ...
  r <- getLine
  c <- getLine
  let checkCol c grid = c == (map (\row -> head $ dropWhile (== '.') row) $ L.transpose grid)
      rowOk r row = r == (head $ dropWhile (== '.') row)
      -- .が複数表れる場合があるので重複の排除をしている
      perms = S.toList $ S.fromList $ L.permutations $ "ABC" ++ replicate (n - 3) '.'
      -- i毎に条件を満たす列を収集
      okRows = [filter (rowOk (r !! i)) perms | i <- [0 .. n - 1]]
      grids =
        [ rows
          | -- 各listから一つづつ選ぶ組み合わせ
            rows <- sequence okRows,
            all (\col -> L.sort (filter (/= '.') col) == "ABC") (L.transpose rows),
            checkCol c rows
        ]
