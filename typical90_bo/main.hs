...
main = do
  ...
  -- 進数変換
  -- n進法文字列を10進法整数に
  let toTen :: String -> Integer
      toTen s = L.foldl' (\acc d -> acc * 8 + fromIntegral (digitToInt d)) 0 s
      -- 10進法文字列をn進法整数に
      toNine :: Integer -> String
      toNine 0 = ""
      toNine n = toNine (n `div` 9) ++ show (n `mod` 9)
      eightToFive '8' = '5'
      eightToFive c = c
  let res = L.foldl step (show n) [1 .. k]
      step acc _ = map eightToFive $ toNine (toTen acc)

  putStrLn $ if n == 0 then "0" else res
