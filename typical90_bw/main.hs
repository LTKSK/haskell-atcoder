...
-- nの素因数を重複アリでlistにして返す
primeFactorize :: Int -> [Int]
primeFactorize n = L.unfoldr step (n, 2)
  where
    -- 生成終了
    step (1, _) = Nothing
    -- mに対して、pが素因数になりうるかチェック
    step (m, p)
      -- mが素数の時
      -- mが合成数なら、a,b>=2でm=a*bと出来る。この時片方は√m以下の必要がある
      -- pをインクリメントして、2~√mの範囲をチェックしている。なのでp^2>mはp>√mなのでmが素数(割り切れる値がない)
      -- memo: unfoldrでJust(x, (y,z))を返したとき、xが結果のリストに追加される値。(y,z)が次の状態
      | p ^ 2 > m = Just (m, (1, m)) -- ここを通ったらstepの終了pathに進む
      -- mをpで割った余りが0の時、pを素因数に追加。mをqに更新して次に進む
      -- 例えばn=12だったら、(2,6)->(2,3)->(3,3)のように割っていって2,2,3が出力される
      | r == 0 = Just (p, (q, p))
      -- mが素数でもなくmの約数でもないならpを+1
      | otherwise = step (m, p + 1)
      where
        -- 割った結果と余り
        (q, r) = m `divMod` p
...
main = do
  [n] <- ints
  -- 素因数の個数を算出。今回魔法を使って行くと最終的にこの個数のボールを生成することになる
  let len = length $ primeFactorize n
      -- 魔法はボールを2のべき乗で増やしていく。
      ans = solve len 1
      solve l p
        | 2 ^ p >= l = p
        | otherwise = solve l (p + 1)

  -- lenが1の時は素数なので0
  print $ if len == 1 then 0 else ans
