...
main = do
  ...
  -- x座標の2つ分を1つになるよう圧縮する
  let adjust x y = if x `mod` 2 == y `mod` 2 then x else x - 1
      sx' = adjust sx sy
      tx' = adjust tx ty
      dx = abs (sx' - tx')
      dy = abs (sy - ty)
  -- y1つ毎に、xのズレ1つ分タダになる。ここは1マス分だけ
  -- 残った分は2マスにつき1かかるので、2で割る
  -- 先にdxを2で割るとdyで無料になる個数が増えちゃう
  print $ dy + max 0 (dx - dy) `div` 2