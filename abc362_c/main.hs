...
main = do
  ...
  lrs <- replicateM n $
    do
      [l, r] <- ints
      return (l, r)
  -- lとrのrangeでn個sumした時に0になる組みを探す
  let suml = sum $ map fst lrs
      sumr = sum $ map snd lrs
      ok = suml <= 0 && 0 <= sumr

      res = L.foldl' step ini lrs
      -- 削らなければならない値がlの合計値s。
      -- 全部li側から見て、riまでの範囲で移動する数を選ぶ
      -- その際に、各lrで最後までに削りきらなきゃいけない値の合計が-suml
      -- sumlは負の値なので計算しやすくするために-している
      ini = ([], -suml)
      step (acc, s) (l, r) =
        -- この数列で増やせる値
        let v = r - l
            -- 実際に増やす量。sの方が小さい時はsピッタリ削ればよくて、vの方が小さい時はvだけ削る
            xi = min v s
         in ((l + xi) : acc, s - xi)

  printYn ok
  when ok $ do
    putStrLn $ unwords . map show $ reverse $ fst res
