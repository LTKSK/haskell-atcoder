...
main = do
  [n] <- ints
  xyuvs <- replicateM n $ do
    [x, y, u, v] <- ints
    -- 0basedだったので+1
    return [x + 1, y + 1, u + 1, v + 1]
  -- いもす法っぽい
  let cumtwo :: Int -> Int -> [[Int]] -> [[Int]]
      cumtwo h w g = result
        where
          ...
          updates =
            concat
              -- 面積は半開区間で求めるため+1しない
              [[((a, b), 1), ((c, b), -1), ((a, d), -1), ((c, d), 1)] | [a, b, c, d] <- g] ::
              [((Int, Int), Int)]
          diff = accumArray @UArray (+) 0 ((1, 1), (h + 1, w + 1)) updates
          -- 横方向の累積和（各行ごとに左から累積）
          scanned_h =
            [ scanl1 (+) [diff ! (y, x) | x <- [1 .. w + 1]]
              | y <- [1 .. h + 1]
            ]
          -- 縦方向の累積和（各列ごとに上から累積）
  ...
  let csum = cumtwo 1001 1001 xyuvs
      res = M.fromListWith (+) [(v, 1) | v <- concat csum, v /= 0]

  forM_ [1 .. n] $ \i -> do
    let val = M.lookup i res
    print $ fromMaybe 0 val
