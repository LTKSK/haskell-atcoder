...
main = do
  [n] <- ints
  ls <- ints
  -- 移動回数が非常に少ない。なにかシミュレートしてもいいのかも?
  -- +に居たらマイナス方向、-にいたら+方向に向かって移動するとWA
  -- ということは、この方向に向かうのが最適ではない場合があるということ
  -- まずはそれがどういう場合かを考えよう>2^20通りは大した数じゃない。全探索
  let calc ls = L.foldl' step (0, 0.5) $ map fromIntegral ls
        where
          step (acc, pos) l = (acc + c, next)
            where
              next = pos + l
              c = if (pos > 0 && next < 0) || (pos < 0 && next > 0) then 1 else 0
  let res' =
        maximum
          [ fst ans
            | ms <- replicateM n [1, -1],
              let ans = calc $ zipWith (*) ls ms
          ]
  print res'
