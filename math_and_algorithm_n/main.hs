...
main = do
  [n] <- ints
  let arr = sieve (10 ^ 6)
      ps = map fst $ filter (\(_, v) -> v) $ assocs arr
      solve acc 1 _ = acc
      -- 素数使い切った後に1でない場合があり得る。その時はxが素数なので追加
      solve acc x [] = x : acc
      solve acc x (p : ps)
        | r /= 0 = solve acc x ps
        | otherwise = solve (p : acc) q (p : ps)
        where
          (q, r) = x `divMod` p
      res = unwords $ map show $ reverse $ solve [] n ps
      solve2 acc x p
        | p * p > n = if x == 1 then acc else x : acc
        | r == 0 = solve2 (p : acc) q p
        | otherwise = solve2 acc x (p + 1)
        where
          (q, r) = x `divMod` p
      res2 = unwords $ map show $ reverse $ solve2 [] n 2
