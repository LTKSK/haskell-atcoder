...
-- 頂点数と辺の数を返す。木は辺の数+1が頂点数であるので、sndに+1すると答えになる
dfs' :: Array Int [Int] -> Array Int Bool -> Int -> Int -> Int -> (Int, Int)
dfs' g isV k parent v = (cnt, edges)
  where
    selfCount = if isV ! v then 1 else 0
    -- 逆流防止のfilter
    children = filter (/= parent) (g ! v)
    results = map (dfs' g isV k v) children
    cnt = selfCount + sum (map fst results)
    -- 指定頂点数が1以上、k個未満であれば辺として採用する
    -- k個丁度の場合を除外するのは、既にk個ある場合、この頂点へつながるedgeは要らないのである
    edges = sum [e + (if c >= 1 && c < k then 1 else 0) | (c, e) <- results]

main :: IO ()
main = do
  [n, k] <- ints
  abs <- replicateM (n - 1) ints
  vs <- ints
  -- 何かしらとらえ方と探索の仕方で解く問題
  -- vsに含まれない頂点の中で採用しないといけないのが道中にある頂点
  let g = buildGraph (1, n) abs
      -- 指定頂点かどうかの判定用
      isV = accumArray @Array (||) False (1, n) [(v, True) | v <- vs]
      -- 0番目の親は存在しないけど、番兵扱い
      res = dfs' g isV k 0 1
  print (snd res + 1)
