...
main = do
  ...
  xss <- replicateM n $ do
    [l] <- ints
    ints
  let res = L.foldl' step ini xss
      ini = ([], S.empty)
      step (acc, s) xs =
        let fil = [x | x <- xs, not $ S.member x s]
            v = if null fil then 0 else head fil
         in (v : acc, if v == 0 then s else S.insert v s)

  mapM_ print $ reverse $ fst res
