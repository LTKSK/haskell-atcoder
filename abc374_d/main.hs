...
buildWeightedGraph' :: (Int, Int) -> [(Int, Int, Double)] -> Array Int [(Int, Double)]
buildWeightedGraph' (i, n) uvcs = accumArray (flip (:)) [] (i, n) xs
  where
    xs = concatMap (\(u, v, c) -> [(u, (v, c)), (v, (u, c))]) uvcs
...
dijkstra graph start = runST $ do
  dist <- newArray (bounds graph) (10 ^ 9) :: ST s (STArray s Int Double)
  -- スタート地点のコストは0
  writeArray dist start 0
...
main = do
  ...
  abcds <- replicateM n $ do
    ...
    return ([a, b], [c, d])
  -- 向きを作る
  let dirs = replicateM n [True, False]
      ps = L.permutations abcds
      calcCost :: Int -> Int -> Int -> Int -> Int -> Double
      calcCost x y x' y' v = sqrt (fromIntegral ((x' - x) ^ 2 + (y' - y) ^ 2)) / fromIntegral v
      entry [a, b] [c, d] flag = bool [c, d] [a, b] flag
      exit [a, b] [c, d] flag = bool [a, b] [c, d] flag
      res =
        [ (pCost, dCost, pCost + dCost)
          | p <- ps,
            let pCost = sum $ map (\([x, y], [x', y']) -> calcCost x y x' y' t) p,
            d <- dirs,
            let prevNexts =
                  let p' = ([0, 0], [0, 0]) : p
                      d' = True : d
                   in zip (zip d' p') (zip (tail d') (tail p')),
            let dCost =
                  L.foldl'
                    ( \acc ((flag, ([a, b], [c, d])), (flag', ([a', b'], [c', d']))) ->
                        let [x, y] = exit [a, b] [c, d] flag
                            [x', y'] = entry [a', b'] [c', d'] flag'
                         in acc + calcCost x y x' y' s
                    )
                    (0 :: Double)
                    prevNexts
        ]
  print $ minimum $ map thd3 res
