...
-- グリッド用のラッパー
bfsGrid :: UArray (Int, Int) Char -> (Int, Int) -> (Int, Int) -> Int
bfsGrid grid start goal = minimum [dist ! (fst goal, snd goal, d) | d <- [U, D, L, R]]
  where
    ((r0, c0), (r1, c1)) = bounds grid
    bnds3d = ((r0, c0, U), (r1, c1, R))
    v0s = [(fst start, snd start, d) | d <- [U, D, L, R]]
...
bfs' :: IOUArray (Int, Int) Int -> UArray (Int, Int) Char -> UArray (Int, Int) Int -> Seq.Seq (Int, Int, Int) -> IO ()
bfs' dist grid ene queue = case queue of
  Seq.Empty -> return ()
  (y, x, energy) Seq.:<| rest -> do
    if energy == 0
      then do
        bfs' dist grid ene rest
      else do
        ...
        newQueue <-
          foldM
            ( \seq (dy, dx) -> do
                let y' = y + dy
                    x' = x + dx
                if inRange (bounds grid) (y', x')
                  then do
                    nextDist <- readArray dist (y', x')
                    -- ここの判定で、先に踏んだけどenergyが0で、他の経路からだとenergy持ちつつ踏める場合がある
                    -- distを現在の残りエネルギーで更新して、distよりenergyが大きい状態なら通過
                    if nextDist < energy && grid ! (y', x') /= '#'
                      then do
                        writeArray dist (y', x') energy
                        return $ seq Seq.|> (y', x', max (energy - 1) (ene ! (y', x')))
                      else
                        return seq
                  else return seq
            )
            rest
            lrud
        bfs' dist grid ene newQueue

main :: IO ()
main = do
  [h, w] <- ints
  as <- concat <$> replicateM h getLine
  [n] <- ints
  rces <- replicateM n ints
  visited <- newArray @IOUArray ((1, 1), (h, w)) (-1 :: Int)
  let grid = listArray @UArray ((1, 1), (h, w)) as
      ene = accumArray @UArray (+) 0 ((1, 1), (h, w)) $ map (\[r, c, e] -> ((r, c), e)) rces
      (sy, sx) = head [(y, x) | (y, x) <- range $ bounds grid, grid ! (y, x) == 'S']
      goal = head [(y, x) | (y, x) <- range $ bounds grid, grid ! (y, x) == 'T']
  bfs' visited grid ene (Seq.singleton (sy, sx, ene ! (sy, sx)))
  -- 残エネルギー付きのbfs。マスが薬だったらmax取る。ゴールにたどり着けたらtrue
  -- まず初期位置をtrueにしつつ、エネルギーを1,1から取得する
  -- bfsで隣接マスを探索
  -- エネルギーが0なら打ち切り。queueにはエネルギーを入れておかないといけないな
  -- 踏むときに、エネルギーを1消費して、そのマスのエネルギーとmaxを取る
  -- 遠回りしてくるパターンがあって、そっちじゃないとエネルギーを拾えないことがある
  -- bfsじゃないのか...?

  res <- readArray visited goal
  -- getElems visited >>= print
  printYn $ res /= -1
