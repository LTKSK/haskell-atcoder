...
main = do
  [n, q] <- ints
  s <- getLine
  xcs <- replicateM q intChar
  arr <- newListArray @IOUArray (1, n) s
  let ini = length [() | (a, b, c) <- zip3 s (tail s) (tail (tail s)), a == 'A', b == 'B', c == 'C']
      step acc (x, c) = do
        -- xの影響する3か所をチェック
        let idxs = filter (\(pre, cur, nex) -> pre >= 1 && cur >= 1 && cur <= n && nex <= n) [(x, x + 1, x + 2), (x - 1, x, x + 1), (x - 2, x - 1, x)]
            -- read頑張るヘルパー
            isABC :: IOUArray Int Char -> (Int, Int, Int) -> IO Bool
            isABC arr (a, b, c) = do
              a' <- readArray arr a
              b' <- readArray arr b
              c' <- readArray arr c
              return (a' == 'A' && b' == 'B' && c' == 'C')
        prevAbcCount <- mapM (isABC arr) idxs
        let subCount = length (filter id prevAbcCount)

        writeArray arr x c

        afterAbcCount <- mapM (isABC arr) idxs
        let addCount = length (filter id afterAbcCount)
            nextCount = acc - subCount + addCount
        print nextCount
        return nextCount
