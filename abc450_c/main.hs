...
main = do
  ...
  -- .が見つかるたびにdfsして、その回数を保存
  -- ただし、見つかった時にいずれかの.が縁に沿っていてはいけない
  visited <- newArray @IOUArray ((1, 1), (h, w)) False
  valid <- newIORef False
  let bnd = bounds ss
      isEdge (y, x) = y == h || y == 1 || x == w || x == 1
  -- 未訪問の#を探す
  let go [] cnt = return cnt
      go ((y, x) : rest) cnt = do
        ...
        if vis || ss ! (y, x) == '#'
          then go rest cnt
            ...
            -- dfsしたらカウント+1
            modifyIORef' valid (\_ -> True)
            dfs' (y, x)
            v <- readIORef valid
            go rest (cnt + if v then 1 else 0)

      dfs' (y, x) = do
        when (inRange bnd (y, x)) $ do
          v <- readArray visited (y, x)
          when (not v && ss ! (y, x) /= '#') $ do
            writeArray visited (y, x) True
            when (isEdge (y, x)) $ do
              modifyIORef' valid (\_ -> False)
            forM_ lrud $ \(dy, dx) ->
              dfs' (y + dy, x + dx)
  res <- go (range bnd) 0
  print res
