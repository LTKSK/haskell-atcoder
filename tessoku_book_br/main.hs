...
main = do
  [n, m] <- ints
  as <- ints
  xyzs <- replicateM m ints
  -- ランプをbitで表現し、状態の遷移を重さ1のグラフで考えbfsする問題
  let g = accumArray @Array (flip (:)) [] (0, 2 ^ n - 1) edges
      edges = concat [[(i, nextState i x y z) | [x, y, z] <- xyzs] | i <- [0 .. 2 ^ n - 1]]
      -- iをbitにする。x,y,z番目のbitを反転する（1-xで求まる
      -- 反転したbitをintにして返す
      nextState :: Int -> Int -> Int -> Int -> Int
      nextState i x y z = i `xor` bit (x - 1) `xor` bit (y - 1) `xor` bit (z - 1)
      goal = 2 ^ n - 1
      start = L.foldl' (.|.) 0 $ [bit i | (i, a) <- zip [0 ..] as, a == 1]
      res = bfs g start
