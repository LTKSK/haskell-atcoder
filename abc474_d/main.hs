...
main = do
  ...
  as <- integers
  bs <- integers
  -- 逆にNoの時は何？ -> Wi->10^18を超える時、aの全てがb以下の時
  let lim = 10 ^ 18
      abs = zip as bs
      -- behind = map (\(a, b) -> max 0 (b - a)) abs
      lead = map (\(a, b) -> max 0 (a - b)) abs
      -- behindSum = sum behind
      leadSum = sum lead
      -- leadするには、leadの合計がまず0より大きくなければならない
      -- 加えて、behindをdivで割った時に、Wiが10^18以下でなければならない。lim丁度の時は余りがないことも確認する
      -- (q, r) = (behindSum + 1) `divMod` leadSum
      -- canLead = leadSum > 0 && (q < lim || (q == lim && r == 0))
      canLead = leadSum > 0
  printYn canLead
  -- lead可能な時の処理を考える。まずbehindしているところは1で良い
  -- leadしているところは>全部10^18でいいんでね
  let res = zipWith (\a b -> if a > b then lim else 1) as bs
  when canLead $ putStrLn $ unwords $ map show res
