...
main = do
  [n] <- ints
  hs <- ints
  -- 累積和取ってから、自身以上最大の個数...と思ったけど、そのままやったらN^2だ
  -- 後ろから累積マックス取ると？
  -- 種類数を計上するのだとイマイチ。どう言い換えれるか
  -- iから見た時に右側に出てくる数字の種類だけだと、同じ数字がある時に破綻しちゃうのよな
  -- 後ろから見てなんかしらな気がする
  -- let res = L.foldl' step ini (reverse hs)
  --     ini = ([], [0] :: [Int]) -- ここまでのstackの数。詰まっている数
  --     step ([], acc@(accv : _)) h = ([h], (accv + 1) : acc)
  --     step (stack@(sv : r), acc@(accv : _)) h
  --       -- stackの先頭よりhが小さいなら追加
  --       | sv > h = (h : stack, (accv + 1) : acc)
  --       -- hが大きい場合はstackをh以上の値が出てくるまで削る
  --       | otherwise =
  --           let (nextStack, nextAccv) = go stack accv
  --            in (nextStack, nextAccv : acc)
  --       where
  --         -- stackの先頭がhより大きくなるまで削る
  --         go [] acc = ([h], 1)
  --         go stack@(sv : r) acc
  --           | sv <= h = go r (acc - 1)
  --           | otherwise = (h : stack, acc + 1)
  let res = L.foldl' step ini (reverse hs)
      ini = ([], 0, [0])
      step (stack, size, cs) h =
        -- spanでhより大きい要素が出てくるまでstack先頭を削る
        let (popped, rest) = span (<= h) stack
            -- hを追加するので+1
            size' = size - length popped + 1
         in (h : rest, size', size' : cs)

  putStrLn $ unwords $ map show $ tail $ thd3 res
