...
main = do
  ...
  let res = [snd $ L.foldl' step init s' | s' <- splitOn "0" s]
      -- 合計必要な枚数最大値、無地シャツの個数、
      init = (0, 0)
      -- 求めるのはTシャツの必要な枚数。mを変化させていって、不足Tシャツの最低値を求める
      -- 最後に+なら0枚。マイナスならabsとか
      -- 競プロイベントの時には必ず買わなきゃいけないから、0が来るまでの2の個数最大値が答えの候補
      step (muji, logo) c = case c of
        '1' -> let muji' = muji + 1 in if muji' > m then (muji, logo + 1) else (muji', logo)
        '2' -> (muji, logo + 1)
  print $ if null res then 0 else maximum res
