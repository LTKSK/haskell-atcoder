...
main = do
  [n, t] <- ints
  abs_ <- replicateM t intwo
  -- NlogNぐらいまで許容
  -- i毎に変動があるのは一人まで
  -- mapで、同じ得点にいるメンバーの人数を管理。sndの値は現在の種類数
  let ini :: (M.Map Int Int, Int) = (M.singleton 0 n, 1)
  -- 各メンバーの現在位置を管理
  arr <- newArray @IOUArray (1, n) 0
  foldM_
    ( \(m, ans) (a, b) -> do
        cur <- readArray arr a
        let next = cur + b
            isMember = M.member next m
            ans' = ans + if M.member next m then 0 else 1
            ans'' = ans' + if m M.! cur == 1 then -1 else 0
            -- 移動先のスコアの種類数+1
            m' = M.insertWith (+) next 1 m
            -- 移動元のスコアを持っている数-1。0になったらkeyを削除
            m'' = M.update (\v -> if v - 1 == 0 then Nothing else Just (v - 1)) cur m'

        writeArray arr a next
