...
main = do
  ...
  let xs' =
        [ (delta, begin, end')
          | (from, to) <- zip xs (tail xs),
            -- 時計回りに1~nの島があるとして、時計回りに見ていったときのcost
            -- n=5として、1,4=3回。3,2は2+5-3で4回。
            let cost = if to >= from then to - from else to + n - from,
            -- costは時計回りで、島間の移動のコストは一定。なのでnから引くと逆順のコストが求まる
            -- そのcostのうち、短い方を通った場合を計上する
            let shortCost = min cost (n - cost),
            -- shortCostを取らない方がコストの高い経路であり、shortCostを取れないように橋を壊したときは
            -- その高いコストを消費する。その差分は上の計算同様に、
            -- (n - shortCost) - shortCost
            -- となり、以下の式になる。このdeltaが最短の経路にある橋が壊された時の増分
            let delta = n - 2 * shortCost,
            -- 通る辺の開始地点と終了地点。島iはi,i+1を繋ぐから島toに行くには辺to-1を通る
            -- この辺の情報は、imosu法で橋ぶっ壊したときにどれだけコストが増えるかの算出に使う
            let (begin, end') =
                  if cost <= n - cost
                    -- 時計回りの方が効率的な時
                    then (from, to - 1)
      ...
      baseCost =
        sum
          [ min cost (n - cost) | (from, to) <- zip xs (tail xs), let cost = if to >= from then to - from else to + n - from
          ]
      -- imosu法。begin,end'の範囲の橋を壊すと、deltaの分だけコストが増える、というのを計上する
      -- ただしimosu法で配置する範囲に n+1 があるので、 accumArray の範囲を (1, n+1) にしてn+1は後で捨てる
      diffArr =
        accumArray @UArray (+) 0 (1, n + 1) $
          concatMap
            ( \(delta, b, e) ->
                if b <= e
                  then [(b, delta), (e + 1, -delta)]
                  else [(b, delta), (n + 1, -delta), (1, delta), (e + 1, -delta)]
            )
            xs'

      increments = tail $ L.scanl' (+) 0 (elems diffArr)
      edgeIncrements = take n increments -- n+1番目は不要
      -- baseに一番小さな増分を足したものが答え
      ans = baseCost + minimum edgeIncrements
  print ans
