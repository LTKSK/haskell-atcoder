...
main :: IO ()
main = do
  cs <- listArray @UArray (1, 9) . concat <$> replicateM 3 ints
  let rows =
        [ [1, 2, 3], -- 横
          [4, 5, 6], -- 横
          [7, 8, 9], -- 横
      ...
      -- 与えられた選び方で、いつがっかりするのかが分かればよい
      order p m = fromMaybe (-1) $ L.findIndex (\v -> v == m) p
      isDis p = any (disappointedLine p) rows
      -- rowの内容について、pの何番目かでsort。あとは条件の比較
      disappointedLine p line =
        let sorted = L.sortBy (comparing (order p)) line
            [first, second, third] = sorted
         in cs ! first == cs ! second && cs ! first /= cs ! third
      res = (fromIntegral $ sum [1 | p <- L.permutations [1 .. 9], not (isDis p)]) / fromIntegral 362880 -- 9!
  print res
