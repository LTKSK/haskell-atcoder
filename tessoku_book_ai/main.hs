...
main = do
  ...
  let as' = listArray @Array (1, n) as
      bounds = ((1, 1), (n, n))
      -- 2次元dp、遅延評価前提でlistArrayの引数の配列で漸化式を記述するのを典型として覚えておきたい
      dp = listArray bounds ([step ij | ij <- range bounds]) :: Array (Int, Int) Int
      -- iが段差。1行目
      -- 段差をひっくり返して、1>nの方に向かって遷移させる
      -- 遅延評価なので、評価する際に範囲外に行かなければ添え字の範囲をチェックする必要はない
      step (i, j)
        | i == n = as' ! j
