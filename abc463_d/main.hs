...
main = do
  [n, k] <- ints
  lrs <- L.sortBy (comparing snd) <$> replicateM n intwo
  -- 最小値の最大化
  -- 条件をx以上にできるか？で考えて2分探索する問題
  -- 探索範囲はすべての布じゃなくて、隣り合う布だけでOK
  -- 適当に1枚選んだら、探索対象のx以上離れている布をk枚分見ていく
  let check :: Int -> Bool
      -- lastRの初期値が0だと、lがx以下の1枚目を取れなくなってしまう
      check x = go 0 minBound lrs
        where
          go cnt lastR [] = cnt >= k
          go cnt lastR ((l, r) : rest)
            | l < lastR + x = go cnt lastR rest
            | otherwise = go (cnt + 1) r rest
      res = binSearchMax check (-1) (10 ^ 9)
  print $ if res == 0 then -1 else res