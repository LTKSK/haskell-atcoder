...
main = do
  [k] <- ints
  -- 9の倍数なので、1の位は1~9だし、10の倍数の時以外に0は出てこない
  -- どうやってどの数字まで数えるか
  -- dp[桁の和] = 組み合わせ
  let dp = listArray @Array (0, k) [step i | i <- [0 .. k]]
      step 0 = 1
      -- 最後の桁に何を置くかで場合分け。9を置くなら残りの和i-9、2ならi-2といった感じ
      step i = sum [dp ! j | j <- [max 0 (i - 9) .. i - 1]] `mod` modulus
  print $ if k `mod` 9 == 0 then dp ! k else 0
