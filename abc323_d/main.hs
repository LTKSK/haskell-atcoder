...
main = do
  [n] <- ints
  scs <- replicateM n ints

  let solve :: M.Map Int Int -> Int
      solve m = go m 0
        where
          go m acc
            | M.null m = acc
            | odd val = go next (acc + 1)
            | otherwise = go next acc
            where
              ((key, val), rest) = M.deleteFindMin m
              next = if val == 1 then rest else M.insertWith (+) (key * 2) (val `div` 2) rest

  print $ solve $ M.fromListWith (+) $ map (\[s, c] -> (s, c)) scs

-- 各s毎に、合成できる回数が決まっている。sの中で合成するのと他で合成するのに計上される回数に変わりはない
-- ので、まずはsの中で可能な限り合成した方が良いと考えよう
-- その上で、余ったものを合成できるか？を計上する必要がある
-- 合成できなかったスライムは必ず一体残っているはず
-- 各sで計上したら、最後に全部合わせて計算したらいけるかも
-- 最後のステップの計上では、heapに詰めていったらいいんじゃないかなぁ
-- それぞれ倍数を計上するとかでも求まるんだろうか
-- WA
-- let calc [s, c] = go [] s c
--       where
--         go acc s 1 = s : acc
--         go acc s c
--           | even c = go acc (s * 2) (c `div` 2)
--           | otherwise = go (c : acc) (s * 2) (c `div` 2)
-- solve :: H.Heap Int -> Int
-- solve h = go h 1
--   where
--     go h acc
--       | H.size h == 1 = acc
--       | otherwise = case H.uncons h of
--           Nothing -> error "nandeya1"
--           Just (x, rest) -> case H.uncons rest of
--             Nothing -> error "nandeya2"
--             Just (y, rest') ->
--               if x == y
--                 then go (H.insert (x + y) rest') acc
--                 else go (H.insert y rest') (acc + 1)