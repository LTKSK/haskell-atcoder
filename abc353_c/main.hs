...
main = do
  ...
  _as <- ints
  let m = 10 ^ 8
      -- aがそれぞれ何回計算に登場するかを考える
      -- n=4,[1,2,3,4]の時(1,2),(1,3),(1,4),(2,3),(2,4),(3,4)
      -- i番目の値は、n-1回左辺に登場して、i-1回右辺に登場する
      sorted = listArray @UArray (1, n) $ L.sort _as
      -- 左辺に来る回数
      -- s1 = sum [a * (n - i) | (i, a) <- assocs sorted]
      -- 右辺に来る回数
      -- s2 = sum [a * (j - 1) | (j, a) <- assocs sorted]
      s = sum $ map (* (n - 1)) (elems sorted)
      ...
      pairs =
        sum $
          map
            ( \(i, a) ->
                let thres = m - a
                    -- 見つからなかった場合n+1が返って結果が0になる
                    -- 探索範囲にi未満を含めてはいけない。探索後にfilterすると、本来たどりつくべきjに到達しない
                    j = binSearchMin (\idx -> sorted ! idx >= thres) i (n + 1)
                 in n - j + 1
            )
            (assocs sorted)
  -- 総和から、modを取る回数分mを引けば良い
  -- 制約からai+ajが2*mを超えることはあり得ないので
  print $ s - m * pairs