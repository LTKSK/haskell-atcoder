...
nCrMod n r = fact n / (fact r * fact (n - r))
...
-- 参考：https://publish.obsidian.md/naoya/atcoder/ABC345+%E6%8C%AF%E3%82%8A%E8%BF%94%E3%82%8A
main :: IO ()
main = do
  s <- getLine
  -- 出現回数を考える。入れ替えても同じ場合は計上しないことに注意
  -- 全ての組み合わせの数はnC2で、そこから変わらないものを探せばよい
  --  変わらない文字の個数をaccumArrayで集計し、それらから2つ選ぶ方法の数を計上する
  let n = length s
      n' = length $ nubOrd s
      bucket = toBucket ('a', 'z') s
      k = sum [nCr i 2 | i <- elems bucket]

  print $ if n == n' then nCr n 2 else nCr n 2 - k + 1
