...
data Wether = Su | Ra | Null deriving (Eq, Ix, Ord, Bounded, Show)
...
main = do
  ...
  replicateM_ t $ do
    ...
    -- yはn-1個しかないのに注意
    ys <- listArray @UArray (1, n - 1) <$> ints
    let dp = accumArrayDP @UArray next relax ini bnds v0s $ zip [2 ..] (tail s)
        relax = max
        ini = (minBound :: Int)
        bnds = (Su, Ra)
        v0s = case head s of
        ...
        next (u, v) (i, c) =
          case (u, c) of
            (Su, 'S') -> [(Su, v), (Ra, v - xs ! i)]
            (Su, 'R') -> [(Su, v - xs ! i), (Ra, v)]
            (Ra, 'S') -> [(Su, v + ys ! (i - 1)), (Ra, v - xs ! i)]
            (Ra, 'R') -> [(Su, v - xs ! i + ys ! (i - 1)), (Ra, v)]
