...
main = do
  [n, m, h, k] <- ints
  s <- getLine
  xys <- replicateM m intwo
  let move (x, y) c = case c of
        'R' -> (x + 1, y)
        'L' -> (x - 1, y)
        'U' -> (x, y + 1)
        _ -> (x, y - 1)
      solve [] v _ _ = v >= 0
      solve (c : rest) v items pos
        | v' < 0 = False
        | otherwise = solve rest v'' items' pos'
        where
          pos' = move pos c
          v' = v - 1
          hasItem = v' < k && S.member pos' items
          v'' = if hasItem then k else v'
          items' = if hasItem then S.delete pos' items else items
  printYn $ solve s h (S.fromList xys) (0, 0)

-- let solve _ (-1) _ _ = False
--     solve [] v _ _ = v >= 0
--     solve (c : rest) v items (x, y) =
--       let hasItem = S.member (x, y) items
--           v' = if hasItem then max v k else v
--           items' = if hasItem && v < k then S.delete (x, y) items else items
--        in case c of
--             'R' -> solve rest (v' - 1) items' (x + 1, y)
--             'L' -> solve rest (v' - 1) items' (x - 1, y)
--             'U' -> solve rest (v' - 1) items' (x, y + 1)
--             'D' -> solve rest (v' - 1) items' (x, y - 1)
--     hc = head s
-- printYn $ solve (tail s) (h - 1) (S.fromList xys) $ case hc of
--   'R' -> (1, 0)
--   'L' -> (-1, 0)
--   'U' -> (0, 1)
--   'D' -> (0, -1)
