...
main = do
  ...
  forM_ [1 .. t] $ \_ -> do
    [n, h] <- ints
    tlus <- replicateM n ints
    -- 2つの要素を比較して畳み込めばいけそう
    -- minusにならないことに注意して、
    -- 最初のl,rがある、経過時間をdtとして、maximum [l-dt,curL, 0]、min u + dt, crlU
    let res = L.foldl' step (0, h, h, True) tlus
        step (accT, accL, accU, accRes) [t, l, u] =
          let dt = t - accT
              !_ = dbg (dt, max (accL - dt) l, min (accU + dt) u)
              can = not (((accL - dt) > u) || ((accU + dt) < l))
           in (t, max (accL - dt) l, min (accU + dt) u, accRes && can)
        ans (_, _, _, b) = b
