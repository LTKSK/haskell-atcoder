...
main = do
  [h, w, q] <- ints
  rcxs <- replicateM q intIntChar
  -- でけぇindexの範囲から埋めていく
  -- 逆順で考えると一度埋めたら二度は埋まらない
  let grid = accumArray @UArray max (minBound :: Int) ((1, 1), (h + 1, w + 1)) $ [((r, c), i) | (i, (r, c, x)) <- zip [1 ..] rcxs]
      xs = listArray @UArray (1, q) $ map thd3 rcxs
  arr <- newArray @IOUArray ((1, 1), (h + 1, w + 1)) (minBound :: Int)
  forM_ [h, h - 1 .. 1] $ \y -> do
    forM_ [w, w - 1 .. 1] $ \x -> do
      -- arrから読むところとgridから読むところの区別に注意
      right <- readArray arr (y, x + 1)
      bottom <- readArray arr (y + 1, x)
      let cur = grid ! (y, x)
      writeArray arr (y, x) (maximum [right, bottom, cur])
  forM_ [1 .. h] $ \y -> do
    row <-
      mapM
        ( \x -> do
            i <- readArray arr (y, x)
            let c = if i == minBound then 'A' else xs ! i
            return c
        )
        [1 .. w]
    putStrLn row
