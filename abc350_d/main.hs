...
main = do
  ...
  -- 条件を満たしたedgeが何本生やせるか
  -- UnionFindとかもありそうな?>xから見て繋がっているから使えない
  -- xから見た二つの連接node同士がつながっているかを判定する
  -- nodeXから隣接するnode群は手に入るけど
  -- これらが繋がっていないことを判定するには片方のnodeを見て、そこにもう片方のnodeがあるかを見ないといけない
  -- 最終的に完全グラフにならないといけない
  uf <- newUf n
  forM_ abs $ \[a, b] -> uniteUf uf a b
  -- 各頂点で、rootならサイズを集計
  xs <- forM [1 .. n] $ \v -> do
    root <- findUf uf v
    if root == v
