...
main = do
  ...
  s2 <- getLine
  s3 <- getLine
  -- 同時に複数のリールを止めることはできない
  -- いずれか1~9まで、順番に走査して最小値を足す。同じ数字があるときは次に同じ数字が出てくるものを探す
  -- mが100までなのでイイ感じに全探索
  -- まずサンプル1を通す。2,3はedgeケースなので後で考える
  let res =
        [ maximum [i1 - 1, i2 - 1, i3 - 1]
          | v <- ['0' .. '9'],
            (i1, c1) <- zip [1 ..] $ s1 ++ s1 ++ s1,
            c1 == v,
            (i2, c2) <- zip [1 ..] $ s2 ++ s2 ++ s2,
            c2 == v,
            i1 /= i2,
            (i3, c3) <- zip [1 ..] $ s3 ++ s3 ++ s3,
            c3 == v,
            i2 /= i3,
            i1 /= i3
        ]
  -- いずれかの行または数字を固定したい
  -- 愚直に考えたら最初に出てくるindexを走査すればいいが、文字が被っているとダメ
  -- じゃあ見つかったindexにmを足せば？と思うが、次に出てくる可能性もあるので走査しないといけない
  -- 愚直に回すのじゃなくて、足きりを早くすれば...?
