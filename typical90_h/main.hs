...
main = do
  [n] <- ints
  s <- getLine
  -- rが出た時、その地点までのeの数*eより前にあるdの数*dより前にある...
  -- 累積和の計算を工夫したらいけそうな予感がする
  let (_, _, _, _, _, _, res) = L.foldl' step (IntMod 0, IntMod 0, IntMod 0, IntMod 0, IntMod 0, IntMod 0, IntMod 0) s
      step :: (IntMod, IntMod, IntMod, IntMod, IntMod, IntMod, IntMod) -> Char -> (IntMod, IntMod, IntMod, IntMod, IntMod, IntMod, IntMod)
      step (ac, tc, cc, oc, dc, ec, rc) c = case c of
        'a' -> (ac + 1, tc, cc, oc, dc, ec, rc)
        't' -> (ac, tc + ac, cc, oc, dc, ec, rc)
        'c' -> (ac, tc, cc + tc, oc, dc, ec, rc)
        'o' -> (ac, tc, cc, oc + cc, dc, ec, rc)
        'd' -> (ac, tc, cc, oc, dc + oc, ec, rc)
