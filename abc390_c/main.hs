...
main = do
  ...
  -- 黒全体が長方形にならなければならない
  -- 黒いマスのrangeのmin,maxを取得して、その中が?または黒であるかを判定
  let bnds = L.foldl' step ((maxBound, maxBound), (minBound, minBound)) (range $ bounds ss)
      step ((hlo, wlo), (hhi, whi)) (y, x)
        | ss ! (y, x) /= '#' = ((hlo, wlo), (hhi, whi))
        | otherwise = ((min hlo y, min wlo x), (max hhi y, max whi x))
      -- where
      --   !_ = dbg (ss ! (y, x), hlo, y, wlo, x)
      res = and [c == '#' || c == '?' | ix <- range bnds, let c = ss ! ix]
