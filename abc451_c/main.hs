...
takeWhileHeap :: H.Heap Int -> Int -> (H.Heap Int, Int)
takeWhileHeap h n = go h n 0
  where
    go h n c = case H.uncons h of
      Nothing -> (H.empty, c)
      Just (v, next) ->
        if v <= n
          then go next n (c + 1)
          else (H.insert v next, c)

main :: IO ()
main = do
  [q] <- ints
  qs <- replicateM q ints
  let (res, h) = L.foldl' step ini qs
      ini = ([0] :: [Int], H.empty)
      step (acc@(prev : _), h) query = case query of
        [1, x] -> ((prev + 1) : acc, H.insert x h)
        [2, x] ->
          let (next, count) = takeWhileHeap h x
           in -- !_ = dbg (x, next, count, "acc=", acc, "prev=", prev)
              ((prev - count) : acc, next)

  mapM_ print $ tail $ reverse res
