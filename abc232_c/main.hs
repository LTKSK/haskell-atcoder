...
solve n es fs = any ok ps
  where
    ps :: [UArray Int Int]
    ps = map (listArray (1, n)) (L.permutations [1 .. n])
    target = toMask fs
    ok p = toMask [(p ! a, p ! b) | (a, b) <- es] == target

main :: IO ()
main = do
  [n, m] <- ints
  es <- replicateM m intwo
  fs <- replicateM m intwo
  -- グラフの同型判定
  -- nがめっちゃちっちゃい
  -- 全探索できそう？
  -- permutationsで、数値のマッピングを作る
  -- let ps :: [UArray Int Int] = map (listArray (1, n)) (L.permutations [1 .. n])
  --     abGraph = assocs $ buildGraph (1, n) abs
  --     cdGraph = map (\(node, to) -> (node, L.sort to)) $ assocs $ buildGraph (1, n) cds
  --     -- 数値を置き換え
  --     convert (node, to) p = (p ! node, L.sort $ map (p !) to)
  --     res = [L.sort $ map (`convert` p) abGraph | p <- ps]
  -- printYn $ any (== cdGraph) res
  printYn $ solve n es fs
