...
main = do
  [n, k] <- ints
  s <- getLine
  -- MEMO: accumArrayDPのboundsで指定する問題空間は、遷移ごとに合流が起きるものを意識すると見つけやすいかも
  -- 何文字目まで見たか、の一般にiで示される情報はfoldで畳まれるので出てこない
  -- また次元数が大きすぎるとメモリ使い過ぎる
  let isPalin xs = xs == reverse xs
      -- 下位bitに文字列が追加されているので、
      bitsToList len x = [if testBit x j then 1 else 0 | j <- [0 .. len - 1]]
      -- k文字見るまでは追加をやめないので判定用にzip
      dp = accumArrayDP @Array next relax ini bnds v0s $ zip [1 ..] s
      ini = 0 :: Int
      relax = addMod
      -- bitは2^k-1までを取りうる
      bnds = (0 :: Int, 2 ^ k - 1 :: Int)
      -- ここの1を伝播させてrelaxで足し合わせていく。パターンのカウントの場合初期値の1を緩和で追加し続けるイメージ
      v0s = [(0, 1)]
      next (bits, acc) (i, c)
        | acc == 0 = []
        | otherwise =
            [ (newBits, acc)
              | b <- letters c,
                -- 第二引数がk個のbit列で表される文字列
                let window = bitsToList k ((bits `shiftL` 1) .|. b),
                i < k || not (isPalin window),
                -- bのbitを末尾に追加しつつ、2^kからはみ出る範囲をclip
                -- 次に行くときはk-1文字にしておきたい。windowを作る時に初めてk文字あるようにしたいので
                -- bitは2^を作るので、k-1乗。-1するとk-1までのbitが全て1になる
                -- 2^m - 1で下位mbitが全部1。2^mは1の後に0がm個続くbit列なのでね
                let newBits = ((bits `shiftL` 1) .|. b) .&. (bit (k - 1) - 1)
            ]
        where
          letters 'A' = [0]
          letters 'B' = [1]
          letters '?' = [0, 1]
          letters _ = []
  print $ L.foldl' addMod 0 $ elems dp
