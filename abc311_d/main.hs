...
bfs' nextStates initial (lower, upper) v0s = runSTUArray $ do
  dist <- newArray (lower, upper) initial
  forM_ v0s $ \v0 -> do
    writeArray dist v0 0
  aux (Seq.fromList v0s) dist
  return dist
  where
    aux Seq.Empty _ = return ()
    aux (v Seq.:<| queue) dist = do
      d <- readArray dist v
      -- 未到達の隣接頂点リストを取得
      -- initialで初期値（つまり）未訪問の頂点一覧を取得。nextStatesを通すと範囲外や#のマスも削れる
      us <- filterM (fmap (== initial) . readArray dist) (nextStates v)
      -- 隣接頂点をqueueに登録。+1はコストを1とみなしているはずなので、ここももしかしたら抽象化できるかも
      queue' <-
        foldM
          ( \q u -> do
...
main = do
  [n, m] <- ints
  grid <- listArray @UArray ((1, 1), (n, m)) . concat <$> replicateM n getLine
  -- 出発地点は4方向
  let startVs = [(2, 2, d) | d <- [L, R, U, D]]
      around grid (x, y, d) = do
        let u@(x', y', _) = case d of
              ...
              U -> (x, y - 1, U)
              D -> (x, y + 1, D)
        -- 壁にぶつかったら方向転換。ぶつからなかったら今向いている方向のみ。ツルツル～
        if grid ! (x', y') == '#'
      ...
      reachable grid (x, y, _)
        ...
        where
          v = (x, y)
      dist = bfs' (filter (reachable grid) . around grid) (-1) ((1, 1, L), (n, m, D)) startVs
      s =
        L.foldl'
          (\acc (x, y, _) -> S.insert (x, y) acc)
          S.empty
          (map fst . filter (\v -> snd v >= 0) $ assocs dist)
