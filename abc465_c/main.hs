...
main = do
  [n] <- ints
  s <- getLine
  -- 1の右にあるか、左にあるかを考えると、偶数回操作されたら右、奇数回なら左
  -- これ操作回数をカウントで行けるかも。右からと左からの配列作ってガっちゃんこ
  -- 各indexに対する操作回数は、累積和で求められるはず。自身のindexから行われる操作回数
  let psum = L.scanr1 (+) [if c == 'o' then (1 :: Int) else 0 | c <- s]
      left = map fst $ filter (\(i, v) -> odd v) $ zip [2 ..] (tail psum)
      right = map fst $ filter (\(i, v) -> even v) $ zip [2 ..] (tail psum)
      res = (reverse left) ++ [1] ++ right
