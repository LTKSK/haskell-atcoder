...
main = do
  [n, s, l] <- ints
  -- n個の直線状の町のaiが長さ
  as <- listArray @UArray (1, n - 1) <$> ints
  -- lまで貪欲出来る。nは10^4未満で小さい
  -- 答えは最低1は存在する
  -- まず明らかにsum as <= lの時答えはn
  -- シミュレーション系な気がする。8000だから全ての位置から折り返すパターンを試すのが良さそうな予感
  -- n^3だと間に合わないけど、n^2までならイケる
  -- 道中どこで折り返すのが適当かわからないもんな
  -- まずは右にずっとシミュレーションするのを作る
  -- 行って帰って戻っては絶対に最適になりえない
  -- 先に右に途中までいって、折り返すsolveを作ります
  let ls = reverse $ take (s - 1) $ elems as
      rs = drop (s - 1) $ elems as
      -- xsが空になった時
      solve acc cur [] ys
        -- 折り返しコストが払えない時はそのまま
        | cur * 2 > l = acc
        -- 払えるなら折り返す
        | otherwise = inner acc (cur * 2) ys
      solve acc cur (x : xs) ys
        | cur + x <= l = solve (acc + 1) (cur + x) xs ys
        | otherwise = acc

      inner acc _ [] = acc
      inner acc cur (y : ys)
        | cur + y <= l = inner (acc + 1) (cur + y) ys
        | otherwise = acc
      resL = [solve 1 0 ls' rs | i <- [1 .. length ls], let ls' = take i ls]
      resR = [solve 1 0 rs' ls | i <- [1 .. length rs], let rs' = take i rs]
      res = resL ++ resR

  print $ if null res then 1 else maximum res
