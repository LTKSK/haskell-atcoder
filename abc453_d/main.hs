...
data Dir = U | D | L | R deriving (Eq, Ord, Ix, Bounded, Enum, Show)
...
nextDirs :: Char -> Dir -> [Dir]
nextDirs c prevDir = case c of
  'o' -> [prevDir]
  'x' -> filter (/= prevDir) [U, D, L, R]
  _ -> [U, D, L, R]
...
main = do
  [h, w] <- ints
  -- ちょっと凝ったbfsっぽい予感？
  -- ただ単純にvisitedを用意すると上手くいかなそう。別々の方向から踏んだ時にねぇ
  ss <- concat <$> replicateM h getLine

  let grid = listArray @UArray ((1, 1), (h, w)) ss
      -- 座標と、その座標にどの向きで入ってきたかを管理する
      bnds3 = ((1, 1, U), (h, w, R))
      start = head [(y, x) | (y, x) <- range (bounds grid), grid ! (y, x) == 'S']
      goal = head [(y, x) | (y, x) <- range (bounds grid), grid ! (y, x) == 'G']

  -- boolでもいいけどのちの汎化を考えてint
  dist <- newArray @IOUArray bnds3 (-1 :: Int)
  -- 各(y,x,dir)ごとに、どこから到達したかを埋めておく。これをgoalから辿ると経路が復元できる。tupleを入れるのでUArrayには出来ない
  parent <- newArray @IOArray bnds3 (-1, -1, U)

  -- 進める方向だけqueueに入れている
  let v0s =
        [ (fst start, snd start, d)
          | d <- [U, D, L, R],
            let (dy, dx) = dirDelta d
                (sy, sx) = start
                (ny, nx) = (sy + dy, sx + dx),
            inRange (bounds grid) (ny, nx),
            grid ! (ny, nx) /= '#'
        ]
  -- start地点は今まで来た方向が存在しないので、bfs'の外で遷移を手書きする
  -- dist,parentのixにはどの方向で入ってきたかの情報が必要で、startにはそれがない
  forM_ v0s $ \(sy, sx, d) -> do
    let (dy, dx) = dirDelta d
        (ny, nx) = (sy + dy, sx + dx)
    writeArray dist (ny, nx, d) 1
    writeArray parent (ny, nx, d) (sy, sx, d)

  let bfs' queue = case queue of
        Seq.Empty -> return ()
        (y, x, dir) Seq.:<| rest -> do
          curD <- readArray dist (y, x, dir)
          -- 入ってきた方向と、現在のマスの情報から次に迎える向きを求める
          let dirs = nextDirs (grid ! (y, x)) dir
          queue' <-
            foldM
              ( \q nd -> do
                  -- 今注目している座標から移動できる方向のマスをenqueue
                  -- #のマスは除外
                  let (dy, dx) = dirDelta nd
                      (ny, nx) = (y + dy, x + dx)
                  if inRange (bounds grid) (ny, nx) && grid ! (ny, nx) /= '#'
                    then do
                      d' <- readArray dist (ny, nx, nd)
                      -- 訪問済みだったら無視
                      if d' == -1
                        then do
                          writeArray dist (ny, nx, nd) (curD + 1)
                          writeArray parent (ny, nx, nd) (y, x, dir)
                          return (q Seq.|> (ny, nx, nd))
                        else return q
                    else return q
              )
              rest
  ...
  bfs'
    ( Seq.fromList
        [ (ny, nx, d)
          | (sy, sx, d) <- v0s,
            let (dy, dx) = dirDelta d,
            let (ny, nx) = (sy + dy, sx + dx)
        ]
    )

  -- goalにいずれかの方向から到達できたかどうか
  reached <- filterM (\d -> (/= -1) <$> readArray dist (fst goal, snd goal, d)) [U, D, L, R]
  ...
  case reached of
    ...
    (d : _) -> do
      ...
      let trace cur acc = do
            let (y, x, _) = cur
            -- startに到達出来たら終了
            if (y, x) == start
              then return acc
              else do
                p <- readArray parent cur
                let (_, _, dir) = cur
                trace p (dirChar dir : acc)
      path <- trace (fst goal, snd goal, d) []
      putStrLn path
