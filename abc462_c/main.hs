...
main = do
  ...
  -- 点を含まない、をx,yを使った言葉で定義しよう
  -- x,yが条件を満たすときは、まずxについて、y順にsortした時、今まで出てきた値の最小値より小さい
  let yxs = map (\(x, y) -> (y, x)) $ L.sortBy (comparing snd) xys
      (res, _) = L.foldl' step ini yxs
      ini = (0, maxBound :: Int)
      step (acc, cur) (y, x)
        | cur > x = (acc + 1, min cur x)
        | otherwise = (acc, cur)
  print res
