...
main = do
  ...
  as <- ints
  ws <- ints
  -- 一つの箱aiに複数荷物が入っている。箱への入れ方に制限はない
  -- なので、asが重複しているものを先頭からみて、軽い順に動かせばよい
  let aws = L.groupBy (\a b -> fst a == fst b) $ L.sortBy (comparing fst) $ zip as ws
      res = sum [if length g == 1 then 0 else sum $ map snd (init (L.sortBy (comparing snd) g)) | g <- aws]
  print res
