...
main :: IO ()
main = do
  as <- listArray @UArray ((1, 1), (9, 9)) . concat <$> replicateM 9 ints
  let rowRes =
        and
          [ S.size res == 9
            | y <- [1 .. 9],
              let res = S.fromList $ map ((as !) . (y,)) [1 .. 9]
          ]
      colRes =
        and
          [ S.size res == 9
            | x <- [1 .. 9],
              let res = S.fromList $ map ((as !) . (,x)) [1 .. 9]
          ]
      lus :: [(Int, Int)] =
        [ (1, 1),
          (1, 3),
      ...
      squareRes = and [S.size res == 9 | ix <- lus, let res = S.fromList $ map (as !) (range (ix, ix + (3, 3)))]
  printYn (rowRes && colRes && squareRes)
