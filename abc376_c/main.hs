...
main = do
  ...
  -- asのいずれか1つを除いた残りがbsに入らないといけない
  -- 後ろからみて、後回しにした値を覚えておこう
  let solve _ _ [_, _] = [-1] -- 2つ以上あったら無理
      solve [] _ acc = acc
      solve [res] [] [] = [res]
      solve _ [] acc = acc
      solve (a : arem) (b : brem) acc
        | a > b = solve arem (b : brem) (a : acc)
        | otherwise = solve arem brem acc
      res = solve as bs []
  print $ head res
