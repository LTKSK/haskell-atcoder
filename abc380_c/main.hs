...
main = do
  ...
  s <- getLine
  let g = L.group s
      onesIdx = [i | (i, x) <- zip [0 ..] g, head x == '1']
      idxK = onesIdx !! (k - 1)
      (pre, zero : ones : rest) = L.splitAt (idxK - 1) g
  putStrLn $ concat $ pre ++ ones : zero : rest

--     (bk, jk, ak, _) = L.foldl' step ini g
--     ini = ([], [], [], 1)
--     step (beforK, justK, afterK, k') xs
--       -- k-1より前
--       | k' <= k - 1 && h == '0' = (xs : beforK, justK, afterK, k')
--       | k' <= k - 1 && h == '1' = (xs : beforK, justK, afterK, k' + 1)
--       -- 条件を満たしたとき
--       | k' == k && h == '0' = (beforK, justK, xs : afterK, k')
--       | k' == k && h == '1' = (beforK, xs : justK, afterK, k' + 1)
--       | otherwise = (beforK, justK, xs : afterK, k')
--       where
--         h = head xs
-- putStrLn $ concat $ concatMap reverse $ [bk, jk, ak]
