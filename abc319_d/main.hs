...
main = do
  ...
  -- m行の制約で最小を探す
  let s = sum ls
      countLines w ls = fst $ L.foldl' step ini (tail ls)
        where
          ini = (1, head ls) -- 行数、今の行の長さ
          step (lc, llen) l
            | llen + l + 1 <= w = (lc, llen + 1 + l)
            | otherwise = (lc + 1, l)
      -- 1行の時を考えると、sum+lengthになる
      res = binSearchMin (\w -> let lc = countLines w ls in lc <= m) (maximum ls - 1) (s + n + 1)
