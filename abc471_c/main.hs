...
main = do
  [n] <- ints
  as <- integers
  -- 最も近いものを取得しないといけない。同じときはより小さい方へ行く
  -- 愚直にシミュレーションすれば良くないか？
  let lefts = filter (< 0) as
      rights = filter (> 0) as
      ...
      solve cur acc (x : xs) [] =
        let cur' = x
            acc' = acc + abs (cur - x)
         in solve cur' acc' xs []
      solve cur acc (l : ls) (r : rs)
        | abs (cur - l) <= abs (cur - r) =
            let cur' = l
                acc' = acc + abs (cur - l)
             in solve cur' acc' ls (r : rs)
        | otherwise =
            let cur' = r
                acc' = acc + abs (cur - r)
             in solve cur' acc' (l : ls) rs
  print $ solve 0 0 (L.sortBy (comparing Down) lefts) (L.sort rights)
