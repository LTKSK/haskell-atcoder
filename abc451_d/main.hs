...
main = do
  [n] <- ints
  -- べき乗だからm進数として考えるのはだめそう。2のべき乗の範囲がたくさんある
  -- 1_000_000_000以下の値ってことは、2^29までの値で作る536870912が2^29
  let pows = takeWhile (<= 1000000000) $ map (2 ^) [0 .. 29]
      digits x = length (show x)
      pk = [(digits p, p) | p <- pows]
      -- 桁数kの2のべき乗を収集
      pkOf k = [p | (d, p) <- pk, d == k]

  -- xk = 丁度k桁の良い整数の集合
  -- 10^9は良い数ではなく、そうすると答えは10^9未満なので9桁
  -- lookupで探す用にkeyとvalのtupleになっている
  -- 0を用意しておかないとvの計算が上手くいかない
  let xk = (0, [0]) : [(k, gen k) | k <- [1 .. 2]]
      -- 生成の流れは、1~9桁までについて、
      gen k =
        IS.toList $
          IS.fromList
            [ v
              | i <- [1 .. k], -- 末尾に置く2の乗数の桁数
                let xki = lookup' (k - i), -- 先頭に置く良い整数リスト
                x <- xki, -- 先頭部分の要素
                p <- pkOf i, -- 作成済みの2のべき乗一覧
                -- 結合。i桁ずらして,i桁の値を配置
                let v = x * (10 ^ i) + p
            ]
      lookup' k = fromMaybe [] (lookup k xk)
      res = L.sort $ concatMap snd $ tail xk
