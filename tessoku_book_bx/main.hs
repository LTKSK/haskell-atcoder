...
-- dpで足場までの移動手段を集計したい
-- ジャンプの結果は都度modulusで割っておこう
-- ジャンプの移動範囲がl以上r以下なので、ジャンプ先を高速に算出できる必要がある
-- xs' = listArray @UArray (1, n + 1) (xs ++ [w])
-- x_iから見て、x_i-l<=x_j<=x_i-rとなるx_jの個数を高速に求めたい
-- これはx_rを上記条件を満たす最大のindex。x_lを上記条件を満たす最小のindexとして、
-- x_r - x_{l-1}で累積和を取るとO(1)で求まる
-- 先に範囲内の足場の個数を求めよう
-- dp = listArray @Array (1, n + 1) [step i | i <- [1 .. n + 1]]
-- step 1 = 1
-- step i =
--   sum
--     [ dp ! j
...
calcRanges :: UArray Int Int -> Int -> Int -> Int -> Array Int (Int, Int)
calcRanges xs' n l r = listArray (1, n + 1) $ go 1 0 0
  where
    go i lo hi
      | i > n + 1 = []
      | otherwise =
          let xi = xs' ! i
              -- hiを右に進める（xi - l 以下の最大インデックス）
              hi' = moveHi xi hi
              -- loを右に進める（xi - r 以上の最小インデックス）
              lo' = moveLo xi lo hi'
           in (lo', hi') : go (i + 1) lo' hi'

    -- xi - l 以下の最大インデックスを探す
    moveHi xi hi
      | hi >= n = n -- 範囲の上限
      | hi < n && xs' ! (hi + 1) <= xi - l = moveHi xi (hi + 1)
      | otherwise = hi

    -- xi - r 以上の最小インデックスを探す
    moveLo xi lo hi'
      | lo > hi' = lo -- loがhiを超えたらそのまま
      | lo < n + 1 && xs' ! lo < xi - r = moveLo xi (lo + 1) hi'
      | otherwise = lo
...
main = do
  [n, w, l, r] <- ints
  xs <- ints
  let xs' = listArray @UArray (0, n + 1) (0 : xs ++ [w])
      res = runST $ do
        dp <- newArray (0, n + 1) 0 :: ST s (STUArray s Int Int)
        -- dp[0]+dp[1]+...+dp[i]の累積和を管理する配列
        -- loが0の時にlo-1でエラーにならないよう-1からスタート。累積和の配列はlo-1がある点に注意
        dpsum <- newArray (-1, n + 1) 0 :: ST s (STUArray s Int Int)
        writeArray dp 0 1 -- 初期値。西岸到達方法は1通り
        writeArray dpsum 0 1 -- 初期値。dp0までの累積和は1
        forM_ [1 .. n + 1] $ \i -> do
          -- iの座標から見て、jの範囲の両端のindexを算出する
          let xi = xs' ! i
              -- xiから見るとl,rが逆に見える点に注意(xi-rが左端だよ)
              lo = binSearchMin (\j -> xs' ! j >= xi - r) (-1) (n + 1)
              hi = binSearchMax (\j -> xs' ! j <= xi - l) (-1) (n + 1)
          sl <- readArray dpsum (lo - 1)
          sr <- readArray dpsum hi

          let delta = (sr - sl) `mod` 1_000_000_007

          writeArray dp i delta
          sv' <- readArray dpsum (i - 1)
          writeArray dpsum i ((sv' + delta) `mod` 1_000_000_007)
        readArray dp (n + 1)
  print res

-- 再帰版。おもーい
-- dp = listArray @Array (0, n + 1) [stepDp i | i <- [0 .. n + 1]]
-- sumDp = listArray @Array (-1, n + 1) [stepSumDp i | i <- [0 .. n + 1]]
-- stepDp 0 = 1
-- stepDp i =
--   let xi = xs' ! i
--       lo = binSearchMin (\j -> xs' ! j >= xi - r) (-1) (n + 1)
--       hi = binSearchMax (\j -> xs' ! j <= xi - l) (-1) (n + 1)
--    in sumDp ! hi - sumDp ! (lo - 1) `mod` 1_000_000_007

-- stepSumDp (-1) = 0
-- stepSumDp 0 = 1
-- stepSumDp i = dp ! (i - 1) + stepSumDp (i - 1) `mod` 1_000_000_007
