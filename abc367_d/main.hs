...
main = do
  [n, m] <- ints
  as <- ints
  let psum = listArray @UArray (1, n * 2) $ map (`mod` m) (L.scanl' (+) 0 (as ++ as))
      res = runST $ do
        b <- newArray (0, m - 1) 0 :: ST s (STUArray s Int Int)
        -- nまでで、mod mした値の個数を集計
        forM_ [1 .. n] $ \k -> do
          let r = psum ! k
          v <- readArray b r
          writeArray b r (v + 1)
        -- ここからはn+1..2*nのiについて、i-n+1からi-1まででiと同じ数の個数を計上していく
        let go i acc
              | i > 2 * n = return acc
              | otherwise = do
                  -- n個前が一周した自身の位置なのでまずはそれを除外
                  let rout = psum ! (i - n)
                  vout <- readArray b rout
                  writeArray b rout (vout - 1)
                  -- 除外した後に個数を確認
                  c <- readArray b (psum ! i)
                  -- 自身の個数を書き戻し
                  let rin = psum ! i
                  vin <- readArray b rin
                  writeArray b rin (vin + 1)
                  go (i + 1) (acc + c)
        go (n + 1) 0
  print res
