...
main = do
  ...
  let w = a + b
      -- dsを曜日に変換して、早い順にする
      d' = L.sort $ map (`mod` w) ds
      -- 予定の入っていない日数を差分から計上。head d'+wがないと要素が一つ足りない
      gaps = zipWith (-) (tail d' ++ [head d' + w]) d'
  -- 最大の予定が入っていない区間がbより大きければOK
  -- 日にちなので、1日目と5日目の休日を達成するには5日間必要
  -- >=bとしちゃうとこの1日の差で落ちる
  printYn $ maximum gaps > b
