...
main = do
  [n] <- ints
  as <- listArray @UArray ((1, 1), (n, n)) . concat <$> replicateM n ints
  [m] <- ints
  xys <- replicateM m $ do
    [x, y] <- ints
    return [(x, y), (y, x)]
  let xys' = S.fromList (concat xys)
  let p = L.permutations [1 .. n]
      l = [sum vs | vi <- p, all (\(a, b) -> not (S.member (a, b) xys')) (zip vi (tail vi)), let vs = map (as !) (zip vi [1 ..])]
      res = if not (null l) then minimum l else -1
