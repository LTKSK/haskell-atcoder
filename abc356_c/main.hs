...
readInt' s = let Just (n, _) = BS.readInt s in n

main :: IO ()
main = do
  [n, m, k] <- ints
  -- 鍵が15本までだから、bitで管理する予感
  -- k本以上差さないとで、cがkより小さい時がある
  rows <- replicateM m $ do
    ws <- BS.words <$> BS.getLine
    let c = readInt' (head ws)
        r = BS.head (last ws)
        as = map readInt' (tail (init ws))
    return (S.fromList as, r)

  -- 全部の組み合わせ全部testするのが早い
  let res =
        [ ()
          | p <- S.fromList <$> L.subsequences [1 .. n],
            let c =
                  and
                    -- oの時sはk本以上。xの時k未満
                    [ if r == 'o' then s >= k else s < k
                      | (as, r) <- rows,
                        let s = S.size $ S.intersection as p
                    ],
            c
        ]
