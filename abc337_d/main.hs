...
main = do
  ...
  let solve s = if null costs then maxBound else minimum costs
        where
          len = length s
          dots = listArray @UArray (0, len) $ L.scanl' (\acc c -> acc + fromEnum (c == '.')) 0 s
          xs = listArray @UArray (0, len) $ L.scanl' (\acc c -> acc + fromEnum (c == 'x')) 0 s
          costs =
            [ dots ! (l + k - 1) - dots ! (l - 1)
              | l <- [1 .. len - k + 1],
                xs ! (l + k - 1) - xs ! (l - 1) == 0
            ]
      resCol = minimum $ map solve ss
      resRow = minimum $ map solve $ L.transpose ss
      res = min resCol resRow
  print $ if res == maxBound then -1 else res

-- h,wは結構大きめ
-- 縦か横がそろえばOK。斜めの無いビンゴ的なものがあるかどうかが分かればよい
-- xで区切って、その中の要素数がk以上で、.の数が最小のものを探す。h*w<=2*10^5だから、全探索でも行けそうな？
-- length数えるのと、.の計上をイイ感じにやらねば
-- 一旦素直にfold書く
-- let rowInis = map (L.foldl' step ini . take k) ss
--     rowRes = [L.foldl' step' ini (zip s $ drop k s) | (s, ini) <- zip ss rowInis]
--     ss' = L.transpose ss
--     colInis = map (L.foldl' step ini . take k) ss'
--     colRes = [L.foldl' step' ini (zip s $ drop k s) | (s, ini) <- zip ss' colInis]

--     -- resRow = map (L.foldl' step ini) ss
--     -- resCol = map (L.foldl' step ini) $ L.transpose ss
--     -- 現在見れた個数。見た文字列
--     ini = (0, 0, maxBound :: Int) -- 現在の○の数と.の数と、答え
--     step (cc, dc, ans) 'x' = (0, 0, ans)
--     step (cc, dc, ans) c = (cc', dc', ans')
--       where
--         cc' = cc + if c == 'o' then 1 else 0
--         dc' = dc + if c == '.' then 1 else 0
--         -- MEMO: これだとoを先にconsumeすることができないのか...
--         -- 出来るかどうかだけなら簡単だけど、いくつ置き換えたかを判定しないといけないからなぁ
--         -- 尺取り的につながっている範囲をずらして計上するのが必要？
--         -- 最初にk個takeして、その中にある○と.の数を計上。ずらしていって、oと.の合計がkを超えているとき、.の値をminで更新
--         -- k個の範囲から出た要素を考慮できていない
--         ans' = if cc' + dc' >= k then min ans (max 0 (k - cc')) else ans

--     step' (cc, dc, ans) (prev, 'x') = (0, 0, ans)
--     step' (cc, dc, ans) (prev, c) = (cc', dc', ans')
--       where
--         cc' = cc + (if c == 'o' then 1 else 0) + (if prev == 'o' then -1 else 0)
--         dc' = dc + (if c == '.' then 1 else 0) + (if prev == '.' then -1 else 0)
--         ans' = if cc' + dc' >= k then min ans (max 0 (k - cc')) else ans
--     rowRes' = thd3 $ minimumBy (comparing thd3) rowRes
--     colRes' = thd3 $ minimumBy (comparing thd3) colRes
--     res = min rowRes' colRes'
