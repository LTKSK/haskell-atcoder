...
main = do
  [n] <- ints
  as <- ints
  -- 最初は値の地点でとれる高さは2、次で1、最後は2に出来ると思いきや残りがないのでダメ
  -- 前の値の+1までに制限される
  let calc a b = if b > a then a + 1 else b
      as' = map (uncurry min) $ zip [1 ..] as
      as'' = map (uncurry min) $ zip [1 ..] $ reverse as
      psum = L.scanl1 calc as'
      ssum = L.scanl1 calc as''

  print $ maximum $ map (uncurry min) $ zip psum $ reverse ssum
