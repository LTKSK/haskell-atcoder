...
-- 精度問題回避で整数版。10^18とか投げ込んだ時に上手くいかないことがあったので
log2LE k
  | k <= 1 = 0
  | otherwise = 1 + log2LE (k `div` 2)
...
digitDP ::
  [Int] -> -- nの各桁
  s -> -- 初期状態
  (s -> Int -> s) -> -- 遷移。現在の状態と選んだ数字から次の状態を演算
  (s -> Bool) -> -- カウント対象か判定
  (s -> Int -> Bool) -> -- その桁でdを選べるか
  Integer
digitDP digits s0 trans judge canPick = go 0 True s0
  where
    len = length digits
    go i tight s
      | i >= len = if judge s then 1 else 0
      | otherwise =
          let limit = if tight then digits !! i else 9
           in sum
                [ go (i + 1) (tight && d == limit) (trans s d)
                  | d <- [0 .. limit],
                    canPick s d
                ]
...
main = do
  ...
  let solve :: Integer -> Integer
      solve n
        | n < 10 = 0
        | otherwise =
            let ds = map digitToInt $ show n
                len = length ds
                -- lenより小さい桁の蛇数を数える
                -- 例えば3421の時に3桁以下の桁数の値
                -- これは先頭の3を気にせず、999,998...10まで数えてしまってよい
                smaller =
                  sum
                    [ h ^ (d - 1) -- 先頭以外、つまりd-1桁に対して0~h-1のh通りを配置できるのでh^(d-1)個
                      | d <- [2 .. len - 1], -- 桁の数。10以上なので2桁からなのに注意
                        h <- [1 .. 9] -- d桁の値の先頭が1~9までの時それぞれを計算
                    ]
                -- 桁数がlenで先頭がnの左端の値未満の蛇数
                sameLen =
                  fromIntegral $
                    sum
                      [ h ^ (len - 1)
                        | h <- [1 .. head ds - 1] -- head dsは3421だったら3。各桁3未満の蛇数は2^len-1(先頭がないから)+1^len-1
                      ]
                -- 残り計算しなければならないのは、桁数がlenで先頭桁がnの左端丁度の場合（桁DP部分）
                top = head ds
                -- go i: i桁目以降を埋めるtightな時、dsのi番目の値が上限
                go i
                  | i >= len = 1
                  | otherwise =
                      -- 3421でi=1なら4が上限
                      let limit = ds !! i
                          -- ds !! i未満を選んだ時、残りは自由
                          -- 最初のminは3412で[1]なら3、[2]なら1となる。蛇数の制約
                          -- 最後の計算は、top未満の数（0を含むからtop個）で、それの桁数分
                          freeCount = fromIntegral (min top limit) * fromIntegral top ^ (len - 1 - i)
                       in -- ds !! iちょうどを選べる場合はtightのまま次の桁に進む
                          freeCount + if limit < top then go (i + 1) else 0
             in -- 先頭の桁はtopで固定している。2つ目のds[1]から探索
                smaller + sameLen + go 1
