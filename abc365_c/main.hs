...
main = do
  ...
  as <- L.sort <$> ints
  -- 上限額*n<=mでないといけない
  -- sum as <= mであればinfinite
  -- xをm/nにすると少なくともmは超えない
  -- 最大値を求めるのだから、x-asがm/nを超えた分だけ総和を再分配できるはず
  let sa = sum as
      isInf = sa <= m
      psum = listArray @UArray (0, n) $ L.scanl' (+) 0 as
      as' = listArray @UArray (1, n) as
      mn = m `div` n
      res' =
        binSearchMax
          ( \x ->
              let i = binSearchMin (\j -> as' ! j >= x) 0 n
                  moreCount = n - i + 1 -- xの位置を含む右側の個数
                  lessVal = psum ! (i - 1) -- xより小さい左側の合計値
               in ((moreCount * x) + lessVal) <= m
          )
          1
          (10 ^ 9)
  putStrLn $ if isInf then "infinite" else show $ res'
