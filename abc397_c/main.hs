...
main = do
  [n] <- ints
  as <- ints
  -- 同じ数字をなるべく含まない場所で分割する必要があるな
  -- 両端からSetに入れてサイズを残す
  let (fromLeft, _) = L.foldl' (\(acc, s) a -> let s' = S.insert a s in ((S.size s') : acc, s')) ([], S.empty) as
      (fromRight, _) = L.foldl' (\(acc, s) a -> let s' = S.insert a s in ((S.size s') : acc, s')) ([], S.empty) $ reverse as
      res = maximum $ zipWith (+) fromLeft fromRight

  print $ maximum $ zipWith (+) (tail fromRight) $ reverse fromLeft
