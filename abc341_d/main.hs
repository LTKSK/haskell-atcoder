...
solve :: Int -> Int -> UArray Int Bool
solve n k = runSTUArray $ do
  arr <- newArray (1, n * k) False
  -- Intそのままだと通らないのでfromIntegralを通す
  forM_ [n, n + n .. n * k] $ \j ->
...
main = do
  [n, m, k] <- integers
  -- nで割れる数を列挙する
  -- なんかしらxがあって、それの約数の数はnとmでそれぞれ割って、lcm m nを引けば良い
  -- nもmそれぞれの約数の計上から、lcmの約数を引かないといけないので*2
  -- nで割れる集合とmで割れる集合にlcmで割れる集合は2つ分出てくる
  -- 単調増加なので2分探索が使える
  let check x =
        let nd = x `div` n
            md = x `div` m
            mnd = x `div` lcm m n
         in (nd + md - mnd * 2) >= k
      res = binSearchMin check 0 (10 ^ 19)
  print res
