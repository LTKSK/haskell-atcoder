...
data Hands = Rock | Paper | Scissors deriving (Eq, Ix, Ord, Bounded, Show)
...
main = do
  ...
  s <- getLine
  -- sは青木君の出した手
  -- 高橋君は全てあいこか勝っており、i番目とi+1番目の手は異なる。この制約の中で勝った回数の最大値を求める
  -- dpっぽい
  let dp = accumArrayDP @UArray next relax ini bnds v0s (tail s)
      ini = 0 :: Int
      relax = max
      bnds = (Rock, Scissors)
      v0s = case head s of
        'R' -> [(Rock, 0), (Paper, 1)]
        'P' -> [(Paper, 0), (Scissors, 1)]
        'S' -> [(Scissors, 0), (Rock, 1)]
      -- phが前回高橋くんが出したやつ。hは今の青木君の手
      next (ph, acc) h = case (ph, h) of
        -- rockは出せないし、負けられない
        (Rock, 'R') -> [(Paper, acc + 1)]
        (Rock, 'P') -> [(Paper, acc), (Scissors, acc + 1)]
        (Rock, 'S') -> [(Scissors, acc)]
        -- paperは出せないし、負けられない
        (Paper, 'R') -> [(Rock, acc)]
        (Paper, 'P') -> [(Scissors, acc + 1)]
        (Paper, 'S') -> [(Rock, acc + 1), (Scissors, acc)]
        -- scissorsは出せないし、負けられない
        (Scissors, 'R') -> [(Paper, acc + 1), (Rock, acc)]
        (Scissors, 'P') -> [(Paper, acc)]
        (Scissors, 'S') -> [(Rock, acc + 1)]
