...
main = do
  s <- getLine
  -- 連続した文字列かと思ったらそうじゃなかった
  -- 重複なくa,b,cのindexを選ぶ手段。s//3までの個数しか無理
  -- 1.累積和で左と右のABCを集計する
  -- 悪くないかも？あるAがある時、そこより右にあるBの個数と、そこより右にあるCの個数を見れば良い
  -- まてよ、右端から見て、Cの個数とBの個数を数えておけばよいのでは？
  let rs = reverse s
      (_, res) = L.foldl' step ini rs
      ini = ((0, 0), 0)
      step ((bc, cc), acc) c
        | c == 'C' = ((bc, cc + 1), acc)
        | c == 'B' = ((min (bc + 1) cc, cc), acc)
        | c == 'A' && bc > 0 = ((bc - 1, cc - 1), acc + 1)
        | otherwise = ((bc, cc), acc)
