...
main = do
  [n] <- ints
  ps <- ints
  let (i, p) = f $ reverse (zip [1 ..] ps)
      -- 後ろから見ていって、単調減少しない最初の値を取る。
      -- 9 8 6 5 10 3 1 2 4 7だったら(6,3)
      f [x1] = x1
      f (x1 : x2 : xs)
        | snd x1 > snd x2 = f (x2 : xs)
        | otherwise = x2
      left = take (i - 1) ps
      right = drop i ps
      (smaller, larger) = span (< p) right
      mid = last smaller
      res = left ++ mid : (L.sortBy (comparing Down) $ p : init smaller ++ larger)
