...
main = do
  [n] <- ints
  -- 規則に則って出力する系。n-1のカーペットがつくれれば、あとは中央を真っ白にすりゃ良さそう
  let makeGrid :: Int -> [[Char]]
      makeGrid 0 = [['#']]
      makeGrid v =
        -- 1段小さいgridを作ってそれを合成する
        let block = makeGrid (v - 1)
            w = 3 ^ (v - 1) -- 1辺
            whites = replicate w (replicate w '.')
            -- 単純なconcatじゃダメで、横にくっつけないとだめ
            -- [["#"],["#"],["#"]] -> ["###"]
            top = zipWith (++) block (zipWith (++) block block)
            -- [["#"],["."],["#"]] -> ["###"]
            mid = zipWith (++) block (zipWith (++) whites block)
            -- [["#"],["#"],["#"]] -> ["###"]
            bot = zipWith (++) block (zipWith (++) block block)
         in -- こっちが縦につなげる。上から順につながってるだけだけど
            -- ["###",
            --  "#.#",
            --  "###"]
            top ++ mid ++ bot
