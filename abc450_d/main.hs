...
main = do
  [n, k] <- ints
  as <- L.sort . (map (`mod` k)) <$> ints
  let as' = as ++ (map (+ k) $ init as)
      arr = listArray @UArray (1, n * 2 - 1) as'
  -- 数列をどの範囲の値にkの加算でまとめられるか
  -- まずmod kとると、kの範囲での各値の位置がわかるはず。これをsortしている
  -- あとは、+kした要素も含めて見ていけばいいかな？
  -- n=5だったら、1,2,3,4,5,1',2',3',4'と、して、i+n-1の最小値を探す
  print $ minimum [arr ! j - arr ! i | (i, j) <- zip [1 .. n] [n .. n * 2 - 1]]
