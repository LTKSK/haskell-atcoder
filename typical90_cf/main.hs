...
main = do
  [n] <- ints
  s <- getLine
  -- let m = listArray (0, n) $ L.scanl' step 0 s :: UArray Int Int
  --     step acc c = case c of
  --       'o' -> acc + 1
  --       'x' -> acc + 0
  --     b = listArray (0, n) $ L.scanl' step' 0 s :: UArray Int Int
  --     step' acc c = case c of
  --       'o' -> acc + 0
  --       'x' -> acc + 1
  -- shaku l r acc
  --   | l >= r = acc
  --   | r > n = acc
  --   | maru >= 1 && batsu >= 1 = shaku l (r + 1) acc
  --   | otherwise = shaku (l + 1) r (acc + (r - l))
  --   where
  --     maru = m ! r - m ! (l - 1)
  --     batsu = b ! r - b ! (l - 1)
  -- res = shaku 1 2 0
  ...
  -- ○または×のみしか含まれない区間を計算する
  -- ○のみの区間の選び方は連続区間の選び方で等差数列。初項1、末項n、項数n
  let total = n * (n + 1) `div` 2
      rl = sum $ map (\v -> v * (v + 1) `div` 2) $ map snd $ runLength s
  print $ total - rl
...
--           | c == 'o' = (i, b) -- aを更新
--           | otherwise = (a, i)
--         -- 最後に出てきたindexを覚える累積計算
--         cum = L.scanl' step (0, 0) (zip [1 ..] s)
--         vs = map (uncurry min) cum
-- print $ solve s
