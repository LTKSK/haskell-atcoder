...
main = do
  [n, q] <- ints
  as <- listArray @UArray (1, n) . L.sort <$> ints
  bks <- replicateM q intwo
  -- 距離で二分探索する問題だ。距離dを二分探索して、k個になる最短の距離を求める
  forM_ bks $ \(b, k) -> do
    let res = binSearchMin check (-1) ((10 ^ 8) * 2 + 1)
        check d =
          let leftIdx = binSearchMin (\i -> (as ! i) >= (b - d)) 0 (n + 1)
              rightIdx = binSearchMax (\i -> (as ! i) <= (b + d)) 0 (n + 1)
           in rightIdx - leftIdx + 1 >= fromIntegral k
    print res
