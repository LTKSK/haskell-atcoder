...
main = do
  ...
  as <- ints
  let init = ([], []) -- res,stack
      (res, _) = L.foldl' step init (zip [1 ..] as)
      step :: ([Int], [(Int, Int)]) -> (Int, Int) -> ([Int], [(Int, Int)])
      step ([], []) ia = ([-1], [ia])
      step acc ia = innerStep acc ia
      -- accの先頭からa以下の値を取り除く
      innerStep (res, acc) (i, a) =
        let filtered = dropWhile (\(_, accV) -> accV <= a) acc
         in -- !_ = dbg (acc, a, filtered)
            if null filtered then ((-1) : res, [(i, a)]) else (fst (head filtered) : res, (i, a) : filtered)
