...
main = do
  ...
  -- 25314
  -- 12345
  arrP1 <- newListArray @IOUArray (1, n) ps
  arrP2 <- newListArray @IOUArray (1, n) $ map fst $ L.sortBy (comparing snd) (zip [1 :: Int ..] ps)
  isP1 <- newIORef True
  replicateM_ q $ do
    q <- ints
    case q of
      [1, x, y] -> do
        ...
        (p, r) <-
          if flag
            then return (x, y)
            else (,) <$> readArray arrP2 x <*> readArray arrP2 y

        xv <- readArray arrP1 p
        yv <- readArray arrP1 r
        writeArray arrP1 r xv
        writeArray arrP1 p yv

        xv2 <- readArray arrP2 xv
        yv2 <- readArray arrP2 yv

        writeArray arrP2 xv yv2
        writeArray arrP2 yv xv2
      [2] -> do
        modifyIORef' isP1 not
  readIORef isP1 >>= \flag ->
    if flag
      then getElems arrP1 >>= putStrLn . unwords . map show
      else getElems arrP2 >>= putStrLn . unwords . map show
