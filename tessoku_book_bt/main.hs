...
main = do
  [h, w, k] <- ints
  cs <- listArray @UArray ((0, 0), (h - 1, w - 1)) . concat <$> replicateM h getLine
  -- どのように埋めた結果が最大かを求めればよい
  -- 二分探索じゃなさそうだけど、探索系な予感。制約がゆるゆるだから全探索するか
  -- 制約が緩いyの選択肢を2^hから全探索して、その中で白いものが多い列を選ぶ

  -- 行選択>列選択(行で選ばれたマスがないものとして数えればよい)>集計(col,rowで選択した座標を洗って白いマスを集計)。という流れが汲めればよい
  -- colsに入れた値のbit=1の行を選んだことにする
  let res =
        maximum
          [ calcScore selectedRows sortedCol
            | rowBits <- [0 .. 2 ^ h - 1],
              let selectedRows = selRows rowBits,
              -- K個しか選べないのを忘れてはいけない
              length selectedRows <= k,
              let sortedCol = L.sortBy (comparing (\(_, v) -> Down v)) [(col, countWhiteCol selectedRows col) | col <- [0 .. w - 1]]
          ]
      selRows :: Int -> [Int]
      -- i番目のbitが今選んでいる組み合わせだったら採用
      selRows rowBits = [i | i <- [0 .. h - 1], testBit rowBits i]
      countWhiteCol :: [Int] -> Int -> Int
      countWhiteCol rows col = sum [1 | r <- [0 .. h - 1], r `notElem` rows, cs ! (r, col) == '.']
      -- rows cols score
      calcScore :: [Int] -> [(Int, Int)] -> Int
      calcScore selectedRows sortedCols =
        -- k個選ぶのを忘れない
        let colsByK = take (k - length selectedRows) $ map fst sortedCols
            remainingWhite =
              sum
                [1 | r <- [0 .. h - 1], r `notElem` selectedRows, c <- [0 .. w - 1], c `notElem` colsByK, cs ! (r, c) == '.']
         in h * w - remainingWhite
  print res
