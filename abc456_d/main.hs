...
data Chr = Null | A | B | C deriving (Eq, Ix, Ord, Bounded, Show)

main :: IO ()
main = do
  s <- getLine
  let cToc c = case c of
        'a' -> A
        'b' -> B
        'c' -> C
        _ -> Null
      -- 今回は前の遷移の値を維持する必要があるので、accumDPで解く
      -- Nullが1を持ち続けることで、全ての文字1文字を選ぶ場合を計上できる
      dp = accumDP @Array next relax ini bnds v0s s
      ini = 0 :: IntMod
      bnds = (Null, C)
      relax = (+)
      v0s = [(Null, 1)]
