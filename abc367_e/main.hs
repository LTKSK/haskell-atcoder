...
printArr xs = putStrLn $ unwords $ map show xs

main :: IO ()
main = do
  [n, k] <- ints
  xs <- listArray @UArray (1, n) <$> ints
  as <- listArray @UArray (1, n) <$> ints
  let d = doubling @UArray (1, n) k (xs !)
      -- aの値が遷移しているわけじゃなくて、x上でindexが遷移している
      res = [as ! doublingQuery d k i | i <- [1 .. n]]
  printArr $ if k == 0 then elems as else res
