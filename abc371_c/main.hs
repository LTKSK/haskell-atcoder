...
main = do
  [n] <- ints
  [mg] <- ints
  uvs <- replicateM mg $ do
    [u, v] <- ints
    return (u, v)
  [mh] <- ints
  abs <- replicateM mh $ do
    [a, b] <- ints
    return (a, b)
  as <- replicateM (n - 1) ints
  let -- 三角行列の入力を扱う。余分な範囲には0が入る
      costArr = array @UArray ((1, 2), (n - 1, n)) [((i, j), cost) | (i, row) <- zip [1 ..] as, (j, cost) <- zip [i + 1 ..] row]
      -- edgeの繋がる頂点のi<jは保証されている
      edgesG = S.fromList uvs
      edgesH = S.fromList abs
      -- 頂点番号のマッピングを管理する順列。arrayのindexとvalueが頂点番号の対応表になっている
      p = map (listArray @UArray (1, n)) $ L.permutations [1 .. n]
      res =
        [ sum
            [ costArr ! (i, j)
              | i <- [1 .. n - 1],
                j <- [i + 1 .. n],
                let [pi, pj] = L.sort [pa ! i, pa ! j],
                let isInG = S.member (pi, pj) edgesG,
                let isInH = S.member (i, j) edgesH,
                isInH /= isInG
            ]
          | pa <- p
        ]

  print $ if null res then 0 else minimum res
