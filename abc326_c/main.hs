...
main = do
  [n, m] <- ints
  as <- L.sort <$> ints
  -- i~i+m-1までの範囲にあるindexを集計する
  -- 尺取りじゃい
  -- let go [] _ _ res = res
  --     go (l : ls) rs cnt res =
  --       -- rsをlsの先頭から見て、条件を満たすものとそうでないものに分ける
  --       let (inRange, outRange) = span (< l + m) rs
  --           -- 条件を満たすものを計上
  --           cnt' = cnt + length inRange
  --           -- ここまでの最大値resと比較し大きい方を採用
  --           res' = max res cnt'
  --        in -- lを除いた残りと、今回条件に入れなかった残りの配列を入れて次の回
  --           --  cnt'-1は、次のgoでlsが一つ減るため
  --           go ls outRange (cnt' - 1) res'
  -- print $ go as as 0 0
      ...
      res =
        maximum
          [ r - i + 1
            | i <- [1 .. n],
              let left = as' ! i,
              let r = binSearchMax (\j -> as' ! j < left + m) i (n + 1)
          ]
  print res
