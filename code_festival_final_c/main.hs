...
main = do
  [a] <- integers
  -- n進数でn。16進数で16だったらf(16)=22だよね、みたいな感じか
  -- 桁数をkとして、最上位の桁の値がどれぐらいになるか考える
  -- aは10^16まで
  -- 10は10*10で100。100(10^2)は1*100^2で10^4
  -- 1000は10^3*10^3で10^6。1000は10^8。なのでaは10^8ぐらいまでがkの上限
  -- kを入力してaに一致するかを調査
  let f x =
        let s = show x
            digitAndNums = zip [0 :: Integer ..] (map (fromIntegral . digitToInt) $ reverse s)
         in sum [(x ^ d) * n | (d, n) <- digitAndNums]
      -- fは単調増加なので二分探索する
      k = binSearchMin (\x -> f x >= a) 9 (10 ^ 16)
  print if f k == a then k else -1
