...
main = do
  ...
  hs <- integers
  -- 3ターンに一度だけ3減る
  -- Tの攻撃力は1か3だけ。hをsumしてもいいかと思ったけどそんな単純じゃないか
  -- 一旦h=7の単純な場合で考えると、1,1,3,1,1で5回
  -- h=9、1,1,3,1,1,3で6回
  -- h=12、1,1,3,1,1,3,1,1で8回
  -- hを何か計算して、その余りのような結果を用いて次のhを計算するといったやつな気がする
  -- hからhを3で割った回数引くとtが上昇する回数
  let solve t [] = t
      solve t (h : hs) =
        let cycle = h `div` 5 -- 1週
            t' = cycle * 3 -- cycle*3がtの増分その1
            remaining = h `mod` 5 -- hを5ダメずつ削った時のあまり。0~4
            offset = (t + t') `mod` 3 -- tがあと何ターン後に3ダメ出すか。3-offset。0~2
            calcExtra (remaining, offset) = case (remaining, offset) of
              (0, _) -> 0
              (1, 0) -> 1
              (1, 1) -> 1
              (1, 2) -> 1
