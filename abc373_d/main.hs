...
main = do
  [n, m] <- ints
  uvws <- replicateM m ints
  let g = buildWeightedGraph (1, n) uvws
  -- 到達管理
  visited <- newArray @IOUArray (1, n) False
  res <- newArray @IOUArray (1, n) (0 :: Int)
  -- 未訪問頂点毎に走査。別にqueueに詰めなくていいからdfsで
  let dfs' i v = do
        vis <- readArray visited i
        unless vis $ do
  ...
  forM_ [1 .. n] $ \i -> do
    dfs' i 0
  -- 頂点を受け取って、隣接頂点すべてを探索する関数を定義
  res' :: UArray Int Int <- freeze res
  putStrLn $ unwords $ map show $ elems res'
