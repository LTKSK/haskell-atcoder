...
main = do
  [n] <- ints
  as <- ints
  let gs = L.groupBy (\a b -> fst a == fst b) $ reverse $ L.sortBy (comparing fst) $ zip as [1 :: Int ..]
      isInvalid = all (\v -> length v > 1) gs
      (_, res) = head $ head $ dropWhile (\g -> length g > 1) gs

  print $ if isInvalid then -1 else res
