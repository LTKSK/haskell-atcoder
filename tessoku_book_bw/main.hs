...
main = do
  [n] <- ints
  tds <-
    listArray @Array (1, n) . L.sortBy (comparing snd)
      <$> replicateM
        n
        ( do
            [t, d] <- ints
  ...
  -- 締め切り順に並べた時に、途中の問題を解くか解かないかを考えるナップザック問題になる
  -- i番目の問題に取り掛かった際に時刻がjの時に解けた最大の問題数
  let dp = listArray ((0, 0), (n, 1440)) [step i j | i <- [0 .. n], j <- [0 .. 1440]] :: Array (Int, Int) Int
      step 0 _ = 0 -- 初期値
      step i j =
        let (t, d) = tds ! i
         in -- jが締め切りより遅い==解けない
            -- jがtより大きくても解けない（遷移する前の状態がないので）
            if j > d || j < t then dp ! (i - 1, j) else max (dp ! (i - 1, j)) (dp ! (i - 1, j - t) + 1)
  print $ maximum $ [dp ! (n, i) | i <- [0 .. 1440]]
