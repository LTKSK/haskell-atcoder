...
bfs' :: IOUArray (Int, Int, Int, Int) Int -> UArray (Int, Int) Char -> Seq.Seq (Int, Int, Int, Int) -> IO Int
bfs' dist grid queue = case queue of
  Seq.Empty -> return (-1)
  (y1, x1, y2, x2) Seq.:<| rest -> do
    let bnds = bounds grid
        move (r, c) (dr, dc) =
          let r' = r + dr
              c' = c + dc
           in if inRange bnds (r', c') && grid ! (r', c') /= '#'
                then (r', c')
                else (r, c)
    curDist <- readArray dist (y1, x1, y2, x2)
    res <-
      foldM
        ( \(seq, found) (dy, dx) -> case found of
            Just d -> return (seq, Just d)
            Nothing -> do
              let (y1', x1') = move (y1, x1) (dy, dx)
                  (y2', x2') = move (y2, x2) (dy, dx)
              nextDist <- readArray dist (y1', x1', y2', x2')
              if nextDist == -1
                then do
                  writeArray dist (y1', x1', y2', x2') (curDist + 1)
                  if y1' == y2' && x1' == x2'
                    then return (seq, Just (curDist + 1))
                    else return (seq Seq.|> (y1', x1', y2', x2'), Nothing)
                else
                  return (seq, Nothing)
        )
        (rest, Nothing)
    ...
    case res of
      (_, Just d) -> return d
      (newQueue, _) -> bfs' dist grid newQueue

main :: IO ()
main = do
  [n] <- ints
  ss <- replicateM n getLine
  let g = listArray @UArray ((1, 1), (n, n)) $ concat ss
      players = [ix | ix <- range $ bounds g, g ! ix == 'P']
      [(r1, c1), (r2, c2)] = players
  visited <- newArray @IOUArray ((1, 1, 1, 1), (n, n, n, n)) (-1)
  -- 初期値設定
  writeArray visited (r1, c1, r2, c2) 0
  res <- bfs' visited g (Seq.fromList [(r1, c1, r2, c2)])
  print res
