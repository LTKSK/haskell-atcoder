...
main = do
  [n, d] <- ints
  xys <- replicateM n intwo
  -- let (sx, sy) = head xys
  --     -- 次に探索する対象
  --     -- startから感染した要素。この各要素が次のstartになる
  --     -- 探索した要素はsetに埋めておく
  --     solve start = go (Seq.singleton start) (S.singleton start)
  --       where
  --         go Seq.Empty s = s
  --         go ((x', y') Seq.:<| rest) s =
  --           let includes =
  ...
  --                     d ^ 2 >= ((x - x') ^ 2 + (y - y') ^ 2)
  --                 ]
  --            in go (rest Seq.>< Seq.fromList includes) (S.insert (x', y') s)
  --     res = solve (sx, sy)
  -- mapM_ (printYn . (`S.member` res)) xys
