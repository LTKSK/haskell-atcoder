...
main = do
  [n, k] <- ints
  las <- replicateM n ints
  cs <- ints
  -- 実際に連結する必要はなくて、再帰でkの値を減らしつつ進めばOKなはず
  let solve ((c, l : as) : rest) v
        | c * l <= v = solve rest (v - c * l)
        | otherwise = as !! (v `mod` l)
  print $ solve (zip cs las) (k - 1)

-- res <- forM (zip cs las) $ \(c, l : as) -> do
--   cur <- readIORef k'
--   if c * l < (cur + 1)
--     then do
--       writeIORef k' (cur - c * l)
--       return Nothing
--     else do
--       let idx = if l > cur then cur else cur `mod` l
--       return $ Just $ as !! idx

-- print $ head $ catMaybes res
