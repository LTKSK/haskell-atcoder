...
main = do
  ...
  let pmin = minimum xs
      pmax = maximum xs
      ls = L.sort $ map snd $ filter (\(c, _) -> c == '0') $ zip s xs
      rs = map snd $ filter (\(c, _) -> c == '1') $ zip s xs
      -- r毎にlsの個数を数える
      -- 単調増加なら何でも条件にぶっこんでいいんじゃないか？
      -- ri,t分後のriの位置をri'とする。liも同様
      -- すれ違う条件は
      -- ri < liかつri' >= li'。この条件をbinSearchに投げ込んで、条件を満たす最大のindexと、条件を満たさない最大のindexを取りたい
      -- ri'とli'をバラすと
      -- ri + t >= li - t
      -- ri >= li - 2t
      len = length ls
      ls' = listArray @UArray (1, len) ls
      res =
        [ maxi - mini + 1
          | ri <- rs,
            -- 複合条件は単調な条件にならない
            let maxi = binSearchMax (\i -> let li = ls' ! i in ri >= li - t * 2) 0 (len + 1),
            let mini = binSearchMin (\i -> let li = ls' ! i in ri < li) 0 (len + 1)
        ]
  print $ sum res
