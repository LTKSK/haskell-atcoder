...
import Data.Maybe
import Data.Ord
import Data.Primitive (freezeArray)
import Data.STRef
...
main = do
  ...
  as' <- newListArray @IOUArray ((1, 1), (h, w)) (concat as)
  bs <- listArray @UArray ((1, 1), (h, w)) . concat <$> replicateM h ints
  -- h,wは2以上なのでエッジケースはいったん無視
  -- 一致することが重要なので差分の配列dに絞って考える
  -- 算出の前に、Yesとなる条件を考えてみよう
  -- 最終的に残る0でない正方形4マスの値が同じでなければならない
  -- ということは、あるマスに着目した時、x+1またはy+1の値とx,yが一致していなければならない
  -- 横か縦にそろっているのが条件
  -- とかもろもろ考えたけど、各マスに対して今見ている座標の差分だけ更新していって、結果のarrayが一致しているかを見ればよかった
  res <-
    foldM
      ( \acc i -> do
          av <- readArray as' i
          let diff = bs ! i - av
          forM_ [i + d | d <- [(0, 0), (1, 0), (0, 1), (1, 1)]] $ \i' -> do
            pv <- readArray as' i'
            writeArray as' i' (pv + diff)
          return (acc + abs diff)
      )
      0
      (range ((1, 1), (h - 1, w - 1)))
  as'' <- freeze as' :: IO (UArray (Int, Int) Int)
  let y = as'' == bs
  printYn y
  when y $ do
    print res
