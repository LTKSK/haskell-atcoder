...
printArray2D :: (Ix i, Ix j, IArray arr Char) => arr (i, j) Char -> IO ()
printArray2D arr = do
  let ((r0, c0), (r1, c1)) = bounds arr
      rows = range (r0, r1)
      cols = range (c0, c1)
  forM_ rows $ \i -> do
    let row = [arr ! (i, j) | j <- cols]
    putStrLn row

main :: IO ()
main = do
  [n] <- ints
  as <- listArray @UArray ((1, 1), (n, n)) . concat <$> replicateM n chars
  let rotate (y, x) = (x, n + 1 - y)
      layer y x = minimum [y, x, n + 1 - y, n + 1 - x]
      res =
        listArray @UArray
          ((1, 1), (n, n))
          [ as ! from
            | (y, x) <- range ((1, 1), (n, n)),
              let from = iterate rotate (y, x) !! (4 - (layer y x) `mod` 4)
          ]
  printArray2D res
...
--     [ ((y, x), to)
--       | (y, x) <- range ((1, 1), (n, n)),
--         let layer = minimum [x, y, n + 1 - y, n + 1 - x]
--             to = iterate rotate (y, x) !! (layer `mod` 4)
--     ]

-- x,yが対象。どの様に置き換えられるか考えること
-- マスは10^6ぐらいなので、素直にSTArrayかIOArrayでこうね
-- 同時に行うという注釈があるので、途中書き変わった状態を参照してはならない
-- 前の状態を引き回さないといけない。または影響が出ないように書き換えていく
-- 1辺の長さはnだから、外周から徐々に内側に入っていく。これがまず範囲の遷移
-- xがyにyが真ん中から見て左右を反転した値になる。x,yが入れ替わるので、90度回転したような結果になるはず
-- 道中を考慮しないから、一気に計算してポンと移動するのが良さそうな予感はする
-- 座標もかぶらない
-- 1.移動をMapで計算
-- xとyはそれぞれ依存がある
