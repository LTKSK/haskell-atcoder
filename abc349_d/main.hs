...
main = do
  [l, r] <- ints
  let s = [l .. r - 1]
  -- rまでの数列を求めて、lまでの数列を引くパターンも頭の隅に入れておこう
  -- 2^i*j,2^i*(j+1)。数列と言いつつ必要な要素は2つだけ
  -- lが馬鹿でかい。?≒10^19ぐらい
  -- 「長さが2のべき乗で、左端がその長さで割り切れる区間」という視点はなかった
  -- l=2^i * j、2のべき乗の倍数
  let solve l r
        | l >= r = []
        | otherwise =
            -- lから見て、取れる区間の右端を求める。いくつ取れるかをlenとして、lenL,lenRのminで求める
            -- lenLはlから始められる良い数列で最大の長さ。長さは2^iの形
            -- lを割り切る2^iをlowestBitで求める（最小は1である）
            -- l=3（0011）なら1だし、l=10（0101）なら2。良い数列は2のべき乗でしか表現できないので、lが10なら2^1=2までの長さしか取れない
            let lenL = if l == 0 then r else lowestBit l -- l=0は特別扱い。2^i*0がいくつでも成立するので、r側を採用する
            -- lから見た距離がrを超えてはいけないので、rからlを引いた値で長さを求める
            -- r-lの長さの中で最大の2のべき乗で表現される長さを返す。lから2取るのか4取るのか8取るのかという感じ
                lenR = largestPow2AtMost (r - l)
                len = min lenL lenR
             in (l, l + len) : solve (l + len) r
      res = solve l r
