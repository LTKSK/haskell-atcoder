...
main = do
  ...
  t <- getLine
  -- 文字の置き換えはできない
  -- 双方からAを除外した文字列が一致しないものについては-1
  -- sの1文字と一致する位置を2分探索で求める可能性も考えておこう
  -- print $ filter (\(i, c) -> c == 'A') (zip [1 ..] s)
  -- print $ filter (\(i, c) -> c == 'A') (zip [1 ..] t)
  let solve [] trem acc = acc + length trem
      solve srem [] acc = acc + length srem
      solve s'@(sc : srem) t'@(tc : trem) acc
        | tc == sc = solve srem trem acc
        | tc == 'A' = solve s' trem (acc + 1)
        | sc == 'A' = solve srem t' (acc + 1)
        | otherwise = error ("nanndeya!")
      res = solve s t 0
  print $ if (filter (/= 'A') s) == (filter (/= 'A') t) then res else -1
