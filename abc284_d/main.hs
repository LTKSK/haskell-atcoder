...
main = do
  [t] <- ints
  -- p^2なので、pは3√n。qは√nまで。√nでも10^9なのでqは探索出来なさそう
  -- pを探索しよう。しかしどうやったもんか
  replicateM_ t $ do
    [n] <- ints
    let solve n = (p, q)
          where
            -- 候補を探索。素数かどうかの判定は実は重要ではない
            -- なぜならnを割り切る時点で√pもしくはqである
            -- またすべての候補を試す必要もない
            d =
              head
                [ x
                  | x <- takeWhile (\x -> x * x * x <= n) [2 ..],
                    n `mod` x == 0
                ]
            (p, q)
              -- dがpの場合
              | n `mod` (d * d) == 0 = (d, n `div` (d * d))
              -- qの場合
              | otherwise =
                  let p' = floorSqrt (n `div` d)
                   in (p', d)

    --     ps = takeWhile (\x -> x ^ 3 <= n) [2 .. n]
    --     res =
    --       [ (p, q')
    --         | p <- ps,
    --           isPrime p,
