...
main = do
  ...
  as <- ints
  -- 状態を再帰で引き回す
  -- (最後にchoutter開いた時間, 合計値), aoki
  let res = solve (0, 0) as
      solve (lastc, acc) [] = acc + (t - lastc)
      solve (lastc, acc) as'@(a : as)
        | lastc >= t = acc -- 終了時刻を過ぎたらもう開けない
        -- 最後に開いてからaoki襲来
        | lastc <= a = solve (min t (a + 100), acc + (a - lastc)) as
        -- 開いてないっすよ
        | lastc > a = solve (lastc, acc) as
  print if n == 0 then t else res
