...
main = do
  ...
  as <- L.sort <$> ints
  bs <- ints
  -- b_i一つをyとおくと、a_i - iはa_i>yに対するa_i-yの和(a_i-yの時)+y-a_iの和(y-a_iの時)
  -- a_i>=yとなる最小のi（なけりゃn+1）をi_yとおくと、a_i>=yを満たすaはn+1-iy。iの範囲で引き算した時は+1しようね（引いた値の地点も含むので）
  -- 仮定を一つ置いてb_iからみた
  let acum = listArray @UArray (0, n) $ L.scanl' (+) 0 as
