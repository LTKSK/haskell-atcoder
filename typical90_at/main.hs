...
main = do
  [n] <- ints
  -- それぞれの値を0~45まででまとめる。modで複数の値を同じ値として扱える
  as <- map (`mod` 46) <$> ints
  bs <- map (`mod` 46) <$> ints
  cs <- map (`mod` 46) <$> ints
  let as' = M.fromListWith (+) [(a, 1) | a <- as]
  let bs' = M.fromListWith (+) [(b, 1) | b <- bs]
  let cs' = M.fromListWith (+) [(c, 1) | c <- cs]
  -- let res =
  --       sum
  --         [ a * b * c
  --           | x <- [0 .. 45],
  --             let a = fromMaybe 0 $ M.lookup x as',
  --             y <- [0 .. 45],
  --             let b = fromMaybe 0 $ M.lookup y bs',
  --             z <- [0 .. 45],
  --             let c = fromMaybe 0 $ M.lookup z cs',
  --             (x + y + z) `mod` 46 == 0
  --         ]
  let res =
        sum
