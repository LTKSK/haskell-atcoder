...
--   -- 頂点とそこへのstartからの距離
--   UArray (Int, Int) Int
-- bfs graph cToPos start h w = runST $ do
--   dist <- newArray (bounds graph) (-1) :: ST s (STUArray s (Int, Int) Int)
--   -- スタート地点のコストは0
--   writeArray dist start 0
...
--   freeze dist
--   where
--     getVs :: UArray (Int, Int) Char -> (Int, Int) -> [(Int, Int)]
--     getVs g (i, j)
--       | g ! (i, j) == '.' =
--           [ (i', j') | (i', j') <- [(i, j - 1), (i - 1, j), (i, j + 1), (i + 1, j)], i' <= h && i' >= 1, j' <= w && j' >= 1
--           ]
--       | g ! (i, j) == '#' = []
--       | otherwise =
--           let c = g ! (i, j)
...
--                 ]
--               -- ワープ先（自分自身を除く）
--               warps = filter (\(i', j') -> i /= i' || j /= j') $ fromMaybe [] $ M.lookup c cToPos
--            in neighbors ++ warps
...
--       queue' <- foldM (step d) queue (getVs graph u)
--       -- 更新したqueueを元に再帰
--       go queue' dist
--       where
--         step d q v = do
...
main = do
  ...
  -- 文字からなる座標の一覧を受けるarrayを作ろう
  -- chartopos =
  --   m.fromlistwith
  --     (flip (++))
  --     [ (c, [(i, j)]) | i <- [1 .. h], j <- [1 .. w], let c = g ! (i, j), c /= '.' && c /= '#'
  --     ]
  -- res = bfs g chartopos (1, 1) h w
  -- print $ res ! (h, w)
