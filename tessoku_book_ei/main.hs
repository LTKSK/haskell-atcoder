...
import Control.Monad.RWS (MonadState (put), modify)
...
findSimplePath ::
  ...
  Int ->
  Maybe [Int]
findSimplePath graph start goal = runST $ do
  visited <- newArray (bounds graph) False :: ST s (STUArray s Int Bool)
  path <- newSTRef []
  found <- dfs visited path start
  if found
    then Just . reverse <$> readSTRef path
    else return Nothing
  where
    dfs visited path u = do
      -- 先頭にpathを追加。後でひっくり返す。引数を受ける関数を第二引数に渡すので、ここではpathにuを追加する処理を書いている
      modifySTRef' path (u :)
      writeArray visited u True
      if u == goal
        then return True
        else do
          result <- anyM (graph ! u) $ \v -> do
            vis <- readArray visited v
            if vis
              then return False
              else dfs visited path v
          -- 到達しなかった際にpathをバックトラックする
          unless result $ do
            writeArray visited u False
            -- tailで先頭要素を削除
...
main = do
  ...
  let g = buildGraph (1, n) abs
      res = findSimplePath g 1 n

  putStrLn $ unwords $ maybe [] (map show) res
