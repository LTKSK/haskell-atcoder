...
strInt :: IO (String, Int)
strInt = do
  line <- BS.getLine
  let ws = BS.words line
  case ws of
    [s, n] -> case BS.readInt n of
      Just (num, _) -> return (BS.unpack s, num)
      Nothing -> error "数字のパースに失敗"
    _ -> error "フォーマットが違う"
...
main = do
  [n, a, b] <- ints
  sds <- replicateM n strInt
  let res = L.foldl' step ini sds
      ini = 0
      step acc ("West", d) =
        let diff = max a $ min b d
         in acc - diff
      step acc ("East", d) =
        let diff = max a $ min b d
         in acc + diff
