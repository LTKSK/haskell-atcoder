...
main = do
  ...
  s <- getLine
  t <- getLine
  -- 違うパターン、長さが違う時と、そうじゃない時で考えましょう
  let slen = length s
      tlen = length t
      -- sに長い方を入れる
      res
        | slen > tlen = solve1 s t 0
        | tlen > slen = solve1 t s 0
        | otherwise = solve2 s t 0
      solve1 _ _ 2 = 2 -- diff二つ以上はng
      solve1 [] [] acc = acc
      solve1 [_] [] acc = acc + 1
      solve1 [] rest acc = acc + length rest
      solve1 (sc : rs) (tc : rt) acc
        | sc == tc = solve1 rs rt acc
        | otherwise = solve1 rs (tc : rt) (acc + 1)

      solve2 [] [] acc = acc
      solve2 (sc : rs) (tc : rt) acc
        | sc == tc = solve2 rs rt acc
        | otherwise = solve2 rs rt (acc + 1)
  printYn $ abs (slen - tlen) <= 1 && res <= 1
