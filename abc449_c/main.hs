...
main = do
  [n, lo, hi] <- ints
  -- i<=jでかつ、l<=j-i<=r。後者の条件はつまり、iからjまでの距離の最小と最大。l=2,r=5だったら差は2~5までが許容される
  -- 一旦l,rの条件がない時を考える
  -- これは単純で、('a','z')の26個のindexに加算していけば良い
  -- 補集合の考え方で、rまでの長さの累積を求めたうえで、l-1までの累積を引けばいい
  -- 次に、sにおいて長さrまでにあるaの数を考えると、その区間にあるaの個数が分かればよい
  -- これはrをスライドさせていけば求められるはず？>よく考えたら別に部分文字列ではない。
  -- aのindexを全て収集。差を取って、l以上rであるものを集計する

  s <- getLine
  -- let arr = accumArray @Array (flip (:)) [] ('a', 'z') xs
  --       where
  --         -- concatMapは配列の各要素に関数を適用した後に結合する。ここではu,vを前後入れ替えたtupleにしている
  --         -- accumArrayはtupleのfstをindexとして、値にtupleのsndを使う。同じindexに複数の値がある時、accumArrayの第一引数で結合する
  --         xs :: [(Char, Int)] = zip s [1 ..]
  ...
  --           | els <- elems arr,
  --             let len = length els,
  --             let as = listArray @UArray (1, len) $ reverse els,
  --             let cumR = shakutori' as len hi,
  --             let cumL = shakutori' as len (lo - 1)
  --         ]
  let sv = listArray @UArray (1, n) s
  cnt <- newArray @IOUArray (0, 25) 0 :: IO (IOUArray Int Int)
  let add i = when (1 <= i && i <= n) $ do
        let c = ord (sv ! i) - ord 'a'
        modifyArray cnt c (+ 1)
      sub i = when (1 <= i && i <= n) $ do
        let c = ord (sv ! i) - ord 'a'
        modifyArray cnt c (subtract 1)

  res <- newIORef (0 :: Int)
  forM_ [1 .. n] $ \j -> do
    -- ウィンドウはj-hi..j-lo。hi,loが逆なのに注意
    --  L<=j-i<=R
    --  L-j <= -i <= R
    --  j-L >= i >= j-R
    add (j - lo) -- 入る
    sub (j - hi - 1) -- でてーく
    let c = ord (sv ! j) - ord 'a'
    v <- readArray cnt c
    modifyIORef' res (+ v)
  readIORef res >>= print
