...
main = do
  [l, r] <- integers
  let f :: Integer -> IntMod
      -- 1+2+...+n
      f n
        | n <= 0 = 0
        | otherwise = fromIntegral n * fromIntegral (n + 1) / 2
      -- aからbまでの和。累積計算で求める
      rangeSum :: Integer -> Integer -> IntMod
      rangeSum a b
        | a > b = 0
        | otherwise = f b - f (a - 1)
      solve :: Integer -> Integer -> IntMod
      solve l r =
        sum
          -- 算出した個数に桁数を掛けるとその桁の個数が求まる
          [ fromIntegral i * rangeSum a b
            | i <- [1 .. 19],
              -- 各桁の最小値と最大値がloとhi。例えば2桁なら10~99
              let lo = 10 ^ (i - 1),
              let hi = 10 ^ i - 1,
              -- lとrの共通する値を見つける。例えば15,150で2桁の時はmax 10 15 = 15, min 150 99 = 99で15~99
              -- 15,150で3桁の時はmax 100 15 = 100, min 150 999 = 150で100~150
              let a = max lo l,
              let b = min hi r,
              a <= b
          ]
