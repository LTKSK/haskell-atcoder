...
main = do
  [n, d] <- integers
  sts <- replicateM (fromIntegral n) $ do
    [s, t] <- integers
    return (s, t)
  -- 犯行開始時刻tから、犯行終了時刻までいれるかってことを考える
  let sts' = L.filter (\(s, t) -> (t - s) >= d) sts
      arr = accumArray @Array (+) 0 (1, 10 ^ 6) $ concat [[(s, 1), (t - d + 1, -1)] | (s, t) <- sts']
      res = L.foldl' step ini (elems arr)
      ini = (0, 0) -- acc, count
      step (acc, c) v =
        let c' = c + v
         in (acc + c' * (c' - 1) `div` 2, c')
  print $ fst res
