...
bfsGrid :: IOUArray (Int, Int) Int -> UArray (Int, Int) Char -> Seq.Seq (Int, Int) -> IO Int
bfsGrid dist grid queue = do
  go queue 1
  where
    go :: Seq.Seq (Int, Int) -> Int -> IO Int
    go Seq.Empty cnt = return cnt
    go ((y, x) Seq.:<| rest) cnt = do
      let bnds = bounds grid
      curdist <- readArray dist (y, x)
      -- 周囲にマグネットがあったらそこで止まる
      let isNeighborMagnet = any (\p -> grid ! p == '#') $ [p | d <- lrud, let p = (y, x) + d, inRange bnds p]
      if isNeighborMagnet
        then do
          go rest cnt
        else do
                    ...
                    then do
                      ...
                      if nextdist == -1 && grid ! (y', x') /= '#'
                        then do
                          ...
                          -- 次のマスを探索するときに、cntをインクリメントする
                          -- print (y', x', c, ">", c + 1)
                          return (seq Seq.|> (y', x'), c + 1)
                        else
                          return (seq, c)
                    else return (seq, c)
              )
...
main = do
  ...
  let bnds = ((1, 1), (h, w))
      grid = listArray @UArray bnds $ concat ss
      hasMag (y, x) =
        any
          ( \(dy, dx) ->
              let p = (y + dy, x + dx)
               in inRange bnds p && grid ! p == '#'
          )
          lrud

  -- 自由に動けるマスかどうかを判定。#でなくて、隣接マスにも#が無ければTrue
  freeArr <- newArray @IOUArray bnds False
  forM_ (range bnds) $ \p ->
    writeArray freeArr p (grid ! p == '.' && not (hasMag p))

  visited <- newArray @IOUArray bnds False
  lastSeen <- newArray @IOUArray bnds (0 :: Int)

  resRef <- newIORef 1
  let bfs' round start = do
        writeArray visited start True
        let go Seq.Empty fc bc = return (fc, bc)
            go ((y, x) Seq.:<| rest) fc bc = do
              -- 4方向探索
              (q', fc', bc') <- foldM (step round (y, x)) (rest, fc, bc) lrud
              go q' fc' bc'
            step rnd (y, x) (q, fc, bc) (dy, dx) = do
              let p = (y + dy, x + dx)
              if not (inRange bnds p) || grid ! p == '#'
                then return (q, fc, bc)
                else do
                  isFree <- readArray freeArr p
                  if isFree
                    then do
                      ...
                      if vis
                        then return (q, fc, bc)
                          ...
                          -- freeで未到達なら追加
                          return (q Seq.|> p, fc + 1, bc)
                    else do
                      -- blockedなセル
                      r <- readArray lastSeen p
                      if r == rnd
                        then return (q, fc, bc)
                        else do
                          -- 現在のラウンド番号で更新
                          writeArray lastSeen p rnd
                          return (q, fc, bc + 1)
        go (Seq.singleton start) 1 0

  resRef <- newIORef 1
  forM_ (zip [1 ..] (range bnds)) $ \(rnd, p) -> do
    isFree <- readArray freeArr p
    vis <- readArray visited p
    when (isFree && not vis) $ do
      (fc, bc) <- bfs' rnd p
      modifyIORef' resRef (max (fc + bc))

  -- forM_ (range ((1, 1), (h, w))) $ \(y, x) -> do
  --   -- 探索順がよくないな
  --   -- 先に1マス目で踏んだけど移動できないものがある
  --   -- じゃあ回りの白いマスの数でsortしちゃうか。対して増えんが
  --   vis <- readArray visited (y, x)
  --   when (vis == -1 && grid ! (y, x) == '.') $ do
  --     writeArray visited (y, x) 0
  --     v <- bfsGrid visited grid (Seq.singleton (y, x))
  --     modifyIORef' resRef (`max` v)
  readIORef resRef >>= print

-- WA1：自由なマスから探索を初めて、magnetマスを踏んだら止まるようにしたけどWA
-- magnetの隣のマスに複数の経路で到達しうる場合があって、visitedにしてしまうと計上出来ない
-- 自由に動けるマスの範囲+その範囲に隣接したmagnetに隣接しているcellの個数が答えになる