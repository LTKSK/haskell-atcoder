...
main = do
  scores <- ints
  let toScore = listArray @UArray ('A', 'E') scores
      ss = L.sortBy (comparing (Down . snd)) [(s, sum $ map (toScore !) s) | s <- L.subsequences ['A', 'B', 'C', 'D', 'E'], not $ null s]
      g = L.groupBy (\a b -> snd a == snd b) ss
      res = concatMap (L.sortBy (comparing fst)) g
  mapM_ (putStrLn . fst) $ res
