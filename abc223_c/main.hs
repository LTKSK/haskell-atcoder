...
main = do
  [n] <- ints
  abs :: [(Double, Double)] <- replicateM n $ do
    [a, b] <- ints
    return (fromIntegral a, fromIntegral b)
  -- どうやったら一致する場所がわかるのか。
  -- 10^5だから左右から愚直シミュレーションかな。i番目が燃え尽きるまでの速度が整数じゃない...
  -- ai/biで燃やすのにかかる時間が求められる
  let t = (sum $ map (uncurry (/)) abs) / 2.0
      go :: Double -> Double -> [(Double, Double)] -> Double
      go pos _ [] = pos
      go pos rest ((a, b) : xs)
        -- restは残り時間。残り時間が今の導火線を燃やしきれるならaの分進めて、残り時間を減らして終わり
        | rest >= a / b = go (pos + a) (rest - a / b) xs
        -- 残りの時間で進める距離
        | otherwise = pos + rest * b

  -- timeLeft :: Double -> [(Double, Double)] -> Double
  -- timeLeft x xs = go x 0 xs
  --   where
  --     -- rが残りの距離
  --     go _ acc [] = acc
  --     go r acc ((a, b) : rest)
  --       | r >= a = go (r - a) (acc + a / b) rest
  --       | otherwise = acc + r / b
  print $ go 0 t abs

-- showFFloat (Just 9) res ""