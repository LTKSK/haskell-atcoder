...
main = do
  [n] <- ints
  cps <- replicateM n ints
  [q] <- ints
  lrn <- replicateM q ints
  let c1s = listArray @UArray (0, n) $ L.scanl' (+) 0 (map (\[c, p] -> if c == 1 then p else 0) cps)
  let c2s = listArray @UArray (0, n) $ L.scanl' (+) 0 (map (\[c, p] -> if c == 2 then p else 0) cps)
  forM_ lrn $ \[l, r] -> do
    let res1 = c1s ! r - (c1s ! (l - 1))
    let res2 = c2s ! r - (c2s ! (l - 1))
    putStrLn $ unwords [show res1, show res2]
