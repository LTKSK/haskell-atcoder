...
main = do
  [t] <- ints
  [l, x, y] <- map fromIntegral <$> ints
  [q] <- ints
  replicateM_ q $ do
    [e] <- ints
    -- t/4で45度動く。初期値は(0,0,0)。大像はx,y...
    -- つっても時刻の範囲はmod4だから、角度の出し方さえわかれば
    -- 周期があるのでeをtでmodを取る
    let e' = e `mod` t
        theta = 2 * pi * fromIntegral e' / fromIntegral t
        -- l/2が半径
        r = l / 2
        -- sin 0 = 0なので初期値はそのまま
        -- sin 90の時に下(x,y平面だから下ではないんだが...)にいるので回り方が逆
        y' = -(r * sin theta)
        -- cos 0 = 1。だけど今回のz座標はcos 0の時点で0。ということは逆
        z' = r - (r * cos theta)

        -- 三平方の定理のために、xのdiffを取るが、観覧車のx軸は常に0
        x' = 0
        dx = x - x'
        dy = y - y'
        -- 自分
