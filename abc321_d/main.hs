...
main = do
  ...
  as <- L.sort <$> integers
  bs <- L.sort <$> integers
  -- sortして、足してpを超える地点についてはlognで出せるな
  -- pを超えないところは素直に累積和でいいのか。その添え字までの累積+a*添え字
  let bs' = listArray @Array (1, m) bs
      bsum = listArray @Array (0, m) $ L.scanl' (+) 0 bs
      res =
        sum
          [ a * idx + bsum ! idx + p * (m - idx) | a <- as, let idx = binSearchMax (\i -> (bs' ! i + a) < p) 0 (m + 1)
          ]
  print res
