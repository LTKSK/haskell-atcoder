...
import Data.List (sort, isSuffixOf, isPrefixOf, unfoldr, tails)
...
main = do
    [n] <- ints
    as <- ints
    let triples = [()| (x:ys) <- tails as, (y:zs) <- tails ys, z <- zs, (x+y+z) == 1000]

    putStrLn $ if null triples then "No" else "Yes"
