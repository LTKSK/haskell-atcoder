...
powMod' a b
  | b == 0 = 1
  | even b = powMod' (a * a) (b `div` 2)
  | otherwise = a * powMod' a (b - 1)
...
main = do
  ...
  let l = length $ show n
      -- 公比
      r = powMod' (IntMod 10) l
      -- 等比数列の和の公式の分子
      num = powMod' r n - 1
      -- 分母
      den = r - 1
      -- (r^n-1) / (r-1)はmodの世界だと出来ない
      -- (r-1)^(p-2)が(r-1)の逆元になるのでこれを掛ける。pはここではmoduls
      geom = num * powMod' den (modulus - 2)
      res = IntMod (n `mod` modulus) * geom
  print res
