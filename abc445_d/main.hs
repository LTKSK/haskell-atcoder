...
main = do
  ...
  hws <- forM [1 .. n] $ \i -> do
    [h, w] <- ints
    return (h, w, i)
  let hs = map (\(hi, _, i) -> (hi, i)) hws
      ws = map (\(_, wi, i) -> (wi, i)) hws
      -- h降順、w降順でインデックスをソート
      ordH = map snd $ L.sortBy (comparing (Down . fst)) hs
      ordW = map snd $ L.sortBy (comparing (Down . fst)) ws

      -- i番目のhとwを引くためのArray
      hArr = listArray @UArray (1, n) (map (\(hi, _, _) -> hi) hws)
      wArr = listArray @UArray (1, n) (map (\(_, wi, _) -> wi) hws)

      -- used をスキップしながら配置していく
      solve _ _ _ _ _ 0 = []
      solve curH curW oh ow used rem =
        -- 先頭からみて、降順配列の頭から除外
        let oh' = dropWhile (`S.member` used) oh
            ow' = dropWhile (`S.member` used) ow
            -- 先頭要素を使う
            ih = head oh'
            iw = head ow'
            -- indexから引いたhの値が一致するかどうか
            (i, isH) = if hArr ! ih == curH then (ih, True) else (iw, False)
            -- 左上の座標を計算するので、hで埋めたら
            x = curH - hArr ! i + 1
            y = curW - wArr ! i + 1
            used' = S.insert i used
            (curH', curW') =
              -- 縦が一致すると横が縮まる
              if isH
                then (curH, curW - wArr ! i)
                else (curH - hArr ! i, curW)
         in (i, x, y) : solve curH' curW' oh' ow' used' (rem - 1)

      result = solve h w ordH ordW S.empty n

  forM_ (L.sortBy (comparing (\(i, _, _) -> i)) result) $ \(_, x, y) ->
