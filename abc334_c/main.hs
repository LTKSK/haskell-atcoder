...
main = do
  [n, k] <- ints
  as <- ints
  let diffs = sum $ map (\[a, b] -> b - a) $ chunksOf 2 as -- 隣接差のリスト
  -- 偶数番目のペアの和（0-indexed で 0,2,4,... 番目の diff）
      prefixSum = L.scanl' (+) 0 $ map (\[a, b] -> b - a) $ chunksOf 2 (init as)
      sufixSum = L.scanr (+) 0 $ map (\[a, b] -> b - a) $ chunksOf 2 (tail as)
      res = zipWith (+) prefixSum sufixSum
  print $ if even k then diffs else if null res then 0 else minimum res
