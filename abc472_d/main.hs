...
bfsGrid :: IOUArray (Int, Int) Int -> UArray (Int, Int) Char -> Seq.Seq (Int, Int) -> Int -> IO ()
bfsGrid dist grid queue k = case queue of
  Seq.Empty -> return ()
  (y, x) Seq.:<| rest -> do
    ...
    newQueue <-
      foldM
        ( \seq (dy, dx) -> do
            ...
            if inRange bnds (y', x')
              then do
                ...
                if (curDist < k && nextDist == -1 && grid ! (y', x') /= '#')
                  then do
        ...
        rest
        lrud
    bfsGrid dist grid newQueue k

main :: IO ()
main = do
  [h, w, k] <- ints
  ss <- listArray @UArray ((1, 1), (h, w)) . concat <$> replicateM h getLine
  -- 安全な空マスの定義、その行と列が全部空マス
  -- 安全な空マスかどうかは、一度走査すれば求めることが可能ではある
  -- まず安全な空マスは漏れなくすべて含まれる
  -- 安全な空マスを全て求めて、その後、全ての空マスから多始点bfsでk回以内の移動を行う
  -- 計算量を見積もる、まずO(HW log HW)で空きマスの位置が求まる。その後、HWで多始点bfsできるから見積もり的にはハマりそう
  let startsW =
        concat
          [ map (y,) xs | y <- [1 .. h], let xs = [1 .. w], and $ map (\x -> ss ! (y, x) == '.') xs
          ]
      startsH =
        concat
          [ map (,x) ys | x <- [1 .. w], let ys = [1 .. h], and $ map (\y -> ss ! (y, x) == '.') ys
          ]
      starts = S.toList $ S.intersection (S.fromList startsW) (S.fromList startsH)

  visited <- newArray @IOUArray ((1, 1), (h, w)) (-1 :: Int)
  forM_ starts $ \idx -> do
    writeArray visited idx 0
  bfsGrid visited ss (Seq.fromList starts) k

  res <- getElems visited
  print $ length $ filter (\v -> v <= k && v >= 0) res
