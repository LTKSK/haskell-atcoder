...
main = do
  ...
  -- 正整数。1以上で0始まりもだめ
  -- 積の最大値
  -- 10桁以下なので、最大でも9桁と1桁の数字になる
  -- 95432 * 9843
  -- let solve :: [Int] -> (Int, Int)
  --     solve (l : r : xs) = go l r xs
  --       where
  --         go l r [] = (l, r)
  --         go l r (x : xs)
  --           | l > r = go l (r * 10 + x) xs
  --           | otherwise = go (l * 10 + x) r xs
  --     res = solve (map digitToInt $ L.sortBy (comparing Down) n)
  -- print $ uncurry (*) res

  -- 全探索版
  let n' = L.sortBy (comparing Down) n
      res =
        maximum
          [ x * y
            | p <- replicateM (length n) [True, False],
              let (xs, ys) = L.foldl' (\(xs, ys) (v, b) -> if b then (digitToInt v : xs, ys) else (xs, digitToInt v : ys)) ([], []) $ zip n' p,
              let xs' = reverse xs,
              let ys' = reverse ys,
              not $ null xs',
              not $ null ys',
              head xs' /= 0,
              head ys' /= 0,
              let x = L.foldl' (\acc v -> acc * 10 + v) (head xs') $ tail xs',
              let y = L.foldl' (\acc v -> acc * 10 + v) (head ys') $ tail ys'
          ]
  print res
