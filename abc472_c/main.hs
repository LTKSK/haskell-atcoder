...
main = do
  [n, m, k] <- ints
  as <- ints
  -- m日間の合計のカロリーが分かればよいので、累積和で求める
  -- 途中食べなかったというコンテキストを引き継ぐのか確認
  let (xs, _) = L.foldl' step ([], 0) $ take m as
      step (acc, v) a =
        let v' = v + a
         in if v' > k then (0 : acc, v) else (a : acc, v')
      -- xsにはai..amまでのものを食べられたかどうか
      -- xsの合計は、以降の要素を食べる時の基準
      -- この方法だとxsが足りなくなってしまう。seqを使わないとダメ
      (_, _, res) = L.foldl' step2 ini $ drop m as
      ini = (sum xs, Seq.fromList (reverse xs), [] :: [Int])
      step2 (prevSum, p Seq.:<| queue, acc) cur =
        let isOver = k < (prevSum - p + cur)
         in if isOver
              then (prevSum - p, queue Seq.|> 0, 0 : acc)
              else (prevSum - p + cur, queue Seq.|> cur, 1 : acc)
  mapM_ (printYn . (> 0)) $ reverse xs ++ reverse res
