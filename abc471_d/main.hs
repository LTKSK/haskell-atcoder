...
main = do
  ...
  let res = L.foldl' step ini qs
      -- 集合と現在時刻と答え
      ini = (M.empty :: M.Map Int Int, [] :: [Int])
      step (m, acc) query = case query of
        [1, t, w] -> (M.insertWith (+) (w - t) 1 m, acc)
        [2, t] -> case M.lookupMax m of
          Nothing -> (m, (-1) : acc)
          Just (x, c) ->
            let m' = if c == 1 then M.delete x m else M.insert x (c - 1) m
             in (m', min v (x + t) : acc)
  mapM_ print $ reverse $ snd res
