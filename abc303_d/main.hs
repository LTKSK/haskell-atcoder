...
main = do
  [x, y, z] <- ints
  s <- getLine
  -- Si文字まで読んだ時に、その文字を入力する手段の最短の時間
  -- 緩和対象は時刻。状態はcapsを押したときと押していない時で遷移すればOK
  let dp = accumArrayDP @UArray next relax ini bnds v0s s
      ...
      next (ps, t) c
        ...
        | otherwise = case (ps, c) of
            (False, 'a') -> [(False, t + x), (True, t + z + y), (True, t + x + z)]
            (True, 'a') -> [(False, t + y + z), (False, t + x + z), (True, t + y)]
            (False, 'A') -> [(False, t + y), (True, t + z + y), (True, t + x + z)]
            (True, 'A') -> [(False, t + y + z), (False, t + x + z), (True, t + x)]
  print $ min (dp ! True) (dp ! False)
