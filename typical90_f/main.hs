...
buildNext :: Int -> String -> Array (Int, Char) Int
buildNext n s = array ((1, 'a'), (n, 'z')) assocs
  where
    initial = listArray @UArray ('a', 'z') (repeat (n + 1)) -- 見つからない時にn+1
    step (i, c) state = state // [(c, i)]
    states = L.scanr step initial (zip [1 ..] s)
