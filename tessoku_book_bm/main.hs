...
subtreeSize :: (Num a) => (a -> a -> a) -> a -> Array Int [Int] -> Array Int a
subtreeSize combine initial graph = runSTArray $ do
  dp <- newArray (bounds graph) initial
  let dfs u parent = do
        forM_ (graph ! u) $ \v -> do
          when (v /= parent) $ do
            -- 帰り掛けなので先に子の端まで再帰
            dfs v u
            childSize <- readArray dp v
            currentSize <- readArray dp u
            -- 子供と自身のサイズを更新
            writeArray dp u (combine currentSize childSize)
...
main = do
  ...
  -- aのiが2から始まろうともaccumArrayでは関係ない
  let g = buildGraph (1, n) $ zipWith (\k v -> [k, v]) [2 ..] as
      res = subtreeSize (+) 1 g

  putStrLn $ unwords $ map (show . subtract 1 . (res !)) [1 .. n]
