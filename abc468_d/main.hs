...
solve :: Int -> UArray Int Char -> Int -> Int
solve len arr i = go i i 0 0 + go i (i + 1) 0 0
  where
    -- goは左方向の探索index、同右、ここまでの差の数、回文文字数合計
    go l r _ acc
      | l < 1 || r > len = acc
    go l r d acc
      | lc /= rc && d == 1 = acc
      | lc /= rc = go (l - 1) (r + 1) (d + 1) (acc + 1)
      | otherwise = go (l - 1) (r + 1) d (acc + 1)
      where
        lc = arr ! l
        rc = arr ! r
...
main = do
  s <- getLine
  let len = length s
      arr = listArray @UArray (1, len) s
      -- iを受けて、左右端方向によんでいく
      res = sum $ map (solve len arr) [1 .. len]
  print res
