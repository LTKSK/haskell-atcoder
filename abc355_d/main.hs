...
main = do
  [n] <- ints
  lrs <- L.sortBy (comparing fst) <$> replicateM n intwo
  -- 尺取りやいもすを考えたけど上手くいかなかった
  -- 補集合を考えて、全部重なってるとした場合の数から、重ならない個数を計上する
  let total = n * (n - 1) `div` 2
      lrs' = listArray @Array (1, n) $ lrs
      -- (l1,r1),(l2,r2)が交わらない条件
      -- r1 < l2であるもの
      -- 各lr_iに対して、lがrより大きいものの数を計上する
      -- にぶたんできるな
      res =
        sum
          [ n - j + 1 | (i, (l, r)) <- zip [1 ..] lrs, let j = binSearchMin (\k -> fst (lrs' ! k) > r) 1 (n + 1)
          ]
  print $ total - res
