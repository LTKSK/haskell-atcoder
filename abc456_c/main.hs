...
main = do
  s <- getLine
  -- dpで前の文字から違う文字が来るたびカウントをしていく形になるのかな
  let g = map length $ L.group s
      -- 部分文字列だから、先頭からn文字削るか、末尾からn文字削ったもの。なので選び方は飛ばない
      -- 全体の選び方はこれ
      total = let l = length s in l * (l + 1) `div` 2
      s'' = L.foldl' step [] s
      step [] c = [c : ""]
      step acc c = if h' == c then (c : "") : acc else (c : h) : tail acc
        where
          h = head acc
          h' = head $ head acc
      calc v = let l = length v in l * (l + 1) `div` 2
      ans = sum $ map calc s''
  print $ ans `mod` modulus
