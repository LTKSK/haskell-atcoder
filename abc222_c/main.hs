...
main = do
  [n, m] <- ints
  as <- listArray @UArray ((1, 1), (n * 2, m)) . concat <$> replicateM (n * 2) getLine
  let janken a b = case (a, b) of
        ('G', 'C') -> 1
        ('C', 'P') -> 1
        ('P', 'G') -> 1
        _ -> 0
      res = L.foldl' step ini [1 .. m]
      ini = M.fromList [(i, 0 :: Int) | i <- [1 .. n * 2]]
      step acc i =
        let ps = map fst $ L.sortOn (\(p, wins) -> (Down wins, p)) $ M.assocs acc
            vs = chunksOf 2 ps
            wins :: [(Int, Int)] =
              concat
                [ [(p1, janken p1h p2h), (p2, janken p2h p1h)]
                  | [p1, p2] <- vs,
                    let p1h = as ! (p1, i)
                        p2h = as ! (p2, i)
                ]
            -- !_ = dbg (acc, ps, vs, wins)
            m' = L.foldl' (\m (k, v) -> M.insertWith (+) k v m) acc wins
         in m'
  mapM_ print $ map fst $ L.sortOn (\(p, wins) -> (Down wins, p)) $ M.assocs res
