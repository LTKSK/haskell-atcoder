...
main = do
  [n, t] <- ints
  as <- ints
  -- n*h-1+wで、整数が置かれている。要は左上から順に1basedな値が置かれているわけね
  -- aがビンゴの出目。縦横斜めにビンゴするかを考える。達成しなかったら-1
  -- aの座標の値は被らないから、mapでw座標の個数とh座標の個数を管理したらなんとかならんか？
  -- wの個数またはhの個数がnに到達したら縦横のビンゴ。斜めはy==xか、y+xがn+1
  let toIdx a =
        let a' = a - 1
            x = a' `mod` n + 1
            y = a' `div` n + 1
      ...
      -- n*n+2個のカウンタが必要だ
      solve _ _ _ _ _ [] = -1
      solve h w d1 d2 c (a : as')
        | h' M.! y == n = c'
        | w' M.! x == n = c'
        | d1' == n = c'
        | d2' == n = c'
        | otherwise = solve h' w' d1' d2' c' as'
        where
          !(y, x) = toIdx a
          h' = M.insertWith (+) y 1 h
          w' = M.insertWith (+) x 1 w
          d1' = d1 + if y == x then 1 else 0
          d2' = d2 + if y + x == n + 1 then 1 else 0
          c' = c + 1

      res = solve M.empty M.empty 0 0 0 as
  print $ res
