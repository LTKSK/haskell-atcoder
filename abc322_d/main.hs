...
main :: IO ()
main = do
  p1 :: UArray (Int, Int) Char <- listArray ((1, 1), (4, 4)) . concat <$> replicateM 4 getLine
  p2 :: UArray (Int, Int) Char <- listArray ((1, 1), (4, 4)) . concat <$> replicateM 4 getLine
  p3 :: UArray (Int, Int) Char <- listArray ((1, 1), (4, 4)) . concat <$> replicateM 4 getLine
  let ps1 = makeRotations $ S.fromList [(y, x) | ((y, x), c) <- assocs p1, c == '#']
      ps2 = makeRotations $ S.fromList [(y, x) | ((y, x), c) <- assocs p2, c == '#']
      ps3 = makeRotations $ S.fromList [(y, x) | ((y, x), c) <- assocs p3, c == '#']
      -- 座標を左上に寄せる
      normalize s = S.map (\(y, x) -> (y - minY, x - minX)) s
        where
          minY = minimum $ S.map fst s
          minX = minimum $ S.map snd s
      -- y,x -> x,-yで90度回転したあとnormalize
      rotateSet s = normalize $ S.map (\(y, x) -> (x, -y)) s
      -- 初期値の座標を受け取る想定なので、そいつらも左上に寄せておく
      -- 4つで360度分
      makeRotations s = take 4 $ iterate rotateSet s
      makePositions s =
        [ shifted
          | dy <- [0 .. 3],
            dx <- [0 .. 3],
            let shifted = S.map (\(y, x) -> (y + dy, x + dx)) s,
            all (\(y, x) -> 1 <= y && y < 5 && 1 <= x && x < 5) shifted
        ]
      ...
      res =
        not $
          null
            [ ()
              | s1 <- concatMap makePositions ps1,
                s2 <- concatMap makePositions ps2,
                S.disjoint s1 s2,
                s3 <- concatMap makePositions ps3,
                S.disjoint s1 s3,
                S.disjoint s3 s2,
                S.size (S.union s1 $ S.union s2 s3) == 16
            ]
  printYn res
