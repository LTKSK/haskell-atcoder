...
main = do
  ...
  let as'' = listArray @UArray (1, n) (reverse as)
  let solve n as = go 0 IM.empty 1
        where
          cnt x m = IM.findWithDefault 0 x m
          go !acc !m !i
            | i == n + 1 = acc
            | otherwise =
                let ai = as ! i
                    m' = IM.insertWith (+) ai 1 m
                 in if ai `rem` 5 == 0
                      then
                        -- ai / 5の数を3倍7倍して、3:5:7を満たす値が何回出てきていたかを取得
                        let t = ai `div` 5
                            -- 組み合わせの数を求めるので掛け算
                            add = cnt (7 * t) m * cnt (3 * t) m
                         in go (acc + add) m' (i + 1)
                      -- 5の倍数じゃないなら次
                      else go acc m' (i + 1)
      -- maxの結果が求まれば逆向きに見ればminの結果も求まる
      res = (solve n as') + (solve n as'')
  print res
