...
main = do
  [n] <- integers
  -- bitが立っている位置を取得。後は桁を上げていくだけなんだけど...
  let digits = [i | i <- [0 .. 59], testBit n i]
      -- 各桁を1,0どっちにするか、全通りのパターンをreplicateMで作る
      -- 後は桁情報と合わせて生成
      choises = replicateM (length digits) [1 :: Int, 0 :: Int]
      res =
        [ v
          | c <- choises,
            let v = L.foldl' (\acc (d, c) -> if c == 1 then setBit acc d else clearBit acc d) n $ zip digits c
        ]
  mapM_ print $ L.sort res
