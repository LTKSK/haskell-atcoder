...
main = do
  [n] <- ints
  ps <- listArray @UArray ((1, 1), (n, n)) . concat <$> replicateM n ints
  -- 横を入れ替えているときは縦の位置は変わらない。逆も同様。列と行の操作は独立
  -- 縦と横、それぞれの数値の位置が昇順になるまでの最小の回数を求めるんじゃないか
  -- psからx方向にみた時に存在する数値の列
  let xs = [v | x <- [1 .. n], y <- [1 .. n], let v = ps ! (y, x), v /= 0]
      -- psからy方向にみた時に存在する数値の列
      ys = [v | y <- [1 .. n], x <- [1 .. n], let v = ps ! (y, x), v /= 0]
      -- 転倒数のカウント実装
      inversionCount xs =
        length [() | (i, xi) <- zip [0 ..] xs, (j, xj) <- zip [0 ..] xs, i < j, xi > xj]
      xc = inversionCount xs
      yc = inversionCount ys
