...
buildWeightedGraph (i, n) uvcs = accumArray (flip (:)) [] (i, n) xs
  where
    xs = concatMap (\[u, v, c] -> [(u, (v, c))]) uvcs

main :: IO ()
main = do
  [n, m, l, s, t] <- ints
  uvcs <- replicateM m ints
  let g = buildWeightedGraph (1, n) uvcs
      go :: Int -> Int -> Int -> S.Set Int -> S.Set Int
      go v pastEdges cost res
        | pastEdges > l = res
        | pastEdges == l && s <= cost && cost <= t = S.insert v res
        | otherwise = L.foldl' step res (g ! v)
        where
          step s (u, c) = go u (pastEdges + 1) (cost + c) s
  putStrLn $ unwords $ map show $ S.elems $ go 1 0 0 S.empty
