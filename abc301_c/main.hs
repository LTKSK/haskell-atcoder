...
main = do
  s <- L.sort <$> getLine
  t <- L.sort <$> getLine
  let sc = length $ takeWhile (== '@') s
      s' = dropWhile (== '@') s
      tc = length $ takeWhile (== '@') t
      t' = dropWhile (== '@') t
      atcoder = S.fromList "atcoder"
      arrS = accumArray @UArray (+) (0 :: Int) ('a', 'z') $ map (,1) s'
      arrT = accumArray @UArray (+) (0 :: Int) ('a', 'z') $ map (,1) t'
      (resS, resT) = L.foldl' step (0, 0) "atcoder"
      resRem = all (\c -> arrS ! c == arrT ! c) $ filter (\c -> not $ c `S.member` atcoder) ['a' .. 'z']
      step (accS, accT) c = (accS + max 0 (tv - sv), accT + max 0 (sv - tv))
        where
          sv = arrS ! c
          tv = arrT ! c
  printYn $ resS <= sc && resT <= tc && resRem
