...
main = do
  [l, r, d, u] <- ints
  -- k層目の正方形と、lrduで指定された範囲の点の数
  let f k =
        let wx = max 0 (min r k - max l (-k) + 1) -- +1は区間で、右辺自体の数を含む必要がある
            wy = max 0 (min u k - max d (-k) + 1)
         in wx * wy
      -- 入力の矩形の範囲は10^6を超えないので、
      res = sum [if even k then f k else -(f k) | k <- [0 .. 10 ^ 6]]
  print res
