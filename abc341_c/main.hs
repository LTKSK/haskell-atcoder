...
main = do
  [h, w, n] <- ints
  t <- getLine
  s <- listArray @UArray ((1, 1), (h, w)) . concat <$> replicateM h getLine
  -- Tのとおりに辿って、結果をSetで集計してSizeを出力
  -- 500*500で25000通りだから全探索から試す
  let res =
        [ -- ここでfoldでチェックした結果通ったらJust Set、通らなかったらNothingでcatMaybesでくっつけてからunionしてsizeを取ろう
          foldM step (ix, [ix]) t
          | -- 縁は海なのでそれ以外を返す
            ix <- range ((2, 2), (h - 1, w - 1)),
            s ! ix /= '#'
        ]
      step (cur, acc) c
        | s ! next == '.' = Just (next, next : acc)
        | otherwise = Nothing
        where
          -- !_ = dbg (cur, acc)
          (y, x) = cur
          next = case c of
            ...
            'U' -> (y - 1, x)
            'D' -> (y + 1, x)
  print $ S.size . S.fromList $ map (head . snd) $ catMaybes res
