...
main = do
  [n, m] <- ints
  adbs <- replicateM n ints
  -- 色の種類を計上する問題
  -- setにしてsizeを計上?いや、重複すると数が消えちゃう。いや、そんなことないか?
  -- いや、なにかの色に何羽いるかわからないといけない。となるとmapかarrayだね
  -- d日目でaを-1。bを+1すればOK。IOArrayでやろう。prevが0で移動して来たら+1。逆に移動元が0なら-1
  -- と思ったけど、これd日目をどう表現するんだ
  -- いや、jが1..mだから、それで走査できる状態になっていればOK
  -- 初期状態がaの種類になる
  -- j=dのものに対して処理を行う。この時、aの色を-1して、bの色を+1して、そのあと種類数を計上する
  -- ということは、まずdの日付があるかどうかをlookupできるようになっていないといけない。これはarrayだと分岐がだるいからMapでやろう
  let mp = M.fromListWith (++) [(d, [(a, b)]) | [a, d, b] <- adbs]
      s = S.fromList [a | [a, d, b] <- adbs]
  -- 0日目の種類数を計上するけど、d=1が考慮されていない？
  -- いや、d=1であれば最初のループで計上されるからそんなことにはならないはず
  cntRef <- newIORef $ S.size s
  -- ここの初期化で0日目を作っているつもり
  arr <- newArray @IOUArray (1, n) (0 :: Int)
  forM_ adbs $ \[a, _, _] -> do
    modifyArray' arr a (+ 1)

  forM_ [1 .. m] $ \j -> do
    let cur = M.lookup j mp
    case cur of
      Nothing -> do
        c <- readIORef cntRef
        print c
      -- 以下は見直したけどあまり変なところがなさそう?
      Just changes -> do
        forM_ changes $ \(a, b) -> do
          when (a /= b) $ do
            -- arrで現在のその色iの種類数を管理している
            -- fromとtoが同じ時を考えよう
            from <- readArray arr a
            to <- readArray arr b
            -- aはdのタイミングでなくなるので-1する
            when (from == 1) $ do
              -- 0になると種類が減るので-1
              modifyIORef' cntRef (\v -> v - 1)
            -- bに変更するとき、toが0だったらば種類が1増えるので+1
            when (to == 0) $ do
              modifyIORef' cntRef (+ 1)
            -- arrのaをdecrementして、bをincrement
            modifyArray' arr a (\v -> v - 1)
            modifyArray' arr b (+ 1)
        -- 更新の結果のカウント数をreadして表示
        c <- readIORef cntRef
        print c