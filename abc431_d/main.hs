...
main = do
  ...
  -- dp[i][w] = (h, b) 単純化すると右辺はvでナップザックなんだよな
  -- 嬉しさと重さの評価はべつである
  -- wb >= whを守らないといけなくて、これをどう漸化式に落とせばいいか考えよう
  -- 他の人の回答を見ると、wがhだったら引いて、bだったら足して、を行って0>=になる最大の値を取ればいいそう。賢い。これをするとh,bそれぞれの遷移を考慮しなくてよいな

  let maxW = sum [w | [w, _, _] <- whbs]
      -- 空間は-重さの最大値～重さの最大値。// [(0,0)]で初期値を設定している
      dp0 = listArray (-maxW, maxW) (repeat minBound) // [(0, 0)] :: UArray Int Int
      -- 畳み込みじゃい
      res = foldl' step dp0 whbs
        where
          step dp [w, h, b] =
            accumArray max minBound (bounds dp) updates
            where
              updates =
                [ (w', acc')
                  | -- 一つ前の状態。indexがwで、嬉しさがaccに入ってる
                    (w0, acc0) <- assocs dp,
                    acc0 /= minBound, -- 初期値ママならまだ更新されていないので除外
                    (w', acc') <-
                      [ -- 頭えらーぶ
                        (w0 - w, acc0 + h),
                        -- 体えらーぶ
  ...
  print $ maximum [acc | (w, acc) <- assocs res, w >= 0]
