...
main = do
  [n] <- ints
  as <- ints
  let as' = listArray @UArray (1, n) (L.sort as)
      cum = listArray @UArray (0, n) $ L.scanl' (+) 0 (elems as')

  -- 累積和で、あるaのindexまでの累積値をcum[n]から引いたら答えになるはず
  putStrLn $ unwords $ map show [cum ! n - cum ! i | a <- as, let i = binSearchMax (\i -> as' ! i <= a) 0 (n + 1)]
