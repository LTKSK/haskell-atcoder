...
main :: IO ()
main = do
  -- 頭が0,0から0,iまで移動したと考えたところを初期値にして、
  -- 頭の移動履歴をi番目まで管理し、都度先頭からp番目の履歴を参照すると答えになるはず
  -- 問題はhaskellのデータ構造でどうやって求めるか...>seqでやってみよう
  [n, q] <- ints
  let init = (Seq.fromList [(0, i) | i <- [1 .. n]], (0, 1))

  queries <- replicateM q intStr
  let step (acc, pos) (query, val) =
        case query of
          1 -> do
            let c = BS.head val
                (y, x) = pos
            ...
            return (acc', pos')
          2 -> do
            let Just (p, _) = BS.readInt val
                (y, x) = Seq.index acc (p - 1)
            putStrLn $ unwords $ map show [x, y]
            return (acc, pos)
  foldM_ step init queries
