...
main :: IO ()
main = do
  -- この手の問題は、x,yの範囲を考えるところから
  [d] <- ints
  -- √d以上にはならなそう
  -- xとyの取りうる値を考える
  -- xは0~√dまで。d-x^2をrとして、|y^2-r|を最小化する
  -- yはだいたい√rになるはず。で、小数部分を切り上げた値と切り捨てた値の2つを計算して結果を求める
  let res =
        minimum
          [ abs (d - x * x - y' * y')
            | x <- [0 .. floorSqrt d],
              let r = d - x * x,
              r >= 0, -- x*xがdを超えているときはyの調整の余地がない。が、yで調整できる方が数字が細かいのでより良い結果があるはず
              let y = floorSqrt r,
              y' <- [y, y + 1]
          ]
  print res
