...
main = do
  ...
  let s = S.fromList as
      len = S.size s
      arr = listArray @UArray (1, len) $ S.elems s
      kindToV = M.fromList $ zip [0 ..] (reverse $ elems arr)
      vToC = M.fromListWith (+) [(a, 1) | a <- as]
  -- 取得できたらその数値がarrの中で何個出てくるかを調べないといけない
  -- メモリ持つかな
  forM_ [0 .. n - 1] $ \k -> do
    let v = M.lookup k kindToV
    let res = case M.lookup k kindToV of
          Nothing -> 0
          Just v -> fromMaybe 0 $ M.lookup v vToC
    print res
