...
main = do
  [n] <- ints
  -- 操作が終わるまでだから、なんか素因数分解的なやつ？
  -- 素直に再帰を書いてみましょう
  let solve 1 memo = (0, memo)
      solve n memo = case M.lookup n memo of
        Just v -> (v, memo)
        Nothing ->
          let n1 = n `div` 2
              n2 = (n + 1) `div` 2
              (v1, memo1) = solve n1 memo
              (v2, memo2) = solve n2 memo1
              v = n + v1 + v2
              memo3 = M.insert n v memo2
           in (v, memo3)

  print $ fst $ solve n M.empty
