...
main = do
  s <- getLine
  -- 変更してカッコ列になった個数を計上する
  -- Snに含まれるカッコ列の数は、Siまでのカッコ列の数
  -- Siを現在の文字含む3通りに遷移しつつ、カッコ列だったら集計？
  let dp = accumArrayDP @Array next relax ini bnds v0s s
      bnds = (0, length s)
      relax :: IntMod -> IntMod -> IntMod
      relax = (+)
      ini = 0
      v0s = [(0, 1)] -- 深さ、カッコ列になる組み合わせ(空文字列が1)
      -- nextは状態の遷移を考えるだけ。合算はrelaxがやる
      next (depth, acc) c
        | depth < 0 = [] -- 閉じカッコの方が多い列は以降成立しえない
        -- ?が出てきたときに分岐があるので、そこで置き換えの数が増える
        | c == '?' = [(depth + 1, acc), (depth - 1, acc)]
        | c == ')' = [(depth - 1, acc)]
        | c == '(' = [(depth + 1, acc)]
        | otherwise = error "nandeya!"
