...
main = do
  [n] <- ints
  hs <- ints
  let g :: [[(Int, Int)]] = L.groupBy (\a b -> snd a == snd b) $ L.sortBy (comparing snd) $ zip [1 ..] hs
      bestForGroup ixs =
        let s = IS.fromList ixs
            count i d =
              if IS.member i s
                then 1 + count (i + d) d
                else 0
            cs =
              [ count i d
                | (idx, i) <- zip [0 ..] ixs,
                  j <- drop (idx + 1) ixs, -- j > i
                  let d = j - i
              ]
         in if null cs then 1 else maximum cs
      res =
        [ bestForGroup ixs
          | xs <- g,
            let ixs = map fst xs
