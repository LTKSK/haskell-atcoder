...
main = do
  [n] <- ints
  as <- ints
  -- i番目が倒れるとき、j=i+a_iとしてi<=x<jの範囲のドミノが倒れる
  -- 最初のドミノは1番目なので、ここから何枚倒れるかを考える
  -- 問題の設定上、途中で止まった座標をkとすると、kより左はすべて倒れていて、kより右は全部無事
  -- なのでkが求まればよい。kが倒れるときの条件から、倒れないkの条件を考えたい
  -- kが倒れるとき、k-(i+a_i)>1。逆にk-(i+a_i)<=1なら条件を満たさないが、k>iなので、aiが1の時にしか起こりえない
  -- aiが1でかつ、それがドミノの範囲の右端にある時、そのaiのi=k
  -- aj=1が見つかった時、そこまでのai+i-1の最大値が、j以下だったらそこまでやな
  -- 以下は提出
  -- let as' = listArray @UArray (1, n) as
  --     asum = listArray @UArray (1, n) $ L.scanl1 max [i + v | (i, v) <- zip [1 ..] as]
  --     is = [i | (i, v) <- zip [1 ..] as, (v == 1) && (asum ! i == i + 1)]
  --     ans = if null is then n else minimum is
