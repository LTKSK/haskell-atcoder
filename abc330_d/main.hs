...
main = do
  [n] <- ints
  ss <- listArray @UArray ((1, 1), (n, n)) . concat <$> replicateM n getLine
  let row = listArray @UArray (1, n) $ [length $ filter (== 'o') $ map (\y -> ss ! (y, x)) [1 .. n] | x <- [1 .. n]]
      col = listArray @UArray (1, n) $ [length $ filter (== 'o') $ map (\x -> ss ! (y, x)) [1 .. n] | y <- [1 .. n]]
      res =
        sum
          [ cc * rc
            | (y, x) <- range $ bounds ss,
