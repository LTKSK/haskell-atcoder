...
main = do
  ...
  replicateM_ t $ do
    ...
    let base = sum (map fst abs)
        minA = minimum (map fst abs)
        diffSorted = L.sort (map (\(a, b) -> b - a) abs)
        -- 通常購入した金額の合計をbaseとし、クーポンを使った個数をkとする
        -- b-aが大きいということは、クーポンを使う効果が大きいということ
        -- 使った枚数分baseから引いていくことで、クーポンを使った枚数ごとの金額を求められる
        curs = listArray @UArray (0, n) $ L.scanl (+) base diffSorted
        res =
          minimum
            -- クーポンを使って購入する
            -- クーポンが手に入る個数は、クーポンを使わずに買う個数n-k
            -- 必要なクーポンの枚数はk枚。k*2<=n(k <= n/2)の時。すなわちクーポンが不足しない場合はそのままの購入でOK
            -- クーポンが不足する場合は、minAを追加購入しないといけない
            -- この個数はk*2-n個。
            [(curs ! k) + max 0 (k * 2 - n) * minA | k <- [0 .. n]]
    print res
