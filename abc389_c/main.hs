...
main = do
  ...
  -- 全体の長さと抜けた蛇の長さ、蛇の数を保持しときゃ都度計算しなくて良さそう
  -- なんか難しく考えすぎじゃない？
  let (_, _, _, res) = L.foldl' step ini qs
      -- ここまでの全ての蛇の長さの合計、抜けた蛇の長さの合計、i番目の蛇の情報、答え
      ini = (0, 0, Seq.Empty, [])
      -- 1の時、totalを追加。totalは増え続ける。queueには長さだけ詰めておく
      -- 2の時、先頭を削除。削除した分の長さを計上
      step (total, rl, seq, acc :: [Int]) query = case query of
        [1, l] -> let total' :: Int = total + l in (total', rl, seq Seq.|> (total, l), acc)
        [2] -> let ((_, removed) Seq.:<| rest) = seq in (total, rl + removed, rest, acc)
        [3, k] ->
          let (t', l') = Seq.index seq (k - 1)
              ans = t' - rl
           in (total, rl, seq, ans : acc)
