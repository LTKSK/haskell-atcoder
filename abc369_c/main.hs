...
main = do
  [n] <- ints
  as_ <- ints
  let diff = zipWith (-) (tail as_) as_
      g = map length $ L.group diff
      res = n + sum [k * (k + 1) `div` 2 | k <- g]
  print res

-- let as = listArray @UArray (1, n) as_
-- let go l r d acc
--       | r > n = acc
--       | r - l <= 0 = go l (r + 1) 0 (acc + 1) -- 長さ1、公差未定
--       -- 長さ2になる瞬間、公差が決まる
--       | r - l == 1 =
--           let d' = as ! r - as ! (r - 1)
--            in go l (r + 1) d' (acc + r - l + 1)
--       -- 公差が一致、rを伸ばす
--       | as ! r - as ! (r - 1) == d =
--           go l (r + 1) d (acc + r - l + 1)
--       -- 公差が不一致、lを進める
--       | otherwise =
--           go (r - 1) r 0 acc
--     res = go 1 1 0 0
