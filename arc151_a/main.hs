...
main = do
  ...
  s <- getLine
  t <- getLine
  -- s,tが一致する個所はUを何にしても値が変わらない
  let -- 差がある箇所が奇数個だったら成立しないはず
      -- もうちょっと考える。si,tiが異なる時、入れ替えても答えは同じか？
      -- いや、結果は異なる。ということは単純な偶奇だけでは求められない
      -- 0,1のうち、0を選ぶインセンティブが大きい
      -- なので1になっているiだけを対象に考える。
      -- 半分がどうこうはあまりまとを得ていなさそう...
      -- 1111
      -- 0000
      -- 0011 これが最小になるはず。これはs,t入れ替わっていても関係ないな
      -- 同じところには0を入れる
      -- 違うところには何を入れる？s,tそれぞれと違う値を入れなければならない。sと違うものがd/2個。tも同じく
      -- s,tそれぞれのカウントを持って、違う位置で0を持っている側を1にしていくか
      count = length . filter id $ zipWith (/=) s t
      exists = even count
      res = L.foldl' step ("" :: String, count `div` 2, count `div` 2) $ zip s t
      step (acc, slim, tlim) ('0', '0') = ('0' : acc, slim, tlim)
      step (acc, slim, tlim) ('1', '1') = ('0' : acc, slim, tlim)
      -- tのカウントがない時はsのハミング距離を調整
      step (acc, slim, 0) ('0', _) = ('1' : acc, slim - 1, 0)
      step (acc, slim, 0) ('1', _) = ('0' : acc, slim - 1, 0)
      step (acc, 0, tlim) (_, '0') = ('1' : acc, 0, tlim - 1)
      step (acc, 0, tlim) (_, '1') = ('0' : acc, 0, tlim - 1)
      step (acc, slim, tlim) (sc, tc) = case (sc, tc) of
        ('1', '0') -> ('0' : acc, slim - 1, tlim)
        ('0', '1') -> ('0' : acc, slim, tlim - 1)

  putStrLn $ if not exists then "-1" else reverse $ fst3 res
