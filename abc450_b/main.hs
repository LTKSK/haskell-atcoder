...
main = do
  [n] <- ints
  cs <- replicateM (n - 1) ints
  -- i->jへ向かう電車のコストが入っていて、csのi番目が駅で、添え字後ろにしか進まない
  -- n*nまでの2次元配列としてまとめよう
  -- let arr = listArray @UArray ((1,1), (n,n)) $ concat $ map (\(cs', i) -> [])(zip cs [1..])
  -- (i番目の駅、j番目の駅),コスト
  let costs =
        concat $
          map
            ( \(cs', i) ->
                let l = length cs'
                    p = n - l
                 in [c | (c, j) <- zip ((replicate p 0) ++ cs') [1 ..]]
            )
            (zip cs [1 ..])
      arr = listArray ((1, 1), (n, n)) costs :: UArray (Int, Int) Int
      -- 用意ができたら全探索
      res =
        not $
          null
