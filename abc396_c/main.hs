...
main = do
  ...
  ws <- L.sortBy (comparing Down) <$> ints
  -- 降順ソートして素直に考えると黒いボールを+の間だけ取った方が良い
  -- ただマイナスでもその分だけ白いボールがより大きなプラスだったら取った方が良い
  -- 白を基準に考えた方がいいかも？
  -- w+bが+の間二つを取って、黒が余ったら+の間取る
  let diffs = takeWhile (\(b, w) -> b + w >= 0) $ zip bs ws
      len = length diffs
      remaining = sum $ takeWhile (>= 0) $ drop len bs
      res = sum (map (\(b, w) -> b + (max 0 w)) diffs) + remaining
