...
main = do
  [n] <- ints
  cs <- listArray @UArray ((0, 0), (n - 1, n - 1)) . concat <$> replicateM n getLine
  let s = 0
      t = n * 2 + 1
      totalVerts = n * 2 + 2 -- 0番目があるから、というかs,tの二つを生徒の数と席の数（生徒と席の個数は==である）に足しただけ
      edges =
        L.concat
          -- sから各生徒へ繋ぐ。重みは以下すべて1である。生徒の番号は1basedになっているのでi+1
          [ [(s, i + 1, 1) | i <- [0 .. n - 1]],
            -- 生徒から席（#の位置）へ繋ぐ。席の番号も1based。n+1を基準にjを足している
            [(i + 1, n + 1 + j, 1) | i <- [0 .. n - 1], j <- [0 .. n - 1], cs ! (i, j) == '#'],
            -- 席からtまで繋ぐ
            [(n + 1 + j, t, 1) | j <- [0 .. n - 1]]
          ]
  g <- buildGraphDG totalVerts edges
  flow <- maxFlowDG g (s, t)
