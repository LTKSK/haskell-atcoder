...
main = do
  ...
  let n' = show n
      res = length [i | i <- [1 .. floorSqrt n], let d = length (show i), i * 10 ^ d + i <= n]
  print res

-- -- 2桁は9個、4桁は99個、6桁は999個。なのでnを10で割って2で割るとその桁の個数がわかる
-- -- 偶数桁の前後半が同じということは、桁の半分で表現できるだけの個数しか存在しない
-- -- ルールが適用できるstepと、そこからの端数のstepがあるんだよな
-- let digits = length $ show n
--     -- 桁数割る2をxでいれる。9+99+999...
--     solveOdd x
--       | x == 0 = 0
--       | x == 1 = 9
--       | otherwise = 10 ^ (x - 1) * 9 + solveOdd (x - 1)
--     -- 偶数桁の場合を考える
--     base = (solveOdd (digits - 3))
-- -- 奇数桁の時は、12345とか考えると、9999までしか考えないから4桁分まで
-- print $ if odd digits then solveOdd (digits - 1) else 1
