...
main = do
  ...
  count <- newIORef 0
  -- i番目のハトがどこの巣にいるかを管理
  pigeons <- newListArray @IOUArray (1, n) [1 .. n]
  holes <- newArray @IOUArray (1, n) (1 :: Int)

  forM_ qs $ \query -> case query of
    [1, p, h] -> do
      -- pのハトの巣の番号をゲット
      ph <- readArray pigeons p
      -- 今の巣のカウントをゲット
      pc <- readArray holes ph
      -- 移動先のカウントをゲット
      nc <- readArray holes h
      modifyIORef'
        count
        ( \prev ->
            prev
              + (if nc == 1 then 1 else 0)
              - (if pc == 2 then 1 else 0)
        )
      writeArray pigeons p h
      modifyArray holes ph (\prev -> prev - 1)
      modifyArray holes h (\prev -> prev + 1)
    [2] -> do
