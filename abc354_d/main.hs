...
main :: IO ()
main = do
  [a, b, c, d] <- map (+ 10 ^ 9) <$> ints

  -- 事前計算の累積和
  let cum :: UArray (Int, Int) Int =
        -- modを添え字に使うので0based
        listArray ((0, 0), (2, 4)) $
          concat
            [ [0, 0, 0, 0, 0],
      ...
      f y x =
        let (xq, xr) = x `divMod` 4
            (yq, yr) = y `divMod` 2
            -- 完全にかぶさる範囲
            res1 = xq * yq * (cum ! (2, 4))
            -- yだけ半端な時。xの幅はxqを使う。res1の長方形から上につながる部分
            res2 = xq * (cum ! (yr `mod` 2, 4))
            -- xだけ半端。yの高さはyqを使う。res1の長方形から右につながる部分
            res3 = yq * (cum ! (2, xr `mod` 4))
            -- 両方半端。res1の長方形の右上の部分
            res4 = cum ! (yr `mod` 2, xr `mod` 4)
         in res1 + res2 + res3 + res4
  print $ f d c - f b c - f d a + f b a

-- MEMO: 大きな周期性に気付けなかった
-- 周期性が手に入ったら、f x yを計算するだけ
-- +にするには4の倍数でないと周期が守れない
-- TODO 問題を解ける形に移行するには何を考えるべきだった？
-- TODO 二次元累積和に落とせる条件は？