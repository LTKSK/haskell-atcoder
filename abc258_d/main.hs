...
main = do
  ...
  -- 途中のステージの時点で、プレイ時間が残りのステージのいずれもよりも小さいならそこをひたすら回ればよいよな
  -- 最初の方にあるステージのストーリーは必須
  -- これで上手くいかないパターンは、クリア回数が例えば3回で、1つ目を3周した方が二つ目に行くより早い場合
  -- 重要なのは、これらは全て試すことが可能。abを1巡するのは問題ない
  -- 全探索する問題かも
  let res = L.foldl' step ini $ zip [0 ..] abs
      -- x回クリアするのに必要な回数,現在のステージ数
      ini = (maxBound :: Int, 0)
      step (ans, time) (i, (a, b)) =
        -- iはプレイ済みのステージの個数。1以上ないとこのステージに来ない
        let cnt = (x - i)
            next =
              if cnt < 1
                then maxBound
