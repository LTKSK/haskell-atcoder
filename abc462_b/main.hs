...
main = do
  [n] <- ints
  kas <- replicateM n ints
  -- iが送った人。送った人に対して自身を追加する
  let arr = accumArray @Array (flip (:)) ([] :: [Int]) (1, n) $ concat [map (,i) as | (i, (k : as)) <- zip [1 ..] kas]
  -- 送られた人数と送った人を記録しないといけない
  -- 人数 送った人（昇順）
  forM_ (elems arr) $ \vs -> do
    let l = length vs
    putStrLn $ unwords $ map show $ l : (L.sort vs)
