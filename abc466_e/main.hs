...
main = do
  ...
  -- dp
  let dp = accumArrayDP @UArray next relax ini bnds v0s abs
      relax = max
      ini = minBound :: Int
      -- boolは今裏向きの状態かどうか
      bnds = ((0, False), (k, True))
      v0s = [((0, False), 0 :: Int)]
      next ((curK, rev), acc) (a, b)
        | acc == minBound = []
        | curK == k =
            if rev
              then [((curK, False), acc + a), ((curK, True), acc + b)]
              else [((curK, False), acc + a)]
        | otherwise =
            if rev
              then [((curK, True), acc + b), ((curK, False), acc + a)]
              else [((curK + 1, True), acc + b), ((curK, False), acc + a)]
  print $ maximum $ concatMap (\kv -> [dp ! (kv, True), dp ! (kv, False)]) [0 .. k]
