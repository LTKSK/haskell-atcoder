...
main = do
  [n, m] <- ints
  rcs <- replicateM m intwo
  -- 後ろから見ていくとなんか楽そう！
  let res = L.foldl' step ini (reverse rcs)
      ini = (S.empty, S.empty, 0)
      step (rs, cs, acc) (r, c)
        | inRs || inCs = (S.insert r rs, S.insert c cs, acc)
        | otherwise = (S.insert r rs, S.insert c cs, acc + 1)
        where
          inRs = S.member r rs
          inCs = S.member c cs
  print $ thd3 res
