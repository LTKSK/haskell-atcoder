...
-- main = do
--   [h, w, q] <- ints
--   rcs <- replicateM q ints

--   let idx i j = (i - 1) * w + (j - 1)
--       total = h * w
--       unIdx x w = (x `div` w + 1, x `mod` w + 1)

--   alive <- newArray @IOUArray (0, total - 1) True
--   up <- newListArray @IOUArray (0, total - 1) [0 .. total - 1]
--   down <- newListArray @IOUArray (0, total - 1) [0 .. total - 1]
--   left <- newListArray @IOUArray (0, total - 1) [0 .. total - 1]
--   right <- newListArray @IOUArray (0, total - 1) [0 .. total - 1]
--   destroyed <- newIORef (0 :: Int)
--   -- arrをそのマスから次に壊れていないマスを指す配列として更新していく
--   -- これが直近の壊れていないマスへの探索をショートカットする
--   let findAndCompress arr x = do
--         p <- readArray arr x
--         -- xのマスが壊れていなかった
--         if p == x
--           then return x
--           else do
--             -- 再帰
--             r <- findAndCompress arr p
--             -- 経路を圧縮して、xから見た直近生きている壁をrに更新
--             writeArray arr x r
--             return r
...
--         a <- readArray alive x
--         when a $ do
--           writeArray alive x False
--           -- 壊れたら個数+1
--           modifyIORef' destroyed (+ 1)
--           -- xに来たら次調べるマスを更新している。ここでは隣接のマスを指すだけで、経路の圧縮はfindで行う
--           when (i > 1) $ writeArray up x (idx (i - 1) j)
--           when (i < h) $ writeArray down x (idx (i + 1) j)
--           when (j > 1) $ writeArray left x (idx i (j - 1))
--           when (j < w) $ writeArray right x (idx i (j + 1))
...
--         -- 上
--         when (r > 1) $ do
--           u <- findAndCompress up (idx (r - 1) c)
--           ua <- readArray alive u
--           when ua $ let (i, _) = unIdx u w in breakWall i c
--         -- 下
--         when (r < h) $ do
--           d <- findAndCompress down (idx (r + 1) c)
--           da <- readArray alive d
--           when da $ let (i, _) = unIdx d w in breakWall i c
--         -- 左
--         when (c > 1) $ do
--           l <- findAndCompress left (idx r (c - 1))
--           la <- readArray alive l
--           when la $ let (_, j) = unIdx l w in breakWall r j
--         -- 右
--         when (c < w) $ do
--           rr <- findAndCompress right (idx r (c + 1))
--           ra <- readArray alive rr
--           when ra $ let (_, j) = unIdx rr w in breakWall r j
--   d <- readIORef destroyed
--   print (total - d)
...
main = do
  ...
  rcs <- replicateM q ints
  -- c列で生きている行番号の集合
  colSet <- newArray @IOArray (1, w) (IS.fromList [1 .. h])
  -- r行で生きている列番号の集合
  rowSet <- newArray @IOArray (1, h) (IS.fromList [1 .. w])

  destroyed <- newIORef (0 :: Int)

  let breakWall i j = do
        modifyArray colSet j (IS.delete i)
        modifyArray rowSet i (IS.delete j)
        modifyIORef' destroyed (+ 1)
  forM_ rcs $ \[r, c] -> do
    col <- readArray colSet c
    if IS.member r col
      then breakWall r c
      else do
        row <- readArray rowSet r
        let up = IS.lookupLT r col -- c列でr未満最大。上
            down = IS.lookupGT r col -- c列でr超えの最小。下
            leftW = IS.lookupLT c row -- r行でc未満最大。左
            rightW = IS.lookupGT c row -- r行でc超え最小。右
        forM_ [up, down] $ \case
          Just i -> breakWall i c
          Nothing -> return ()
        forM_ [leftW, rightW] $ \case
          Just j -> breakWall r j
          Nothing -> return ()
