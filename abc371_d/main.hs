...
main = do
  ...
  xs <- ints
  ps <- ints
  let sorted = L.sortBy (comparing snd) $ zip ps xs
      pxs = listArray @Array (1, n) $ sorted
      psum = listArray @UArray (0, n) $ L.scanl' (+) 0 $ map fst sorted
  [q] <- ints
  lrs <- replicateM q ints
  forM_ lrs $ \[l, r] -> do
    let li = binSearchMin (\i -> l <= snd (pxs ! i)) 0 (n + 1)
    let ri = binSearchMax (\i -> r >= snd (pxs ! i)) 0 (n + 1)
    print $ psum ! ri - psum ! (li - 1)
