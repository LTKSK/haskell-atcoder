...
main = do
  ...
  s <- getLine
  cs <- ints
  let dp = accumArrayDP @UArray next relax ini bnds v0s $ zip (map digitToInt s) cs
      relax = min
      ini = (maxBound :: Int)
      bnds = ((0, 0), (2, 1)) -- (state, 前の文字)。stateは文字列開始前、ゴママヨ前、ゴママヨ後
      -- 初期値を2通り用意して、1文字目を変更した場合と変更しなかった場合のいずれも計上できるようにする
      v0s = [((0, 0), 0), ((0, 1), 0)]
      next ((state, prev), acc) (c, cost)
        | acc == maxBound = []
        | otherwise = case state of
            0
              | prev == c -> [((1, if c == 1 then 0 else 1), acc + cost)]
              | otherwise -> [((1, c), acc)]
            1
              | prev == c -> [((1, if c == 1 then 0 else 1), acc + cost), ((2, c), acc)]
              | otherwise -> [((1, c), acc), ((2, if c == 1 then 0 else 1), acc + cost)]
            2
              | prev == c -> [((2, if c == 1 then 0 else 1), acc + cost)]
              | otherwise -> [((2, c), acc)]

  -- let ((firstC, firstCost) : xs) = zip (map digitToInt s) cs
  --     dp = accumArrayDP @UArray next relax ini bnds v0s $ xs
  --     relax = min
  --     ini = (maxBound :: Int)
  --     bnds = ((0, 0), (2, 1))
  --     -- 初期値を2通り用意して、1文字目を変更した場合と変更しなかった場合のいずれも計上できるようにする
  --     v0s = [((1, firstC), 0), ((1, if firstC == 1 then 0 else 1), firstCost)]
  --     next ((state, prev), acc) (c, cost)
  --       | acc == maxBound = []
  --       | otherwise = case state of
  --           1
