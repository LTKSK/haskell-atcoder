...
main = do
  ...
  let gen :: Int -> Int -> Int -> [[Int]]
      ...
      res = gen n m 1
  -- kが残りのn個作るまでに必要な個数だから、10*(k-1)をmから引いた値がi番目で作れる個数の上限になる
  print (length res)
  mapM_ (putStrLn . unwords . map show) res

--     -- 長さkの列を作る
--     gen1 0 _ = [[]]
--     gen1 k m = [v : rest | v <- [1 .. m], rest <- gen1 (k - 1) m]
--     gen2 0 _ _ = [[]]
--     -- 下限を
--     gen2 k m p = [v : rest | v <- [p .. m], rest <- gen2 (k - 1) m (v + 10)]
-- print $ gen2 2 12 1

-- 1つの解は何か（今回は長さ N の整数列）
-- それを何回の選択で作るか（N 回。1項ずつ）
-- 1回の選択で何を決めるか（その位置の値）
-- 選ぶときに必要な情報は何か（残り個数、下限、上限）