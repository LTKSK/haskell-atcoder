...
-- 桁数 d の中に収まるか順に引いていく。
-- 返り値は (その桁の中で何番目か[1始まり],桁数)
findDigits :: Integer -> Integer -> (Integer, Integer)
findDigits n d
  | n <= cnt = (n, d)
  | otherwise = findDigits (n - cnt) (d + 1)
  where
    -- 10^0で1になるのを忘れずに
    cnt = 10 ^ ((d + 1) `div` 2 - 1) * 9

nthPalindrome :: Integer -> String
nthPalindrome 1 = "0"
nthPalindrome n = palindrome
  where
    -- 1の時は0で、0があると1桁の時だけ個数が10個になるので1つずらす
    n' = n - 1
    (rem, d) = findDigits n' 1
    -- 桁数が手に入ったので、半分の桁数を手に入れる
    halfDigit = (d + 1) `div` 2
    base = 10 ^ (halfDigit - 1)
    -- remはbaseから始めてrem個目。個数なので1引く
    start = base + rem - 1
    half = show start
    palindrome
      | even d = half ++ reverse half
      | otherwise = half ++ reverse (init half)

main :: IO ()
main = do
  [n] <- integers
  putStrLn $ nthPalindrome n
