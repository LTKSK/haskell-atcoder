...
main = do
  [n] <- ints
  uvs <- replicateM (n - 1) ints
  let g = buildGraph (1, n) uvs
  -- 葉までの最短距離じゃないんだよな。1から考えるのでいいのか...?
  -- node1から考える。まず葉の最短距離は、その葉が兄弟がいるかわからないので使えないな
  -- bfsの亜種でいけそうな気はする。多始点bfs的なのも考えよう
  -- でも考えるのはある頂点から繋がっているnodeの数か。木DPっていうんだっけ
  -- aに繋がる頂点すべてに対して、木DPで最大値を求め、その中の最小値を使おう
  -- bfsで親子関係を作りつつ
  let dist = bfs g 1
      targets = L.sortBy (comparing (Down . snd)) $ assocs dist
  res <- newArray @IOUArray (1, n) (0 :: Int)
  -- 1+sumで緩和！
  forM_ targets $ \(n, _) -> do
    let childlen = g ! n
    v <- filter (/= 0) <$> mapM (readArray res) childlen
    writeArray res n (if null v then 1 else sum v + 1)

  ans <- forM (g ! 1) $ \node -> do
    readArray res node
  print $ sum (init $ L.sort ans) + 1
