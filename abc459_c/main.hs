...
main = do
  ...
  modifyIORef' mref (\prev -> M.insert 0 n prev)

  forM_ qs $ \case
    [1, x] -> do
      prev <- readArray arr x
      dc <- readIORef decCount

      let next = (prev + 1)
      writeArray arr x next
      -- 個数が増えたところをincrement
      modifyIORef' mref (\prev -> M.insertWith (+) next 1 prev)
      -- mのnextのマスの数がnと同じだったらdc+1
      m <- readIORef mref
      let v = m M.! next
      when (v == n) $ do
        modifyIORef' decCount (+ 1)
    [2, y] -> do
      dc <- readIORef decCount
      m <- readIORef mref
      let y' = y + dc
          res = case M.lookup y' m of
            Nothing -> 0
            Just v -> v
      print res
