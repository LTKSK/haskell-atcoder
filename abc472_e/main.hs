...
bfs' graph start = runST $ do
  dist <- newArray (bounds graph) (-1) :: ST s (STUArray s Int Int)
  -- 2部グラフの塗分け用
  par <- newArray (bounds graph) 0 :: ST s (STUArray s Int Int)
  -- スタート地点のコストは0
  writeArray dist start 0
  -- queueは再帰で引き回すのでrefで持たない
  go (Seq.singleton start) dist par
  d <- freeze dist
  p <- freeze par
  pure (d, p)
  where
    go Seq.Empty dist par = return ()
    go (u Seq.:<| queue) dist par = do
      d <- readArray dist u
      queue' <- foldM (step d) queue (graph ! u)
      go queue' dist par
      where
...
main = do
  ...
  replicateM_ t $ do
    ...
    let g = buildGraph (1, n) abs
        (dist, par) = bfs' g 1
        -- 偶奇を求める関数
        color v = dist ! v `mod` 2
        -- 奇数の辺を求める。全域木では繋がっているnodeは必ず色が違うので、
        -- 両端の色が同じ辺は必ず閉路を作る
        pathToRoot v = v : if par ! v == 0 then [] else pathToRoot (par ! v)
    case L.find (\[a, b] -> color a == color b) abs of
      Nothing -> print (-1)
      Just [u, v] -> do
        let ru = reverse (pathToRoot u)
            rv = reverse (pathToRoot v)
            common = length $ takeWhile id $ zipWith (==) ru rv
            res = reverse (drop (common - 1) ru) ++ drop common rv
        print (length res)
