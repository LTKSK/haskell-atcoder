...
main = do
  [n, m] <- ints
  as <- ints
  -- 単純にやるならarrayとかmapに詰めてmaximumを取る
  -- mapでいいかな。全体の数もそこまで多くないから
  -- 全体の票数と、最後に当選した人を記録しておけば求められるな
  let (res, m'') = L.foldl' step ini as
      ini = ([], M.empty)
      step ([], m) a =
        let m' = M.insertWith (+) a 1 m
         in ([(a, 1)], m')
      step (acc@((pk, pv) : _), m) a = case M.lookup a m of
        -- aがない場合
        Nothing ->
          let m' = M.insertWith (+) a 1 m
              next =
        ...
        Just v ->
          let m' = M.insertWith (+) a 1 m
              next
                | pv == v + 1 = min (a, v + 1) (pk, pv)
                | pv > v + 1 = (pk, pv)
                | otherwise = (a, v + 1)
           in (next : acc, m')

  mapM_ print $ map fst $ reverse res
