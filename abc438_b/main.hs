...
main = do
  ...
  t <- getLine
  -- 部分文字列になるまで足せばよい
  -- t = 23, s = 115だったら、|11 - 23| = 12, |15 - 23| = 8+1 + 2 マイナスになったら10足せばいいのか
  -- この計算をtをずらして行ってminを取りましょう
  let check (sv, tv)
        | digitToInt sv < digitToInt tv = digitToInt sv - digitToInt tv + 10
        | otherwise = digitToInt sv - digitToInt tv
      res = minimum [sum $ map check $ zip (drop d s) t | d <- [0 .. n - m]]
  print res
