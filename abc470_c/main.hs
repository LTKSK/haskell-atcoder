...
main = do
  ...
  b <- newIORef (0 :: Int)
  -- set
  s <- newIORef (S.empty :: S.Set Int)
  replicateM_ q $ do
    qs <- ints
    -- ai+1するというのはxorにどのような影響があるか
    -- 2の操作はxorにどのような影響があるか
    -- この二つが分かればいけそうな？
    case qs of
      -- まずxorが1になる条件は2つのbitで片方だけが1
      -- ai..anの場合、1の個数が奇数個じゃないとダメ
      -- 1,1,1は1。0,0,1も1。0,1,0も1。011は0.0,1,0,1は0。
      -- 0,0,0,1は1。1110は1。1111100は1。11110は0
      -- なので1の個数が奇数個じゃないとダメ
      [1, x] -> do
        -- 数値の種類。同じ数字が偶数個あったらxorは0なんだよーね
        -- bitの1が奇数個あると数値に乗るんだけど、これをどうやると綺麗に計算できるんだ？
        -- 足し算の影響。bitが増減する範囲が広いんだよな
        -- いや待てよ？都度計算しようと思っていたけれど、
        -- xorの前回の結果に対して何か操作をすることで結果を得ることができないか？
        v <- readIORef b
        -- 現在の値から2が出た回数を引くと実際の値が手に入る
        cur <- readArray arr x
        let cur' = cur + 1
            -- cur'をxorして答えのbitから今参照している影響の値を削除
            -- +1した値でxorすれば残りへの影響は無視して計算できる
            v' = v `xor` cur `xor` cur'
        writeArray arr x cur'

        print v'
        writeIORef b v'
        modifyIORef' s (S.insert x)
      [2] -> do
        -- 1以上になっている要素を取得
        targets <- readIORef s
        forM_ (S.toList targets) $ \i -> do
          v <- readArray arr i
          writeArray arr i (v - 1)
          modifyIORef' b (\x -> x `xor` v `xor` (v - 1))
          when (v - 1 == 0) $ modifyIORef' s (S.delete i)
        readIORef b >>= print
