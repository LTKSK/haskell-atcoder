...
buildDirGraph :: (Int, Int) -> [[Int]] -> Array Int [Int]
buildDirGraph (i, n) uvs = accumArray (flip (:)) [] (i, n) xs
  where
    -- 黒い頂点からvに到達できるかという形に考え方を変えるためにu,vをひっくり返す
    xs = map (\[u, v] -> (v, u)) uvs

main :: IO ()
main = do
  [n, m] <- ints
  xys <- replicateM m ints
  [q] <- ints
  -- vectorはarrayと違って添え字の範囲は指定できない
  -- indexこねたくないのでn+1作っておく
  visited <- VUM.replicate (n + 1) False

  -- dfsをやる
  let g = buildDirGraph (1, n) xys
      dfs :: Int -> IO ()
      dfs u = do
        VUM.write visited u True
        forM_ (g ! u) $ \v -> do
          vis <- VUM.read visited v
          unless vis $ do
