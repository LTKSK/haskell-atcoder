...
main = do
  [n, m] <- ints
  abxys <- replicateM m ints
  let buildGraph' :: (Int, Int) -> [[Int]] -> Array Int [(Int, (Int, Int))]
      buildGraph' (i, n) abxys = accumArray (flip (:)) [] (i, n) xs
        where
          xs = concatMap (\[a, b, x, y] -> [(a, (b, (y, x))), (b, (a, (-y, -x)))]) abxys
      -- arr <- newArray @IOArray (1, n) (-1, -1)
      -- 人を辿って、原点に辿りつけるか？ってことだと思うんだよな
      -- たどりつけない座標にあるものがundecidableな感じでね
      g = buildGraph' (1, n) abxys
      -- このgraphを探索済みの頂点を覚えつつ探索する
      solve :: Seq.Seq ((Int, Int), Int, (Int, Int)) -> M.Map Int (Int, Int) -> M.Map Int (Int, Int)
      solve Seq.Empty visited = visited
      solve ((prevPos, i, delta) Seq.:<| rest) visited = case M.lookup i visited of
        Just _ -> solve rest visited
        Nothing ->
          let nextPos = (prevPos + delta)
              visited' = M.insert i nextPos visited
              -- 走査したい頂点を探す
              vs = [(nextPos, j, ix) | (j, ix) <- g ! i, not $ M.member j visited]
           in solve (rest Seq.>< Seq.fromList vs) visited'
      res = solve (Seq.fromList (map (\(i, delta) -> ((0, 0), i, delta)) $ g ! 1)) (M.singleton 1 (0, 0))

  forM_ [1 .. n] $ \i -> do
    let pos = M.lookup i res
    putStrLn $ case pos of
      Just (y, x) -> do
