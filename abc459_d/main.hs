...
main = do
  ...
  replicateM_ t $ do
    ...
    let slen = length s
        b = (slen + 1) `div` 2
        arr :: UArray Char Int = accumArray (+) 0 ('a', 'z') $ map (,1) s
        can = all (<= b) $ elems arr
    if not can
      then do
        putStrLn "No"
      else do
        let vs = L.sortBy (comparing (Down . snd)) $ filter ((/= 0) . snd) $ assocs arr
            vs' = concat [replicate v k | (k, v) <- vs]
            idx = [i | i <- [0 .. slen - 1], even i] ++ [i | i <- [0 .. slen - 1], odd i]
            res = map snd $ L.sortBy (comparing fst) $ zip idx vs'
        putStrLn "Yes"
        putStrLn res
