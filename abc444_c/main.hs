...
-- lを全てのasが消化できるかを検証する
check :: Int -> [Int] -> Bool
check l as = go mp sorted
  where
    -- mapで出現回数をまず集計
    mp = M.fromListWith (+) [(a, 1 :: Int) | a <- as]
    sorted = L.sort as
    -- sort済みのasに対して処理を実行
    go freq [] = True -- 全て消化できたので有効なl
    go freq (a : rest)
      | M.findWithDefault 0 a freq <= 0 = go freq rest -- 既に消化済み
      | a == l = go (M.adjust (subtract 1) a freq) rest -- lと同じなのでそのまま一本消化して、aの値を1減らす
      | b <= 0 = False -- aがlを超えてしまっている
      | M.findWithDefault 0 b freq' <= 0 = False -- 0未満なのでもう相方が居ない
      | otherwise = go (M.adjust (subtract 1) b freq') rest -- ペアを組めるので、aとbを減らして次
      where
        b = l - a
        freq' = M.adjust (subtract 1) a freq

main :: IO ()
main = do
  -- as <- listArray @Array (1, n) <$> integers
  -- 1回シェイクする前の状態としてありうるLを示す
  -- 全てが同じ長さLである
  -- 長さLの制約は、maximum as * 2
  -- Lとしてありうるのは、as++asから2つ選んだ組み合わせを足したもの
  -- Lの本数としてありうるのは、(n+1)`div`2
  -- sample3は、(n+1)`div`2以上の本数があるのが444だけなんだよな
  -- asの総和を割り切れる値でなければならない
  -- 選択肢はasのmax含めて、asすべてを足し合わせた数である
  -- let mx = maximum as
  --     mn = minimum as
  --     s = sum as
  --     -- ありうるパターン
  ...
  --     -- 両方が答え
  --     -- これ以外答えになりえない
  --     -- ここの最後のelse節はmxの場合とmx+miの場合がありえる。2で割れるか？じゃダメだ
  --     -- 総和を割り切れるか、でやるか
  --     -- mx == 任意のa + mn
  --     -- mx+mn == ai+aj
  --     -- res = if mx == mn then [mx] else if s `rem` mx == 0 then [mx, mx + mn] else [mx + mn]
  --     res = if mx == mn then [mx] else filter (\v -> s `rem` v == 0) [mx, mx + mn]
  -- putStrLn $ unwords $ map show res
  -- let s = sum as
  --     mx = maximum as
  --     -- 候補を列挙。sの約数のうち、mx以上のものが答えの候補になりうる
  --     -- Lの必要条件はmx以上で、sumの約数であること
  --     divs = L.sort $ filter (>= mx) $ divisors s
  --     -- 候補をcheck
  --     res = filter (\l -> check l as) divs
  -- putStrLn $ unwords $ map show res
  ...
  as <- L.sort <$> ints
  -- 全部割れとる場合
  -- sort済みなら頭と末尾を順に足し合わせる以外に上限を満たす組み合わせはない
  let solve1 sorted
        | even n, all (== l) (zipWith (+) sorted (reverse sorted)) = [l]
        | otherwise = []
        where
          l = head sorted + last sorted

      -- どれか残ってる場合
      solve2 sorted
        | even (length remaining), all (== l) (zipWith (+) remaining (reverse remaining)) = [l]
        | otherwise = []
        where
          l = last sorted
          remaining = filter (/= l) sorted
  putStrLn $ unwords $ map show $ (solve2 as ++ solve1 as)