...
main = do
  ...
  as <- L.sort <$> integers
  -- 日本酒の条件を固定する系。だと思う
  -- 確実に飲める時を、小さい方から水だけ入っている場合、酒だけ入っている場合で考えます
  -- 酒にありつくためには、少なくともn-k+1個は飲まないといけない。なぜならn-k個は水の可能性があるから
  -- これを考えると、答えはn-k個+aをsortして、少ない方からkを越すまで飲んだ時の個数
  let drank as x = go as x 0 0
        where
          go :: [Integer] -> Integer -> Integer -> Integer -> Integer
          go [] _ acc c = c
          go (a : as) x acc c
            | acc >= x = c
            | otherwise = go as x (acc + a) (c + 1)
      sakes = take (fromIntegral k) as

      res = drank (reverse sakes) x
      sake = sum sakes

  print $ if sake < x then -1 else n - k + res
