...
main = do
  [n] <- ints
  as <- ints
  -- 積の素因数分解で、全ての指数が偶数であること
  let ks = map (toKey . M.fromListWith (+) . primeFactors) as
      -- 肩が奇数の素因数を探して、それをかけ合わせる。この結果が同じものがペアになるので
      toKey m = product [p | (p, e) <- M.assocs m, odd e]
      g = M.fromListWith (+) $ map (,1) ks
      -- 0の場合を考慮
      res =
        sum
          [ v' -- nC2の計算
            | (k, v) <- M.assocs g,
              let v' = if k /= 0 then v * (v - 1) `div` 2 else v * (v - 1) `div` 2 + v * (n - v)
          ]
  print res
