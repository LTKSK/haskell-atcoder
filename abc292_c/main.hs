...
main = do
  [n] <- ints
  -- ab+cdの個数を数え上げ
  -- a*bが2つあるから、nのrootまでかな
  -- 入れ替えて同じものを区別せずに計上する。なので一方向に計上して4倍すれば良さそうな

  -- 型定義必須
  -- let d :: UArray Int Int
  --     d = runSTUArray $ do
  --       arr <- newArray (1, n) 0
  --       forM_ [1 .. n] $ \i ->
  --         forM_ [i, i * 2 .. n] $ \j ->
  ...
  -- d(m)をmの正の約数の個数として考える
  -- a*b=mを満たす整数の組A,bの個数はAの約数をどれかに決めたらBの個数が決まるのでちょうどm個
  -- 1..nまでの値に対して、1を約数とするものの個数、2を約数とするものの個数...と計上
  let d :: UArray Int Int = accumArray (+) 0 (1, n) [(j, 1) | i <- [1 .. n], j <- [i, i * 2 .. n]]
      res = sum [d ! x * d ! (n - x) | x <- [1 .. n - 1]]
  -- divCount x = sum [if q == a then 1 else 2 | a <- [1 .. floorSqrt x], let (q, r) = x `divMod` a, r == 0]
  -- res' =
  --   sum
  --     [ divCount x * divCount (n - x)
  --       | x <- [1 .. n - 1]
  --     ]
  print res
