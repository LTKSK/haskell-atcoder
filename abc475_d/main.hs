...
main = do
  s <- getLine
  -- pは素数。7桁まで。これを文字列として表す
  -- si=sjであり、ti=tjである。つまりsで同じ数値のところは同じ数値であるとした時の素数
  -- cocotだったらcとoは同じ数字で、tはそれ以外の数字じゃないといけない
  -- 文字は最大7種類。全探索して、都度isPrimeとっても間に合いそうではある
  -- 10^7までの数字を順に追いかけていくとそれだけで間に合わない
  -- まてよ、10^7までじゃないや。sの桁数の数値だけを見ればいいんだから、最大でも10^6回だ
  let len = length s
      -- 比較用に登場したindicesをsortした多次元配列で持つ
      -- 文字は先頭からみた登場順を参照する
      make s = map (\(i, xs) -> L.sort xs) $ L.sortOn (\(k, _) -> fromMaybe (-1) $ L.elemIndex k s) $ M.assocs $ M.fromListWith (++) [(c, [i]) | (i, c) <- zip [1 ..] s]
      m = make s
      -- 候補一覧
      l = 10 ^ len - (10 ^ len - 10 ^ (len - 1))
      ps = dropWhile (<= l) $ map fst $ filter snd $ assocs $ sieve (10 ^ length s)
      -- 1つ見つかればOK
      res =
        take
          1
  ...
  putStrLn $ if null res then "-1" else show $ head res
