...
main = do
  [h, w, a, b] <- ints
  -- dfsをして、Trueになったら1を返す感じかな...?
  -- 使い切らなきゃいけないことに注意。Aを埋めたらBは自動的に埋まるな
  -- 次に畳を置ける場所を探す
  -- 畳みを置けるか判定する
  -- 90度回転する
  -- 注意点。使わない選択肢は存在しない。パターン数を列挙する必要がある
  let canPlace s ps = all (inRange ((1, 1), (h, w))) ps && all (\p -> not $ S.member p s) ps
      findEmpty s =
        let ps = [p | p <- range ((1, 1), (h, w)), not $ S.member p s]
         in if null ps then Nothing else Just $ head ps
      place s ps = S.union s (S.fromList ps)

      solve s ac bc
        | S.size s == h * w = if ac == 0 && bc == 0 then 1 else 0 -- 埋まったら1
        -- 0枚でまだ埋まってなかったら0
        | otherwise =
            let next = findEmpty s
             in case next of
                  Nothing -> 0
                  Just p ->
                    sum
                      [ solve (place s ps) (ac - 1) bc
                        | ps <- [[p, p + (0, 1)], [p, p + (1, 0)]],
                          canPlace s ps
                      ]
                      + if bc > 0 then solve (place s [p]) ac (bc - 1) else 0
      res = solve S.empty a b
  print res
