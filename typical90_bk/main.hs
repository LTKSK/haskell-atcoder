...
main = do
  [h, w] <- ints
  ps <- listArray @UArray ((1, 1), (h, w)) . concat <$> replicateM h ints
  -- subsequencesで行の選び方を一通り列挙できる
  -- 探索の仕方を学ぼう
  let allSame xs = all (== head xs) xs
      res =
        maximum
          -- 選んだ行の数*列の数(groupのlength)がその選び方の答え
          [ length rows * colsVal
            | -- 行の選び方列挙
              rows <- L.subsequences [1 .. h],
              -- subsequenceは[]の時があるのでfilter
              not $ null rows,
              let sameCols =
                    [ head cols -- 全て同じ数字の列が見つかったらその配列の要素だけ取れれば良い
                      | x <- [1 .. w],
                        -- 選んだ行ごとに、各列の値を集計
                        let cols = [ps ! (y, x) | y <- rows],
                        -- 全ての要素が同じものだけ抽出
                        allSame cols
                    ],
              -- ここも空の時があるでな
              not $ null sameCols,
              -- sortしてgroupを取ると、列を占めている同じ数字がそれぞれいくつ出てくるか集計できる
              -- あとはそれらの長さの最大を取る
              let colsVal = maximum $ map length $ L.group $ L.sort sameCols
          ]
  print res
