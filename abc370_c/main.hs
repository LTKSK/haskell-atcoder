...
main = do
  s <- getLine
  t <- getLine
  -- sとtの差を減らす過程を記録しろという問題
  let -- sc,tcを辞書順を考慮して並べなければならない
      -- tを頭から見て、tcがscより小さいなら有線。大きいなら後回し
      -- 更新する文字をどの順番で選ぶか
      -- まず差が+、tcに置き換えると改善するものについてはiが小さいものを採用するべき
      -- 逆に差が-、tcに置き換えると悪化するものについてはiが大きいものを採用するべき
      -- filterしてsortしてくっつけよう
      diff :: [(Int, Char, Int)]
      diff = catMaybes $ (map \(i, sc, tc) -> if sc == tc then Nothing else Just (i, tc, ord sc - ord tc)) (zip3 [1 ..] s t)
      diffLeft = L.sortBy (comparing fst3) $ filter (\(i, tc, d) -> d > 0) diff
      diffRight = L.sortBy (comparing (Down . fst3)) $ filter (\(i, tc, d) -> d < 0) diff
      s' = listArray @UArray (1, length s) s
      res = L.foldl' (\acc@(s' : _) (i, v, _) -> let s'' = (s' // [(i, v)]) in (s'' : acc)) [s'] (diffLeft ++ diffRight)
  print $ length res - 1
  mapM_ putStrLn $ reverse $ init $ map elems res
