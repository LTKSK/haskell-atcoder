...
main = do
  [n] <- ints
  s <- getLine
  let psumO :: UArray Int Int = listArray (1, n) $ tail $ L.scanl' (\acc v -> acc + bool 0 1 (v == 'o')) 0 s
      solve i offset
        | offset <= 0 = i
        | i + offset >= n = n
        | otherwise =
            let oc = (psumO ! (i + offset) - psumO ! i)
             in solve (i + offset) oc

  forM_ [1 .. n] $ \i -> do
    print $ solve i (psumO ! i)

-- WA後
-- まずpsumOがiからjump可能な位置であることは間違いないと思う
-- ミスがあるとしたら、
-- 3>5に飛んだ時、4,5が○だった場合はどうなるか。そもそもjumpの考えが間違っているかもしれない
-- i番目までの累積和で、追加で何個空けられるかは決まっている。これは自信あり
-- i番目をスタート地点としたときに、どこまで到達できるか
-- 今だとpsumの算出がイマイチかもな。psumは全体を通しての累積和になっているけれど、今回関心があるのは後ろからだもんな
-- じゃあそもそも後ろから数えたらOKか？

-- oxxxooxox
-- 5の時の残りはoxox。これは最後まで行けちゃうんだよな>5から飛ぶ先は7。
-- 7まで飛んだ時、oの個数とxの個数はわかるんだよな。
-- 5の時点でoffsetは2
-- 2進んだ時に+-0だからoffsetはまだ2だ
