...
main = do
  [n] <- ints
  -- let topX = floor $ sqrt (fromIntegral n / 2 :: Double)
  -- let cnt = runSTUArray $ do
  --       arr <- newArray (1, n) 0 :: ST s (STUArray s Int Int)
  --       forM_ [1 .. topX] $ \x -> do
  --         let x2 = x * x
  --             minY = x + 1
  --             maxY = floor $ sqrt $ (fromIntegral (n - x2))
  --         forM_ [minY .. maxY] $ \y -> do
  --           let v = x2 + y * y
  --           c <- readArray arr v
  --           writeArray arr v (c + 1)
  --       return arr
  --     res = [i | i <- [1 .. n], cnt ! i == 1]
  --     len = length res
  -- print len
...
-- このvを生成できる値が他にあるなら除外しないといけないが...
-- 重複している値は削ろう
-- m =
--   IM.fromListWith
--     (+)
-- [ (v, 1) | x <- [1 .. topX], let x2 = x * x
--                                  minY = x + 1
--                                  maxY = floor $ sqrt $ (fromIntegral (n - x2)), y <- [minY .. maxY], let v = x2 + y * y
-- ]
-- res = [v | (v, c) <- IM.toAscList m, c == 1]
-- len = length res
