...
main = do
  ...
  -- n,mが<=10なので全探索しよう
  let ss' = [S.fromList [i | (i, c) <- zip [1 ..] s, c == 'o'] | s <- ss]
      check p = S.size $ L.foldl' (\acc i -> let s' = ss' !! i in S.union acc s') S.empty p
      res = minimum [length p | p <- L.subsequences [0 .. n - 1], not $ null p, check p == m]
