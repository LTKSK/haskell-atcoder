...
-- type MGraph = MV.IOVector [Edge]

-- newGraph :: Int -> IO MGraph
-- newGraph n = MV.replicate n []

-- addEdge :: MGraph -> Int -> Int -> Int -> IO ()
-- addEdge g from to cap = do
--   fromEdges <- MV.read g from
--   toEdges <- MV.read g to

--   let fromLen = length fromEdges
--       toLen = length toEdges
--   -- 順方向の辺。容量アリ
--   MV.write g from (Edge to cap toLen : fromEdges)
--   -- 逆方向の辺。最初は容量無し
--   MV.write g to (Edge from 0 fromLen : toEdges)

-- dfsmg :: MGraph -> VUM.IOVector Bool -> Int -> Int -> Int -> IO Int
-- dfsmg g visited s t !flow
--   | s == t = return flow -- 終点
--   | otherwise = do
--       vis <- VUM.read visited s
--       if vis
--         then return 0 -- 訪問済み
--         else do
--           VUM.write visited s True
--           edges <- MV.read g s
--           go edges []
--   where
--     -- 隣接する辺を順に試す。seenには処理済みの辺を入れ、後で戻せるように
--     go [] seen = do
--       MV.write g s (reverse seen)
--       return 0
--     go (e : es) seen
--       | cap e <= 0 = go es (e : seen) -- 容量0なら飛ばす
--       | otherwise = do
--           flow <- dfsmg g visited (to e) t (min flow (cap e))
--           if flow > 0
--             then do
--               -- 流せるので辺を更新して順方向の辺の容量を減らし、逆方向の辺の容量を増やす
--               let e' = e {cap = cap e - flow}
--               revEdges <- MV.read g (to e)
--               let revEdges' = updateEdge (rev e) flow revEdges
--               MV.write g s (reverse seen ++ e' : es)
--               MV.write g (to e) revEdges'
--               return flow
--             else
--               -- 流せなかったら次の辺を試す
--               go es (e : seen)

-- updateEdge :: Int -> Int -> [Edge] -> [Edge]
-- updateEdge idx flow edges = go idx edges
--   where
--     go 0 (e : es) = e {cap = cap e + flow} : es
--     go n (e : es) = e : go (n - 1) es
--     go _ [] = []

-- maxFlow :: MGraph -> Int -> Int -> Int -> IO Int
-- maxFlow g source sink n = go 0
--   where
--     go !total = do
--       -- 訪問済みフラグを初期化
--       visited <- VUM.replicate n False
--       -- 増加パスを探索
--       flow <- dfsmg g visited source sink maxBound
--       if flow > 0
--         then go (total + flow) -- フローを追加して繰り返し
--         else return total
...
main = do
  [n, m] <- ints
  abcs <- replicateM m $ do
    [a, b, c] <- ints
    return (a - 1, b - 1, c)
  dg <- buildGraphDG n abcs
  flow <- maxFlowDG dg (0, n - 1)
  print flow
