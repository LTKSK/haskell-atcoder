...
main = do
  ...
  let check x ascs = go ascs k
        where
          go [] k' = k' >= 0
          go ((i, a) : rest) k'
            | a >= x = go rest k'
            -- divで割り切れる時に+1だとおかしくなる。i-1を足しておくことでceilが成立
            | otherwise = let next = (x - a + i - 1) `div` i in go rest (k' - next)
      res = binSearchMax (\x -> check x (assocs as)) 1 ((as ! 1) + k + 1)
  print res

-- WA: go [] 0 = Falseとしたけど、sortしていないので全要素みないとだめ
-- WA: asをsortしてしまった
-- WA: ceilの計算ミス。`div` i + 1 は割り切れるときにおかしくなる
-- WA: integerにしなかった
