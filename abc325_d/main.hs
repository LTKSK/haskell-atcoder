...
main = do
  [n] <- ints
  tds <- replicateM n (do [t, d] <- integers; return (t, t + d))
  -- 終了時間昇順でsort。この時開始時間の早いものから採用する
  -- 1.現在時刻t以下の開始時間を持つ商品をheapへ入れる
  -- 2.ヒープが空になったらば次の商品の開始時間までtを移動
  -- 3.ヒープから終了時刻が最も早いものを取り出す。tは現在時刻なので、t未満のendを持つものは採用できない
  let tds' = L.sort tds
      -- heapが空になるまで続けるので、tdsの入力が終わったらdrainに進む
      solve [] heap t acc = drain heap t acc
      solve remaining@((start, end) : rest) heap t acc
        -- 追加フェーズ。開始時刻がt以下の商品をヒープへ
        | start <= t = solve rest (H.insert (H.Entry end start) heap) t acc
        -- ヒープが空ならtを次の要素のstartまで移動
        | H.null heap = solve remaining heap start acc
        -- ヒープから取り出して印字を行う
        | otherwise = tryPrint remaining heap t acc

      -- heapが空になるまでprintして、空になったらsolveに戻る
      tryPrint remaining heap t acc = case H.viewMin heap of
        Nothing -> solve remaining heap t acc
        Just (H.Entry end _, heap')
          -- 間に合わないので捨てる
          | end < t -> tryPrint remaining heap' t acc
          -- 印字できるのでtを進める
          | otherwise -> solve remaining heap' (t + 1) (acc + 1)

      -- heapの追加投入が必要ないtryPrint
      drain heap t acc = case H.viewMin heap of
        Nothing -> acc
        Just (H.Entry end _, heap')
          | end < t -> drain heap' t acc
          | otherwise -> drain heap' (t + 1) (acc + 1)

      res = solve tds' H.empty (fst (head tds')) (0 :: Integer)
  print res

-- WA: いもす的なやつは時間の範囲がでかすぎて無理
-- WA: 終端のsortだけではだめ
-- MEMO: H.Entryは優先度付きエントリで、優先度、値を持つ。今回は優先度にendを使っている