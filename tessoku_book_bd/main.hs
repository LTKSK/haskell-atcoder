...
powMod a b
  | b == 0 = 1
  | even b = powMod ((a * a) `mod` modulo) (b `div` 2)
  | otherwise = (a * powMod a (b - 1)) `mod` modulo

addmod :: Int -> Int -> Int
addmod a b = (a + b) `rem` modulo

multmod :: Int -> Int -> Int
multmod a b = (a * b) `rem` modulo

submod :: Int -> Int -> Int
submod a b = (a - b) `mod` modulo

bulldHash :: String -> VU.Vector Int
bulldHash s = VU.scanl' step 0 (VU.fromList s)
  where
    -- ordでaから97が得られるように、文字に対応する数値を取得
    step hash c = (hash * modulo + ord c) `mod` modulo
...
main = do
  ...
  -- ローリングハッシュを覚えよう
  let base = 10 ^ 5
      hashs = listArray @UArray (0, n) $ L.scanl' (\h c -> h `multmod` base + ord c) 0 s
      hash :: Int -> Int -> Int
      hash a b = (hashs ! b) `submod` (hashs ! (a - 1) `multmod` powMod base (b - a + 1))

  replicateM_ q $ do
    [a, b, c, d] <- ints
    printYn $ hash a b == hash c d
