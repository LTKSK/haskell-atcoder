...
main = do
  [n, q] <- ints
  -- ハノイの塔みたいな感じだ
  -- 重なってくから、操作は不可逆。操作の結果をどのように記録するかが肝
  -- 一度踏まれた場所は、他から移動しない限りは空
  -- 色々考えたけど愚直に操作を一旦組む
  -- pは必ず上にある。一方cはどの山のどこにいるかに制限はない

  -- 操作対象は常に二つ
  -- あるカードの上に何が乗っているかをなにかの手段で把握できれば移動は簡単か
  -- なんかグラフの組み換え的な問題な予感も...?
  -- 有効グラフ上で、あるnodeの繋がる先を変えるんだもんな

  -- 各番号がどこの上にいるかを追跡する。0に着いたらそこが番号
  -- そうか、移動時にどこにいるかわからなくなっちゃうのか？いや、pは必ずどこかの上にいることが保証されている
  res <- newArray (1, n) 0 :: IO (IOArray Int Int)
  replicateM_ q $ do
    [c, p] <- ints
    writeArray res c p
    return ()

  res' <- freeze res :: IO (Array Int Int)
  -- 経路復元！
  let m = M.fromList $ map (\(a, b) -> (b, a)) $ assocs res'
      res'' =
        -- 0じゃなかったら0（どこかに移動しているので）
        [ if v /= 0
            then 0
            else solve m k
          | (k, v) <- assocs res'
        ]
      solve m start = go m start 1
        where
          go m start acc = case M.lookup start m of
            Nothing -> acc
            Just v -> go m v (acc + 1)
