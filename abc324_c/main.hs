...
main = do
  (n, t) <- do
    ...
    let ws = BS.words line
    case ws of
      [n, t] -> case BS.readInt n of
        Just (num, _) -> return (num, BS.unpack t)
        Nothing -> error "数字のパースに失敗"
      _ -> error "フォーマットが違う"
  let lent = length t
  res <-
      ...
      <$> forM
        ...
        ( \i -> do
            ...
            let solve [] [] _ _ d = d
                solve [] _ _ lt d = d + lt
                solve _ [] ls _ d = d + ls
                solve s@(sc : scs) t@(tc : tcs) ls lt d
                  | d > 1 = d
                  | sc == tc = solve scs tcs (ls - 1) (lt - 1) d
                  | ls > lt = solve scs t (ls - 1) lt (d + 1)
                  | ls < lt = solve s tcs ls (lt - 1) (d + 1)
                  | otherwise = solve scs tcs (ls - 1) (lt - 1) (d + 1)
                res = solve s t (length s) lent 0
            return $ if res > 1 then Nothing else Just i
        )
  print $ length res
