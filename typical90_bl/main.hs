...
main = do
  [n, q] <- ints
  as <- ints
  lrvs <- replicateM q ints
  -- まず地殻変動前の不便さを計算する方法を考える
  -- 素直にfoldlっぽい
  -- 地殻変動が起こった範囲の絶対値の差は不変である。[l:r]範囲外の境界値だけ絶対値が変わる
  -- 境界で値が変わることに気付けたまでは良かったがその先がなかったな
  -- let diff = cum (zip as (tail as)) []
  --     cum [] acc = acc
  --     cum ((a, a') : as) acc = cum as (abs (a - a') : acc)
  -- e2-e1。zipの向きに注意
  let diff = zipWith (-) (tail as) as

  -- 添え字アクセスに+-1があるので番兵配置
  diff' <- newListArray @IOArray (0, n) (0 : diff ++ [0])
  foldM_
    ( \acc [l, r, v] ->
        do
          lv <- readArray diff' (l - 1)
          rv <- readArray diff' r
          let before = abs lv + abs rv

          when (l - 1 /= 0) $ do
            pv <- readArray diff' (l - 1)
            -- diff作成時ではなく計算時にabsを取る。このタイミングにしないとvの値を正しく計算できない
            -- +vしてからabsを取らねば
            writeArray diff' (l - 1) (pv + v)

          when (r /= n) $ do
            pv <- readArray diff' r
            writeArray diff' r (pv - v)

          lv' <- readArray diff' (l - 1)
          rv' <- readArray diff' r
          let after = abs lv' + abs rv'
