...
main = do
  ...
  g <- newArray @IOUArray ((1, 1), (h, w)) (0 :: Int)
  -- y*hにしないこと。wがy個分ある（1敗）
  let idxToNum (y, x) = y * w + x
  -- 1basedで配列を作ったので、範囲に注意
  uf <- newUf (h * w + w)
  forM_ qs $ \case
    [1, r, c] -> do
      v <- readArray g (r, c)
      ...
      writeArray g (r, c) 1
      -- 四方探索
      let ixs = [(y, x) | ix <- lrud, let (y, x) = (r, c) + ix, 1 <= y && y <= h && 1 <= x && x <= w]
      forM_ ixs $ \(y, x) -> do
        u <- readArray g (y, x)
        when (u == 1) $ void $ uniteUf uf (idxToNum (r, c)) (idxToNum (y, x))
    [2, ra, ca, rb, cb] -> do
      v <- readArray g (ra, ca)
      u <- readArray g (rb, cb)
      same <- sameUf uf (idxToNum (ra, ca)) (idxToNum (rb, cb))
      printYn $ v == 1 && u == 1 && same
