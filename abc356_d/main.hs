...
main = do
  [n, m] <- integers
  -- i=2とした時、
  -- p=4、full=8。ibit目が立つ整数は4,5,6,7,12,13,14,15,20,21,22,23
  -- 0,1,2,3は立たないので、p*2の後半にbitが立つという周期になっている
  -- なのでblockあたり後半p個のbitが立つという規則がある
  let countBit i = full * p + max 0 (rem' - p)
        where
          p = 1 `shiftL` i -- 2^i
          block = p * 2 -- (2^(i+1))
          -- 0~nまでなので+1
          full = (n + 1) `div` block -- 完全な周期
          rem' = (n + 1) `mod` block -- 端数
      res =
        sum
          [ countBit i
            | i <- [0 .. 60],
