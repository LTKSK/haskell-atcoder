...
main = do
  [n, m] <- ints
  abs <- replicateM m intwo
  let winRounds =
        M.fromListWith S.union $
          concat
            [ [(a, S.singleton i), (b, S.singleton i)]
              | (i, (a, b)) <- zip [1 ..] abs
            ]
      -- 半分以上勝っているプレイヤーを探す。
      -- プレイヤーが優勝できる回数の合計は2mまでなので、半分以上勝っている選手は多くても4人（m/2回が半分なので）
      ps = M.keys $ M.filter (\s -> S.size s * 2 >= m) winRounds
      -- 合計がm回になるものを探す
      isGood x y =
        let sx = M.findWithDefault S.empty x winRounds
            sy = M.findWithDefault S.empty y winRounds
         in S.size (S.union sx sy) == m
      pairs =
        S.fromList
          [ (min x y, max x y) | x <- ps, y <- [1 .. n], y /= x, isGood x y
          ]
