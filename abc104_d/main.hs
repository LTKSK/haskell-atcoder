...
data S = NULL | A | B | C deriving (Eq, Ix, Ord, Bounded, Show)

main :: IO ()
main = do
  s <- getLine
  -- aから遷移していって、Cになったらおしまい
  -- CになったタイミングでのそれまでのBの数の合計だな
  let dp = accumArrayDP @UArray next relax ini bnds v0s s
      relax = addMod
      bnds = (NULL, C)
      ini = 0
      v0s = [(NULL, 1)]
      next (v, x) u = case (v, u) of
        -- Aは道中複数出てくる。A,B,Cをそれぞれ拾う場合、拾わない場合で分岐が必要
        (NULL, 'A') -> [(NULL, x), (A, x)]
        (NULL, '?') -> [(NULL, x), (A, x), (NULL, x), (NULL, x)]
        (A, 'B') -> [(A, x), (B, x)]
        (A, '?') -> [(A, x), (B, x), (A, x), (A, x)]
        (B, 'C') -> [(B, x), (C, x)]
        (B, '?') -> [(B, x), (C, x), (B, x), (B, x)]
        (c, '?') -> [(c, x), (c, x), (c, x)]
        (c, _) -> [(c, x)] -- A, 'A'やB,'A'やB,'B'が来る
  print $ dp ! C

-- WA: 道中のA,B,Cを必ず選ぶと思って遷移を組んでしまった。実際はi文字目までどのAを使うか選ぶ必要がある