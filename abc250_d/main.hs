...
main = do
  [n] <- integers
  -- kはp*q^3
  -- 今回はn以下の個数を数え上げるから、先頭を使うだけじゃダメかな
  -- p<qの制約があるから、qとpの範囲はそれぞれ4√n。kもn以下の範囲なので探索範囲は4乗根>じゃなかった
  -- qの制約を見て3乗根までの素数が対象になる
  let primes = map (fromIntegral . fst) $ filter snd $ assocs $ sieve (10 ^ 6)
      len = length primes
      arr = listArray @Array (1, len) primes
      f :: Integer -> Int -> Bool
      f p i = let q = arr ! i in p < q && p * q * q * q <= n
      res =
        sum
          [ j - i
            | (i, p) <- zip [1 ..] primes,
              let j = binSearchMax (f p) i (len + 1)
                  q = arr ! i
          ]
  print res

-- memo: 素因数分解の性質からp,qから求まる値は一意に求まる