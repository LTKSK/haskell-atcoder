...
main = do
  ...
  -- 順によんで、transpose
  let cs1 = dropWhile (all (== '.')) ss
      cs1' = reverse $ dropWhile (all (== '.')) $ reverse cs1
      cs2 = dropWhile (all (== '.')) $ L.transpose $ cs1'
      cs2' = reverse $ dropWhile (all (== '.')) $ reverse cs2
  forM_ (L.transpose $ cs2') $ \row -> do
    putStrLn row

-- print cs'
-- print cs''
-- print $ L.transpose cs''
