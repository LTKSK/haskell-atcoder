...
main = do
  [h, w] <- ints
  cs <- listArray @UArray ((1, 1), (h, w)) . concat <$> replicateM h getLine
  let toIdx (y, x) = (y - 1) * w + x
      starts = map toIdx $ [ix | ix <- range (bounds cs), cs ! ix /= '#']
      -- 山を避けたgraphにしておく。隣接する頂点いずれも山であってはならないのに注意
      edges =
        -- 右への辺
        [[v, v + 1] | y <- [1 .. h], x <- [1 .. w - 1], let v = (y - 1) * w + x, cs ! (y, x) /= '#' && cs ! (y, x + 1) /= '#']
          -- 下の辺
          ++ [[v, v + w] | y <- [1 .. h - 1], x <- [1 .. w], let v = (y - 1) * w + x, cs ! (y, x) /= '#' && cs ! (y + 1, x) /= '#']
      -- 頂点番号からx,yの復元はx=i`mod`w, y=i`div`w
      graph = buildGraph (1, h * w) edges

  -- 頂点番号は1basedにしたいけど、bitの位置は0番目から始まるので1引いてる。頂点が1の時はbitは0になってほしい
  results <- forM starts $ \start -> do
    -- dp[到達済み頂点の集合][現在地] = 到達可否
    arr <- newArray @IOUArray ((0, 1), (2 ^ (h * w) - 1, w * h)) False
    writeArray arr ((1 :: Int) `shiftL` (start - 1), start) True
    forM_ [0 .. 2 ^ (h * w) - 1] $ \mask -> do
      forM_ [1 .. h * w] $ \v -> do
        reached <- readArray arr (mask, v)
        when reached $ do
          -- 隣接頂点走査
          forM_ (graph ! v) $ \u -> do
            -- 未訪問か判定
            when (mask .&. (1 `shiftL` (u - 1)) == 0) $ do
              writeArray arr (mask .|. (1 `shiftL` (u - 1)), u) True
    res <- newIORef (-1)
    forM_ [0 .. 2 ^ (h * w) - 1] $ \mask -> do
      forM_ [1 .. h * w] $ \v -> do
        reached <- readArray arr (mask, v)
        -- 移動回数は3以上でないとサイクルにならない
        when (reached && start `elem` (graph ! v) && popCount mask >= 3) $ do
          modifyIORef res (max (popCount mask))
    readIORef res
