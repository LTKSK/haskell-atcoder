...
swapIOUArray :: IOUArray Int Int -> Int -> Int -> IO (IOUArray Int Int)
swapIOUArray arr from to = do
  f <- readArray arr from
  t <- readArray arr to
  writeArray arr from t
  writeArray arr to f
  return arr
...
main = do
  ...
  as <- ints
  -- sortをして、その過程をログに残す
  -- 1..nまでがごちゃ混ぜになったasであるところが通常のsortと違う
  arr <- newListArray (1, n) as :: IO (IOUArray Int Int)
  pos <- newArray (1, n) 0 :: IO (IOUArray Int Int)
  -- aの値からindexの逆引き表
  forM_ (zip as [1 ..]) $ \(a, i) ->
    writeArray pos a i

  -- 入れ替えるindexを記録しないといけない
  -- aは添え字aの位置にあることを求められる
  res <- forM [1 .. n] $ \a -> do
    -- 現在のaの位置を特定
    i <- readArray pos a
    -- 現在の位置aの値を取得
    -- swap前はaの位置にbがあり、iの位置にaがいる
    b <- readArray arr a
    -- swap
    -- aはaの位置に移動
    writeArray arr a a
    ...
    -- posもswap。index>valueなのでbとiが逆
    writeArray pos a a
    writeArray pos b i
    -- aは添え字で見るとvalueのaが本来いる値。iがaが元々いたところ
    return (a, i)
  let res' = filter (uncurry (/=)) res
  print $ length res'
  forM_ res' $ \(a, b) -> do
