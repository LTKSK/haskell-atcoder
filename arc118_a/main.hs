...
main = do
  [t, n] <- ints
  -- 100個で1周期
  -- (100+t) / 100 * aとすると割り算で誤差が生まれる
  let f a = (100 + t) * a `div` 100
      -- 出てくる値を計算
      appear = S.fromList [f a | a <- [1 .. 100]]
      -- 100+tまでで出てこんかった値を集計
      missing = [v | v <- [1 .. 100 + t], not (S.member v appear)]
      -- 1周期毎に消える個数
      c = length missing
      -- qは何回周期があるか計算している
      -- nから1引かないと、nが丁度割り切れる周期のcの時、
      -- 例えばnが3でcも3の時、周期としては最初なので0になってほしいが、結果1になって次の周期扱いになってしまう
      -- これを回避するために-1している
      (q, r) = (n - 1) `divMod` c
  -- print $ (n - 1) `divMod` c
  -- print missing
  -- 100回ごとに100+tが追加される
  -- 何百回足されるかは、求めるn番目までに何回周期があるか==消える個数でnを割った回数でわかる
  -- print $ q * (100 + t)
  print $ q * (100 + t) + missing !! r
