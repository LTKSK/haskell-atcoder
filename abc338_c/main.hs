...
main = do
  ...
  as <- ints
  bs <- ints
  -- 一旦aに絞って、aiとqiが対応している。全てのiに対して、ai<=qiでければ料理が作れない
  -- この場合、qi `div` aiのminimumが作れる料理の最大値
  -- このminを取った値が、a,bそれぞれで料理を作れる回数
  -- ai,bi,qiに着目した時、qi/ai>qi/biなら、aiの方が値が小さいはず。なのでここまでは貪欲で良い
  -- よく読んだらNが10なので全探索やーん
  let aMim = minimum [if a /= 0 then q `div` a else q | (q, a) <- zip qs as]
      res =
        maximum
          [ v + i
            | i <- [0 .. aMim],
              let v =
                    minimum
                      [ res
                        | (a, b, q) <- zip3 as bs qs,
                          let res = if b == 0 then maxBound else (q - a * i) `div` b
                      ]
          ]
  print res
