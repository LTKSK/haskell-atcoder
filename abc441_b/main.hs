...
main = do
  ...
  [q] <- ints
  forM_ [1 .. q] $ \_ -> do
    w <- S.fromList <$> getLine
    let takahashi = S.isSubsetOf w s
        aoki = S.isSubsetOf w t
        isTakahashi = takahashi && not aoki
        isAoki = aoki && not takahashi
    putStrLn $ if isTakahashi then "Takahashi" else if isAoki then "Aoki" else "Unknown"
