...
main = do
  [a, b] <- ints
  -- これだとtle
  -- let res = maximum [gcd x y | x <- [a .. b - 1], y <- [x + 1 .. b]]
  -- a..bの範囲の数字しか登場しない。b-aをdとすると、1..dの倍数がどこかにはいるんだよな
  let res =
        maximum
          [ g
            | g <- [1 .. b - a],
              -- bまでに、gの倍数g,2g,3g...がいくつあるかはb div gで求まる
              -- (a-1)でaより手前の倍数の個数が求まるので、
              -- (a-1)をgが割り切るかは関係ない
              -- gの倍数が2つ以上あるならそれはx,yの組み合わせのgcd足り得るので採用
              b `div` g - (a - 1) `div` g >= 2
          ]
  print res
