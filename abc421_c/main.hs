...
main = do
  [n] <- ints
  s <- getLine
  let (as, bs) = L.foldl' (\(as, bs) (i, x) -> if x == 'A' then ((i, x) : as, bs) else (as, (i, x) : bs)) ([], []) $ zip [1 ..] s
      as' = reverse as
      bs' = reverse bs
      res1 = sum [max 0 (i - j) | (j, (i, _)) <- zip [1, 3 ..] as'] + sum [max 0 (i - j) | (j, (i, _)) <- zip [2, 4 ..] bs']
      res2 = sum [max 0 (i - j) | (j, (i, _)) <- zip [2, 4 ..] as'] + sum [max 0 (i - j) | (j, (i, _)) <- zip [1, 3 ..] bs']
  print $ min res1 res2
