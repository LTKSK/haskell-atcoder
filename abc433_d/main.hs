...
main = do
  [n, m] <- ints
  as <- ints
  let numDigit n = length $ show n
      -- 桁数ごとに、A_j mod Mの出現回数を管理する。singletonは要素が一つだけのmapを作っていて、それをunionWithでmergeしている
      byDigit = M.fromListWith (M.unionWith (+)) [(numDigit a, M.singleton (a `mod` m) 1) | a <- as]
      maxDig = numDigit (maximum as)
      -- mod計算を分割している。aでmodを取った値と10^kのmodを取った結果を乗算するのと先にaを乗算してmod取るのは同じ
      pow10 = M.fromList ([(k, (10 ^ k) `mod` m) | k <- [1 .. maxDig]])
      count a =
        sum
          [ M.findWithDefault 0 target digMap
            | k <- [1 .. maxDig],
              let target = (-(pow10 M.! k) * a) `mod` m, -- ここでmod取るのを忘れない
              let digMap = M.findWithDefault M.empty k byDigit
          ]
      res = sum [count a | a <- as]
  print res
