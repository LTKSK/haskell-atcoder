...
main = do
  ...
  bs <- ints
  cs <- ints
  -- let ds = listArray @UArray (0, n) $ L.scanl' (+) 0 as
  --     es = listArray @UArray (0, n) $ L.scanl' (+) 0 bs
  --     fs = listArray @UArray (0, n) $ L.scanl' (+) 0 cs
  --     gs = listArray @UArray (0, n) $ [ds ! i - es ! i | i <- [0 .. n]]
  --     hs = listArray @UArray (0, n) $ [es ! i - fs ! i | i <- [0 .. n]]
  --     maxGs =
  --       listArray @UArray (1, n) $ L.scanl1 max [gs ! i | i <- [1 .. n]]
