...
main = do
  ...
  as <- replicateM q $ do
    [a] <- ints
    pure a
  -- res = [asに含まれなかったpsの値] ++ [asに含まれるもので、出現順番が遅い順になっているもの]
  let (moved, as') = L.foldl' step ini (reverse as)
      -- 既に出てきたものを除外するset、出てきた順に入れるacc
      ini = (S.empty, [])
      step (s, acc) a
        | not $ S.member a s = (S.insert a s, a : acc)
        | otherwise = (s, acc)
      res = (filter (\p -> not $ S.member p moved) ps) ++ as'
