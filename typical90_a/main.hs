...
main = do
  ...
  -- 最大値kを2分探索する
  -- x以上になるように貪欲で取得
  -- 状態は、分割数、ここまでに切ったピースの長さ、現在集計中の長さ
  -- 最後のピースも計算に入れることを忘れない。貪欲なのでcutsはkより多くなるが、最後のピースが小さくてもほかの条件を満たしたピースにくっつければいいので問題ない
  let check x = (cuts + if (l - lastPos) >= x then 1 else 0) >= k + 1
        where
          (lastPos, cuts) = L.foldl' step init as
          init = (0, 0)
          step (lastPos, cuts) currentPos
            | (currentPos - lastPos) >= x = (currentPos, cuts + 1)
            | otherwise = (lastPos, cuts)
      ans = binSearch check 1 (l + 1)
  print ans
