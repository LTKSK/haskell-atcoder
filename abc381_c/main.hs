...
main = do
  ...
  s <- getLine
  let g = L.group s
      res = L.foldl' step ini $ zip3 g (tail g) (tail (tail g))
      ini = 0
      step acc (pre, cur, next)
        | isSlash && isOnes && isTwos = max acc (minLen * 2 + 1)
        | otherwise = acc
        where
          isSlash = head cur == '/' && length cur == 1
          isOnes = head pre == '1'
          isTwos = head next == '2'
          minLen = min (length pre) (length next)
  print $ max 1 res
