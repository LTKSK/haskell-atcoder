...
main = do
  ...
  as <- map (+ 1) <$> ints
  -- 操作を行うと肩の数字が1増えるっぽい？
  -- でもって2のべき乗だから、素直に計算しなくても肩が同じなら同じ値だ
  -- aは0もある。asを再帰していって、隣り合う数字が同じ時に計上していく操作がしたい
  -- 個数だけなのでlistで前に詰めていって良い
  let res = L.foldl' step init as
      init = []
      step acc a = go acc a
        where
          -- !_ = dbg (acc, a)
          ...
          go acc''@(p : acc') a
            -- ぷよぷよ
            | p == a = let next = p + 1 in go acc' next
            | otherwise = a : acc''
  print $ length res
