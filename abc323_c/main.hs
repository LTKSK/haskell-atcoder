...
main = do
  [n, m] <- ints
  as <- listArray @UArray (1, m) <$> ints
  ss <- replicateM n getLine
  let scores = [L.foldl' step (i, []) (zip [1 ..] s) | (i, s) <- zip [1 ..] ss]
      step (oks, ngs) (i, c)
        | c == 'x' = (oks, as ! i : ngs)
        | otherwise = (oks + as ! i, ngs)
      m = fst $ maximumBy (comparing fst) scores
  forM_ scores $ \(total, rem) -> do
    -- 素朴に人数分の最高得点を出して、残りの問題から必要な点数を出しましょう
    -- ここでi点が常に加算されることに注意が必要
    let solve [] acc count = count
        solve (r : rem) acc count
          | acc > m = count
          | otherwise = solve rem (acc + r) (count + 1)
    print $ if total == m then 0 else solve (L.sortBy (comparing Down) rem) total 0
