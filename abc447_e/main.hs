...
buildGraph :: (Int, Int) -> [[Int]] -> Array Int [(Int, Int)]
...
buildGraph (i, n) uvs = accumArray (flip (:)) [] (i, n) xs
  where
    ...
    xs = concatMap (\[u, v] -> [(u, (v, 2 ^ i)), (v, (u, 2 ^ i))]) uvs
...
main = do
  [n, m] <- ints
  uvs <- replicateM m ints
  -- 連結なので、辺を繋いでいけば必ず連結成分が1つになる
  -- 2つになったタイミングで止めれば良い
  -- 2^iの制約より、今見ている辺を残せるなら残すのが最適（後ろから見ると残りの辺すべてを削除するよりコストが高いので）
  let m = 998244353
  uf <- newUf n
  cRef <- newIORef n -- 連結成分の個数
  costRef <- newIORef (0 :: Int) -- 削除したコストの合計
  forM_ (reverse $ zip [1 ..] uvs) $ \(i, [u, v]) -> do
    c <- readIORef cRef
    same <- sameUf uf u v -- 連結かを判定
    if same
      then return () -- その辺は残してよい
      else
        if c > 2
          then do
            -- 連結成分が3個以上あるなら繋げてしまって良い
            uniteUf uf u v
            modifyIORef' cRef (subtract 1)
          else do
            -- c=2,連結成分が2つで異なる連結成分を繋ぐ辺は削除
            -- overflowするのでpowModしないとダメ
            modifyIORef' costRef (\acc -> (acc + powMod 2 i m) `mod` m)
  readIORef costRef >>= print
