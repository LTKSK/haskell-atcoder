...
main = do
  [n] <- ints
  txas <- replicateM n ints
  -- 各時刻で、1~5の穴にいた場合の最大値
  let txas' = head txas : zipWith (\[pt, _, _] [t, x, a] -> [t - pt, x, a]) txas (tail txas)
      dp = accumArrayDP @UArray next relax ini bnds v0s txas'
      relax = max
      ini = minBound
      bnds = (0, 4) -- 穴の数および時刻Tの制約からの最大値
      v0s = [(0, 0)]
      -- ナップザックと違い、どの穴を選ぶかについて、pt-timeの制約があるはず
      next (ph, pv) [timeDiff, hole, siz]
        | pv == ini = []
        | otherwise =
            -- 今の穴に到達できるかどうか+移動した先ですぬけ君を捕まえられるかどうか
            [ (nextHole, pv + if nextHole == hole then siz else 0)
              | nextHole <- [0 .. 4],
                abs (ph - nextHole) <= timeDiff -- 到達可能な穴に絞る
            ]
  print $ maximum [dp ! h | h <- [0 .. 4]]
