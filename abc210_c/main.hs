...
main = do
  [n, k] <- ints
  cs <- ints
  let arr = listArray @UArray (1, n) cs
      (res, _, _) = L.foldl' step ini $ zip [1 ..] (drop k cs)
      ini =
        let m :: M.Map Int Int = M.fromListWith (+) $ map (,1) (take k cs)
            s = S.fromList $ take k cs
         in (S.size s, s, m)
      -- setにすると複数ある場合を計上出来ないことに気付いた
      -- mapにするか...
      step (acc, s, m) (i, c) =
        -- 追加処理
        let m' = M.insertWith (+) c (1 :: Int) m
            v = fromMaybe 0 $ M.lookup (arr ! i) m'
            -- 減算処理
            m'' =
              if v == 1
                then M.delete (arr ! i) m'
                else M.insert (arr ! i) (v - 1) m'
            s' = if v == 1 then S.delete (arr ! i) s else s
            s'' = S.insert c s'
         in (max acc (S.size s''), s'', m'')
  print res
