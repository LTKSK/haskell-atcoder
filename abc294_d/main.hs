...
main = do
  ...
  let res = L.foldl' step ini qs
      ini = (S.fromList [1 .. n], S.empty, [])
      step (acc1, acc2, ans) q = case q of
        [1] ->
          let i = S.findMin acc1
           in (S.delete i acc1, S.insert i acc2, ans)
        [2, x] -> (acc1, S.delete x acc2, ans)
        [3] ->
          let i = S.findMin acc2
           in (acc1, acc2, i : ans)

  mapM_ print $ reverse $ thd3 res
