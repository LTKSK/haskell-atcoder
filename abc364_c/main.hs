...
import Control.Monad qualified as L
...
main = do
  [n, x, y] <- ints
  as <- L.sortBy (comparing Down) <$> ints
  bs <- L.sortBy (comparing Down) <$> ints

  let solveA [] count _ = count
      solveA (v : vs) count acc
        | acc > x = count
        | otherwise = solveA vs (count + 1) (acc + v)
  let solveB [] count _ = count
      solveB (v : vs) count acc
        | acc > y = count
        | otherwise = solveB vs (count + 1) (acc + v)
  print $ min (solveA as 0 0) (solveB bs 0 0)
