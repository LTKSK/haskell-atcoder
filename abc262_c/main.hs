...
main = do
  ...
  -- let as' = listArray @UArray (0, n) $ L.scanl' (+) (0 :: Int) $ map (\(i, a) -> if i == a then 1 else 0) $ zip [1 ..] as
  --     arr = listArray @UArray (1, n) as
  -- -- iを固定するとaiがiまたはjに一致していなければならない
  -- -- iより大きい要素で、
  -- -- aiと同じjの要素かつajがiに一致する
  -- -- またはaiがiと同じで、以降のjとaiが一致する数
  -- let res =
  --       sum
  --         [ c
  --           | i <- [1 .. n - 1],
  ...
  --                   if (ai == i)
  --                     then (as' ! n - as' ! i)
  --                     else if ai <= n && ai > i && arr ! ai == i then 1 else 0
  --         ]
  let arr = listArray @UArray (1, n) as
      k = length [i | i <- [1 .. n], arr ! i == i]
      pairs = length [i | i <- [1 .. n], let v = arr ! i, v > i, arr ! v == i]
  -- kC2
  print $ k * (k - 1) `div` 2 + pairs
