...
main = do
  ...
  t <- do
    ...
    return (t ++ "EE")
  let emptyPos s =
        let k = fst . head $ filter (\(i, c) -> c == 'E') $ zip [1 ..] s
         in k
      -- 添え字x,x+1を空いている位置に移動した関数を作る
      move s x =
        let k = emptyPos s
            a = s !! (x - 1)
            b = s !! x
         in [ c' | (i, c) <- zip [1 ..] s, let c'
                                                 | i == x || i == x + 1 = 'E'
                                                 | i == k = a
                                                 | i == k + 1 = b
                                                 | otherwise = c
      ...
      bfs' :: Seq.Seq String -> M.Map String Int -> IO Int
      bfs' q dist = case q of
        Seq.Empty -> return (-1)
        (s' Seq.:<| rest) -> do
          if s' == t
            then
            ...
            -- 次の探索
              ...
              -- distで未探索のものを探す
              -- foldを使って次のdistとqueueを作る
              let ns = filter (`M.notMember` dist) $ nexts s'
                  cur = dist M.! s'
                  dist' = L.foldl' (\acc k -> M.insert k (cur + 1) acc) dist ns
                  q' = L.foldl' (Seq.|>) rest ns
               in bfs' q' dist'
  res <- bfs' (Seq.singleton s) (M.singleton s 0)
  print res