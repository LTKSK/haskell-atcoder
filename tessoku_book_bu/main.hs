...
buildWeightedGraph (i, n) uvcds = accumArray (flip (:)) [] (i, n) xs
  where
    xs = concatMap (\[u, v, c, d] -> let cost = c * 10000 - d in [(u, (v, cost)), (v, (u, cost))]) uvcds
...
main = do
  [n, m] <- ints
  abcds <- replicateM m ints
  let g = buildWeightedGraph (1, n) abcds
      res = dijkstra g 1
      distance = ((res ! n) + 9999) `div` 10000
      -- 1本毎に1引いているので、その数が木の数。別途復元は不要
      countTree = distance * 10000 - (res ! n)
  putStrLn $ unwords $ map show [distance, countTree]