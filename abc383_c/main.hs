...
bfs' :: IOUArray (Int, Int) Int -> UArray (Int, Int) Char -> Seq.Seq (Int, Int) -> Int -> IO ()
bfs' dist grid queue d = case queue of
  Seq.Empty -> return ()
  (y, x) Seq.:<| rest -> do
...
main = do
  ...
  let grid = listArray @UArray ((1, 1), (h, w)) ss
      starts = [(y, x) | (y, x) <- range (bounds grid), grid ! (y, x) == 'H']
  forM_ starts $ \idx -> do
    writeArray visited idx 0
  bfs' visited grid (Seq.fromList starts) d

  res <- getElems visited
  print $ length $ filter (inRange (0, d)) res
